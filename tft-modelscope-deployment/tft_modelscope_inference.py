...
class ModelScopeTFT:
    ...
    def hierarchical_decomposition(self, data: np.ndarray) -> List[np.ndarray]:
        """N-HITS hierarchical decomposition for backup model"""
        ...
        if len(data) >= 4:
            level2 = np.array([np.mean(data[i:i+2]) for i in range(0, len(data)-1, 2)])
            levels.append(level2)
        
        # Level 3: Low frequency (4-day pooling) 
        if len(data) >= 8:
            level3 = np.array([np.mean(data[i:i+4]) for i in range(0, len(data)-3, 4)])
            levels.append(level3)
        
        return levels
    
    def interpolate_and_combine(self, levels: List[np.ndarray]) -> float:
        """N-HITS interpolation and combination"""
        
        if not levels:
            return 0.0
        
        # Weighted combination of different frequency levels
        weights = [0.6, 0.3, 0.1]  # High to low frequency weights
        combined_prediction = 0.0
        
        for i, level in enumerate(levels[:3]):  # Use up to 3 levels
            if len(level) > 0:
                # Simple prediction: trend from last few points
                if len(level) >= 2:
                    trend = level[-1] - level[-2]
                    prediction = level[-1] + trend * 0.1  # Small trend extrapolation
                else:
                    prediction = level[-1]
    ...
    def predict_with_nhits(self, sequence_data: List[List[float]]) -> Dict[str, Any]:
        """N-HITS backup prediction method"""
