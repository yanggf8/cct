 ...
 */
export async function handleMarketIntelligenceRoutes(request, env, path, headers) {
    ...
    try {
        ...
        logger.error('MarketIntelligenceRoutes Error', { error: error.message, requestId, path, method });
...
async function handleUnifiedDashboard(request, env, headers, requestId) {
    ...
    try {
        ...
        const unifiedInsights = generateUnifiedInsights(sectorRotation, driversSnapshot, timer.getElapsedMs());
        const dashboard = {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(dashboard, {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate unified market dashboard', 'DASHBOARD_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
async function handleMarketSynopsis(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(synopsis, {
            source: 'fresh',
            ttl: 900, // 15 minutes
            requestId,
            processingTime: timer.finish(),
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate market synopsis', 'SYNOPSIS_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
async function handleTopPicks(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate investment top picks', 'TOP_PICKS_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
async function handleRiskReport(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(riskReport, {
            source: 'analysis',
            ttl: 1800, // 30 minutes
            requestId,
            processingTime: timer.finish(),
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate risk report', 'RISK_REPORT_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
async function handleComprehensiveAnalysis(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(comprehensiveAnalysis, {
            source: 'fresh',
            ttl: 3600, // 1 hour
            requestId,
            processingTime: timer.finish(),
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to execute comprehensive market analysis', 'COMPREHENSIVE_ANALYSIS_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
function generateUnifiedInsights(sectorRotation, driversSnapshot, processingTime) {
    ...
    const favoredSectorsPerformance = driversSnapshot.regime.favoredSectors.map(favoredSector => {
        const sectorETF = sectorRotation.etfAnalyses.find(etf => etf.name.toLowerCase().includes(favoredSector.toLowerCase()));
        return {
...
function generateTopPicks(sectorRotation, driversSnapshot) {
    ...
    return sectorRotation.etfAnalyses
        ...
        const isFavored = driversSnapshot.regime.favoredSectors.some(favored => etf.name.toLowerCase().includes(favored.toLowerCase()));
...
function calculateRiskScore(driversSnapshot) {
    ...
    // VIX contribution
    if (driversSnapshot.marketStructure.vix > 30)
        score += 0.2;
    else if (driversSnapshot.marketStructure.vix > 20)
        score += 0.1;
    // Yield curve contribution
    if (driversSnapshot.macro.yieldCurveSpread < -1)
        score += 0.2;
    else if (driversSnapshot.macro.yieldCurveSpread < 0)
        score += 0.1;
...
function calculateAlignmentScore(favoredSectorsPerformance) {
    if (favoredSectorsPerformance.length === 0)
        return 0.5;
...
//# sourceMappingURL=market-intelligence-routes.js.map