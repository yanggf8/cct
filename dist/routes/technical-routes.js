...
import { validateSymbol, validateSymbols, validateRequestBody, ValidationError } from '../modules/validation.js';
 ...
 */
export async function handleTechnicalRoutes(request, env, path, headers) {
    const method = request.method;
    const requestId = headers['X-Request-ID'] || generateRequestId();
    // Require API key
    const auth = validateApiKey(request, env);
    if (!auth.valid) {
        return new Response(JSON.stringify(ApiResponseFactory.error('Invalid or missing API key', 'UNAUTHORIZED', { requestId })), { status: HttpStatus.UNAUTHORIZED, headers });
    }
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error(`Method ${method} not allowed for ${path}`, 'METHOD_NOT_ALLOWED', { requestId })), { status: HttpStatus.METHOD_NOT_ALLOWED, headers });
    }
    catch (error) {
        logger.error('TechnicalRoutes Error', { error: error.message, requestId, path, method });
        return new Response(JSON.stringify(ApiResponseFactory.error('Internal server error', 'INTERNAL_ERROR', { requestId, error: error.message })), { status: HttpStatus.INTERNAL_SERVER_ERROR, headers });
    }
}
...
async function handleTechnicalBatch(request, env, headers, requestId) {
    ...
    try {
        ...
        const { runIndependentTechnicalAnalysis } = await import('../modules/independent_technical_analysis.js');
        const result = await runIndependentTechnicalAnalysis(symbols, env);
...
//# sourceMappingURL=technical-routes.js.map