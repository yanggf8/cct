...
import { validateApiKey, parseQueryParams, extractSymbolsParam, generateRequestId } from './api-v1.js';
 ...
 */
export async function handleSentimentRoutes(request, env, path, headers) {
...
async function handleSentimentAnalysis(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform sentiment analysis', 'ANALYSIS_ERROR', { requestId, error: error instanceof Error ? error.message : 'Unknown error' })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
            headers,
        });
    }
...
async function handleSymbolSentiment(symbol, request, env, headers, requestId) {
    ...
    try {
        ...
        if (!analysisResult.results || analysisResult.results.length === 0 || analysisResult.results[0].error) {
            return new Response(JSON.stringify(ApiResponseFactory.error('No analysis data available for symbol', 'NO_DATA', { requestId, symbol })), {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform symbol sentiment analysis', 'ANALYSIS_ERROR', {
            requestId,
            symbol,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
async function handleMarketSentiment(request, env, headers, requestId) {
    ...
    try {
        ...
        if (!analysisData || !analysisData.trading_signals) {
            return new Response(JSON.stringify(ApiResponseFactory.error('No market data available', 'NO_DATA', { requestId })), {
                status: HttpStatus.NOT_FOUND,
                headers,
            });
        }
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to compute market sentiment', 'ANALYSIS_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
...
async function handleSectorSentiment(request, env, headers, requestId) {
    ...
    try {
        ...
        if (sectors.length === 0) {
            return new Response(JSON.stringify(ApiResponseFactory.error('No sectors specified. Use ?sectors=XLK,XLE,XLF', 'NO_SECTORS', { requestId })), {
                status: HttpStatus.BAD_REQUEST,
                headers,
            });
        }
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform sector sentiment analysis', 'ANALYSIS_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
