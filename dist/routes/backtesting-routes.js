...
import { validateApiKey, parseQueryParams, generateRequestId as generateApiRequestId } from './api-v1.js';
 ...
 */
export async function handleBacktestingRoutes(request, env, path, headers) {
...
async function handleRunBacktest(request, env, headers, requestId) {
    ...
    try {
        ...
        if (validationErrors.length > 0) {
            return new Response(JSON.stringify(ApiResponseFactory.error('Invalid backtest configuration', 'INVALID_CONFIG', {
                requestId,
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to start backtest', 'BACKTEST_START_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
...
async function handleBacktestStatus(backtestId, request, env, headers, requestId) {
    ...
    try {
        ...
        // Backtest not found
        return new Response(JSON.stringify(ApiResponseFactory.error('Backtest not found', 'NOT_FOUND', { requestId, backtestId })), {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to get backtest status', 'STATUS_ERROR', {
            requestId,
            backtestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
async function handleGetBacktestResults(backtestId, request, env, headers, requestId) {
    ...
    try {
        ...
        // Result not found
        return new Response(JSON.stringify(ApiResponseFactory.error('Backtest results not found', 'NOT_FOUND', { requestId, backtestId })), {
            status: HttpStatus.NOT_FOUND,
            headers,
        });
    }
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to get backtest results', 'RESULTS_ERROR', {
            requestId,
            backtestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
async function handleGetPerformanceMetrics(backtestId, request, env, headers, requestId) {
    ...
    try {
        ...
        if (!metrics) {
            ...
            return new Response(JSON.stringify(ApiResponseFactory.success(performanceDetails, {
                source: 'storage',
                requestId,
                processingTime: timer.finish()
            })), { status: HttpStatus.OK, headers });
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(metrics, {
            source: 'cached',
            requestId,
            processingTime: timer.finish()
        })), { status: HttpStatus.OK, headers });
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to get performance metrics', 'PERFORMANCE_ERROR', {
            requestId,
            backtestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
async function handleCompareBacktests(request, env, headers, requestId) {
    ...
    try {
        ...
        if (!requestBody.backtestIds || requestBody.backtestIds.length < 2) {
            return new Response(JSON.stringify(ApiResponseFactory.error('At least 2 backtest IDs required for comparison', 'INVALID_REQUEST', { requestId })), {
                status: HttpStatus.BAD_REQUEST,
                headers,
            });
        }
        ...
        if (backtestResults.length < 2) {
            return new Response(JSON.stringify(ApiResponseFactory.error('Insufficient valid backtest results found', 'INSUFFICIENT_DATA', { requestId, foundCount: backtestResults.length })), {
                status: HttpStatus.BAD_REQUEST,
                headers,
            });
        }
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to compare backtests', 'COMPARISON_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
...
async function handleBacktestHistory(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to get backtest history', 'HISTORY_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
...
async function handleGetValidationResults(backtestId, request, env, headers, requestId) {
    ...
    try {
        ...
        if (cached.success && cached.data) {
            return new Response(JSON.stringify(ApiResponseFactory.cached(cached.data, 'hit', {
                source: 'cache',
                requestId,
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success({
            status: 'running',
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to get validation results', 'VALIDATION_ERROR', {
            requestId,
            backtestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
async function handleWalkForwardOptimization(backtestId, request, env, headers, requestId) {
    ...
    try {
        ...
        if (wfCached.success && wfCached.data) {
            return new Response(JSON.stringify(ApiResponseFactory.cached(wfCached.data, 'hit', {
                source: 'cache',
                requestId,
                processingTime: timer.getElapsedMs(),
            })), { status: HttpStatus.OK, headers });
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to start walk-forward optimization', 'WALK_FORWARD_ERROR', {
            requestId,
            backtestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
async function handleMonteCarloSimulation(backtestId, request, env, headers, requestId) {
    ...
    try {
        ...
        if (mcCached.success && mcCached.data) {
            return new Response(JSON.stringify(ApiResponseFactory.cached(mcCached.data, 'hit', {
                source: 'cache',
                requestId,
                processingTime: timer.getElapsedMs(),
            })), { status: HttpStatus.OK, headers });
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to start Monte Carlo simulation', 'MONTE_CARLO_ERROR', {
            requestId,
            backtestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
function estimateBacktestDuration(config) {
    ...
    const yearsMultiplier = Math.ceil((new Date(config.data.endDate).getTime() - new Date(config.data.startDate).getTime()) /
        (1000 * 60 * 60 * 24 * 365)) * 10; // 10 seconds per year
...
async function runValidationInBackground(backtestId, result, env) {
    try {
        const storage = createBacktestingStorage(env);
        const cache = createBacktestingCache(env);
        const validator = createModelValidator(result.config, env, result.equityCurve, result.trades, result.positions);
...
async function runWalkForwardInBackground(backtestId, result, env) {
    try {
        const storage = createBacktestingStorage(env);
        const cache = createBacktestingCache(env);
        const optimizer = createWalkForwardOptimizer(result.config, env, result.equityCurve, result.trades, result.positions);
...
async function runMonteCarloInBackground(backtestId, result, env, numSimulations) {
    try {
        const storage = createBacktestingStorage(env);
        const cache = createBacktestingCache(env);
        const simulator = createMonteCarloSimulator(result.config, env, result.equityCurve, result.trades, result.positions);
...
async function handleModelValidation(request, env, headers, requestId) {
    ...
    try {
        const requestBody = (await request.json());
        const { backtestId: backtestId = '', validationConfig = {
            crossValidation: {
        ...
        if (!backtestId) {
            return new Response(JSON.stringify(ApiResponseFactory.error('Backtest ID is required', 'INVALID_REQUEST', { requestId })), {
                status: HttpStatus.BAD_REQUEST,
                headers,
            });
        }
        ...
        const cached = await dal.read(`backtest_validation_${backtestId}`);
        if (cached.success && cached.data) {
            return new Response(JSON.stringify(ApiResponseFactory.success(cached.data, {
                source: 'cached',
                requestId,
                processingTime: timer.finish()
            })), { status: HttpStatus.OK, headers });
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(validation, {
            source: 'fresh',
            requestId,
            processingTime: timer.finish()
        })), { status: HttpStatus.OK, headers });
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform model validation', 'VALIDATION_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
async function handleMonteCarloSimulationDirect(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(monteCarlo, {
            source: 'fresh',
            requestId,
            processingTime: timer.finish()
        })), { status: HttpStatus.OK, headers });
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform Monte Carlo simulation', 'MONTE_CARLO_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
//# sourceMappingURL=backtesting-routes.js.map