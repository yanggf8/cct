...
import { validateOptionalField, ValidationError } from '../modules/validation.js';
 ...
 */
export async function handleDataRoutes(request, env, path, headers) {
    ...
    try {
        ...
        logger.error('DataRoutes Error', { error: error.message, requestId, path, method });
...
async function handleAvailableSymbols(request, env, headers, requestId) {
    ...
    try {
        ...
        logger.error('AvailableSymbols Error', { error: error.message, requestId });
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to retrieve available symbols', 'DATA_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
...
async function handleSymbolHistory(symbol, request, env, headers, requestId) {
    ...
    try {
        ...
        if (!symbol || symbol.length > 10) {
            return new Response(JSON.stringify(ApiResponseFactory.error('Invalid symbol format', 'INVALID_SYMBOL', { requestId, symbol })), {
        ...
        const days = validateOptionalField(params.days ? parseInt(params.days) : 30, (val) => {
        ...
        if (cached) {
            ...
            return new Response(JSON.stringify(ApiResponseFactory.cached(cached, 'hit', {
                source: 'cache',
                ttl: 1800, // 30 minutes
                requestId,
                processingTime: timer.getElapsedMs(),
        ...
        logger.error('SymbolHistory Error', { error: error.message, requestId, symbol });
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to retrieve symbol history', 'DATA_ERROR', {
            requestId,
            symbol,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
async function handleModelHealth(request, env, headers, requestId) {
    ...
    try {
        ...
        logger.error('ModelHealth Error', { error: error.message, requestId });
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform model health check', 'HEALTH_CHECK_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error',
            processingTime: timer.finish()
        })), {
...
async function handleCronHealth(request, env, headers, requestId) {
    ...
    try {
        ...
        logger.error('CronHealth Error', { error: error.message, requestId });
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform cron health check', 'HEALTH_CHECK_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error',
            processingTime: timer.finish()
        })), {
...
async function handleSystemHealth(request, env, headers, requestId) {
    ...
    try {
        ...
        logger.error('SystemHealth Error', { error: error.message, requestId });
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform system health check', 'HEALTH_CHECK_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
...
async function handleKVSelfTest(request, env, headers, requestId) {
    ...
    try {
        ...
        if (!env.MARKET_ANALYSIS_CACHE) {
            ...
            return new Response(JSON.stringify(ApiResponseFactory.error('KV binding not found', 'KV_BINDING_MISSING', {
                requestId,
                testResults,
                availableBindings: Object.keys(env).filter(key => !key.startsWith('__') && typeof env[key] === 'object')
            })), {
                status: HttpStatus.INTERNAL_SERVER_ERROR,
                headers,
            });
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Cache abstraction test failed with exception', 'KV_SELFTEST_ERROR', {
            requestId,
            testResults,
            error: error instanceof Error ? error.message : String(error),
...
async function handleShowBindings(request, env, headers, requestId) {
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to retrieve bindings', 'BINDINGS_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : String(error)
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
