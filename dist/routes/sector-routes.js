 ...
 */
export async function getSectorSnapshot(request, env) {
    ...
    try {
        ...
        try {
            ...
            if (!services.circuitBreaker.canExecute()) {
                const body = ApiResponseFactory.error('Service temporarily unavailable due to high error rate', 'SECTOR_API_CIRCUIT_OPEN');
            ...
            if (cachedSnapshot) {
                ...
                const body = ApiResponseFactory.success(cachedSnapshot, {
                    cacheHit: true,
                    responseTime,
            ...
            const body = ApiResponseFactory.success(freshData, {
                cacheHit: false,
                responseTime,
            ...
            const responseTime = Date.now() - startTime;
            const body = ApiResponseFactory.success(fallbackData, { message: 'Sector snapshot generated with fallback data' });
            return new Response(JSON.stringify(body), { status: 200 });
        }
    }
    catch (error) {
        ...
        const body = ApiResponseFactory.error('Failed to generate sector snapshot', 'SECTOR_SNAPSHOT_ERROR', {
            error: error.message,
            responseTime,
            cacheHit
        });
...
async function fetchFreshSectorData(services) {
    ...
    for (const sector of sectorData) {
        try {
            ...
            if (historicalData.length >= 20) { // Minimum data for indicators
                ...
                const indicators = await services.indicators.calculateAllIndicators(sector.symbol, historicalData, spyData);
                if (indicators) {
                    sector.indicators = {
                        obv: indicators.obv ? {
                    ...
                    if (indicators.overallSignal === 'bullish')
                        bullishCount++;
                    else if (indicators.overallSignal === 'bearish')
                        bearishCount++;
                    else
                        neutralCount++;
    ...
    if (sectorData.length > 0) {
        ...
        topPerformer = sectorData.reduce((best, current) => current.changePercent > best.changePercent ? current : best);
        worstPerformer = sectorData.reduce((worst, current) => current.changePercent < worst.changePercent ? current : worst);
    }
 ...
 */
export async function getSectorHealth(request, env) {
    try {
        const services = initializeSectorServices(env);
        const cacheStats = services.cacheManager
            ? await services.cacheManager.getCacheStats()
        ...
        const body = ApiResponseFactory.success(health, { message: isHealthy ? 'All sector services operational' : 'Some services degraded' });
        return new Response(JSON.stringify(body), { status: 200 });
    }
    catch (error) {
        logger.error('Error in getSectorHealth:', { error: error instanceof Error ? error.message : String(error) });
        const body = ApiResponseFactory.error('Sector health check failed', 'SECTOR_HEALTH_ERROR');
        return new Response(JSON.stringify(body), { status: 500 });
    }
}
/**
 ...
 */
export async function getSectorSymbols(request, env) {
    try {
        ...
        const body = ApiResponseFactory.success({ symbols, total: symbols.length }, { message: 'Sector symbols retrieved successfully' });
        return new Response(JSON.stringify(body), { status: 200 });
    }
    catch (error) {
        logger.error('Error in getSectorSymbols:', { error: error instanceof Error ? error.message : String(error) });
        const body = ApiResponseFactory.error('Failed to retrieve sector symbols', 'SECTOR_SYMBOLS_ERROR');
        return new Response(JSON.stringify(body), { status: 500 });
    }
}
/**
...
async function generateSimpleSectorSnapshot() {
    ...
    const topPerformer = sectors.reduce((best, current) => current.changePercent > best.changePercent ? current : best);
    const worstPerformer = sectors.reduce((worst, current) => current.changePercent < worst.changePercent ? current : worst);
    return {
        timestamp,
        date: new Date().toISOString().split('T')[0],
        sectors,
 ...
 */
export async function getSectorIndicatorsSymbol(request, env, symbolParam) {
