 ...
 * Main v1 API Router
 */
export async function handleApiV1Request(request, env, path) {
    ...
    try {
        ...
        if (!securityCheck.allowed) {
            ...
            const errorResponse = ApiResponseFactory.error(securityCheck.reason === 'API_KEY_LOCKED_OUT' ? 'API key temporarily locked due to repeated failures' :
                securityCheck.reason === 'IP_RATE_LIMIT_EXCEEDED' ? 'IP rate limit exceeded' :
                    'API rate limit exceeded', securityCheck.reason === 'API_KEY_LOCKED_OUT' ? 'API_KEY_LOCKED_OUT' :
        ...
        if (path.startsWith('/api/v1/sentiment/')) {
            ...
            if (!securityCheck.authenticated) {
                const errorResponse = ApiResponseFactory.error('Authentication required for production guards endpoints', 'AUTHENTICATION_REQUIRED', {
                    requestId: headers['X-Request-ID'],
                    endpoint: path,
                    requires_auth: true
                });
            ...
            if (requestedPath === '/cache/health')
                cachePath = '/cache-health';
            else if (requestedPath === '/cache/metrics')
                cachePath = '/cache-metrics';
            else if (requestedPath === '/cache/config')
                cachePath = '/cache-config';
            else if (requestedPath === '/cache/promote')
                cachePath = '/cache-promotion';
            else if (requestedPath === '/cache/warmup')
                cachePath = '/cache-warmup';
            else if (requestedPath === '/cache/stats')
                cachePath = '/cache-system-status';
            else if (requestedPath === '/cache/status')
                cachePath = '/cache-system-status';
            else if (requestedPath === '/cache/timestamps')
                cachePath = '/cache-timestamps';
            else if (requestedPath === '/cache/debug')
                cachePath = '/cache-debug';
            else if (requestedPath === '/cache/deduplication')
                cachePath = '/cache-deduplication';
            ...
            if (!auth.valid) {
                return new Response(JSON.stringify(ApiResponseFactory.error('Invalid or missing API key', 'UNAUTHORIZED', { requestId: headers['X-Request-ID'] })), { status: HttpStatus.UNAUTHORIZED, headers });
            }
