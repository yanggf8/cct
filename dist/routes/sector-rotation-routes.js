 ...
 */
export async function handleSectorRotationRoutes(request, env, path, headers) {
    ...
    try {
        ...
        logger.error('SectorRotationRoutes Error', { error: error.message, requestId, path, method });
...
async function handleSectorRotationAnalysis(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(result, {
            source: 'fresh',
            ttl: 3600,
            requestId,
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform sector rotation analysis', 'ANALYSIS_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
async function handleSectorRotationResults(request, env, headers, requestId) {
    ...
    try {
        ...
        if (cachedResults) {
            ...
            return new Response(JSON.stringify(ApiResponseFactory.success(cachedResults, {
                source: 'cache',
                ttl: 3600,
                requestId,
                processingTime: timer.finish(),
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error(`No sector rotation analysis available for ${targetDate}. Run the analysis first.`, 'NO_DATA', {
            requestId,
            date: targetDate,
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to retrieve sector rotation results', 'RETRIEVAL_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
async function handleSectorInformation(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to retrieve sector information', 'SECTOR_INFO_ERROR', {
            requestId,
            error: error.message,
            processingTime: timer.finish()
        })), {
...
async function handleETFAnalysis(symbol, request, env, headers, requestId) {
    ...
    try {
        // Validate ETF symbol
        if (!SPDR_ETFs[symbol]) {
            return new Response(JSON.stringify(ApiResponseFactory.error(`Invalid ETF symbol: ${symbol}. Valid symbols: ${Object.keys(SPDR_ETFs).join(', ')}`, 'INVALID_SYMBOL', { requestId, symbol })), {
        ...
        if (!cachedResults) {
            return new Response(JSON.stringify(ApiResponseFactory.error(`No analysis data available for ${symbol}. Run sector rotation analysis first.`, 'NO_ANALYSIS_DATA', {
                requestId,
                symbol,
                suggestion: 'POST /api/v1/sector-rotation/analysis to generate new analysis'
            })), {
        ...
        if (!etfAnalysis) {
            return new Response(JSON.stringify(ApiResponseFactory.error(`No analysis data found for ${symbol} in today's results`, 'NO_ETF_DATA', { requestId, symbol })), {
                status: HttpStatus.NOT_FOUND,
                headers,
            });
        }
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to retrieve ETF analysis', 'ETF_ANALYSIS_ERROR', {
            requestId,
            symbol,
            error: error.message,
            processingTime: timer.finish()
...
//# sourceMappingURL=sector-rotation-routes.js.map