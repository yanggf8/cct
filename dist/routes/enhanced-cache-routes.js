...
import { StorageGuards } from '../modules/storage-guards.js';
 ...
 */
function generateMarketDataWarmup(symbols) {
...
function generateSentimentAnalysisWarmup(symbols) {
    return symbols.map(symbol => ({
...
function generateBasicSentimentWarmup(symbols) {
    return symbols.map(symbol => ({
        namespace: 'sentiment_analysis',
...
function generateSectorDataWarmup() {
    const sectors = [
...
function generatePredictiveModelsWarmup() {
    return [
        {
...
function generateReportsWarmup() {
...
function generateWeeklyReportsWarmup() {
...
function generateComprehensiveDataWarmup(symbols) {
    return [
...
function generateRandomStockSymbols(count) {
    const stockPool = [
 ...
 */
export function createEnhancedCacheRoutes(env) {
    ...
    const routes = [
        ...
        },
        {
            ...
            handler: async (request, env, ctx) => {
                try {
                    ...
                    const { strategy = 'comprehensive', namespaces = [], preload_symbols = ['AAPL', 'MSFT', 'GOOGL'], force_refresh = false } = body;
        ...
        },
        {
            ...
            handler: async (request, env, ctx) => {
                try {
                    ...
                    const { operation = 'get', key = 'drill_test_analysis_AAPL_2024-01-01', storageClass = 'hot_cache', mode = 'warn', latencyMs, caller = 'admin_drill' } = body;
                    // Validate inputs
                    if (!key || !storageClass) {
                        return new Response(JSON.stringify({
                            success: false,
        ...
        },
        {
            ...
            handler: async (request, env, ctx) => {
                try {
                    const body = await request.json();
                    const { storageClass = 'hot_cache', key = 'test_forbidden_kv_operation', mode = 'warn' } = body;
        ...
        },
        {
            ...
            handler: async (request, env, ctx) => {
                try {
                    ...
                    const d1Storage = new D1ColdStorage(env.ANALYTICS_DB);
                    const result = await d1Storage.upsertRollup(body.day, body.keyspace, body.storageClass, body.metrics);
                    if (result.success) {
                        return new Response(JSON.stringify({
                            success: true,
                            timestamp: new Date().toISOString(),
...
//# sourceMappingURL=enhanced-cache-routes.js.map