...
export class MigrationManager {
    ...
    async recordPerformanceComparison(endpoint, legacyTime, newTime, legacySuccess, newSuccess) {
        ...
        comparison.improvement.overallImprovement = (comparison.improvement.responseTimeImprovement * 0.7 +
    ...
    async getMigrationStatistics() {
        ...
        const recommendations = this.generateRecommendations(totalEvents, legacyRequests, newAPIRequests, successRate, performance);
        return {
            events: {
                total: totalEvents,
                legacyRequests,
     ...
     */
    generateRecommendations(totalEvents, legacyRequests, newAPIRequests, successRate, performance) {
    ...
    cleanupOldEvents() {
        ...
        this.migrationEvents = this.migrationEvents.filter(event => new Date(event.timestamp).getTime() > cutoffTime);
    }
    /**
 ...
 */
let globalMigrationManager = null;
/**
 * Get or create global migration manager
 */
export function getMigrationManager(env, config) {
 ...
 */
export async function migrationMiddleware(request, env, endpoint) {
