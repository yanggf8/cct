...
import { initializeIntegrationTestSuite } from '../modules/integration-test-suite.js';
import { createLogger } from '../modules/logging.js';
const logger = createLogger('integration-test-routes');
/**
 * Handle all integration test routes
 */
export async function handleIntegrationTestRoutes(request, env, path, headers) {
    ...
    try {
        ...
        logger.error('IntegrationTestRoutes Error', { error: error.message, requestId, path, method });
 ...
 */
async function handleRunFullTestSuite(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(results, {
            source: 'live',
            ttl: 300, // 5 minutes
            requestId,
        ...
        logger.error('RunFullTestSuite Error', { error: error.message, requestId });
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to run integration test suite', 'TEST_SUITE_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
 ...
 */
async function handleHealthCheck(request, env, headers, requestId) {
    const timer = new ProcessingTimer();
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to perform health check', 'HEALTH_CHECK_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
 ...
 */
async function handleTestStatus(request, env, headers, requestId) {
    const timer = new ProcessingTimer();
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.success(status, {
            source: 'static',
            ttl: 300,
            requestId,
            processingTime: timer.finish(),
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to get test status', 'STATUS_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
