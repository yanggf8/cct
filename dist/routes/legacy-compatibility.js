...
function addDeprecationHeaders(response, oldPath, newPath) {
    ...
    headers.set(DEPRECATION_CONFIG.warningHeader, `This endpoint is deprecated and will be removed on ${DEPRECATION_CONFIG.sunsetDate}. ` +
 ...
 */
export async function handleLegacyEndpoint(request, env, oldPath) {
 ...
 */
export function isLegacyEndpoint(path) {
    return path in LEGACY_MAPPINGS;
}
/**
 * Get new endpoint path for legacy endpoint
 */
export function getNewEndpointPath(legacyPath) {
    return LEGACY_MAPPINGS[legacyPath] || null;
}
/**
 ...
 */
export function legacyCompatibilityMiddleware(request, env) {
    const url = new URL(request.url);
 ...
 */
export async function handleLegacyEndpointWithTracking(request, env, oldPath) {
