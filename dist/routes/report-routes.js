 ...
 */
function extractDateParam(url, paramName) {
    return url.searchParams.get(paramName);
}
/**
 * Handle all report routes
 */
export async function handleReportRoutes(request, env, path, headers) {
...
async function handleDailyReport(date, request, env, headers, requestId) {
    ...
    try {
        ...
        if (!dateRegex.test(date)) {
            return new Response(JSON.stringify(ApiResponseFactory.error('Invalid date format. Expected YYYY-MM-DD', 'INVALID_DATE', { requestId, date })), {
        ...
        if (!analysisData || !analysisData.trading_signals) {
            return new Response(JSON.stringify(ApiResponseFactory.error('No daily report data available for this date', 'NO_DATA', { requestId, date })), {
        ...
        const response = {
            ...
            report: {
                ...
                recommendations: signals
                    ...
                    .slice(0, 5)
                    .map(signal => signal.symbol || 'UNKNOWN'),
            },
            metadata: {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate daily report', 'REPORT_ERROR', {
            requestId,
            date,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
async function handleWeeklyReport(week, request, env, headers, requestId) {
    ...
    try {
        ...
        if (!weekRegex.test(week)) {
            return new Response(JSON.stringify(ApiResponseFactory.error('Invalid week format. Expected YYYY-W## (e.g., 2025-W41)', 'INVALID_WEEK', { requestId, week })), {
                status: HttpStatus.BAD_REQUEST,
                headers,
            });
        }
        ...
        if (dailyReports.length === 0) {
            return new Response(JSON.stringify(ApiResponseFactory.error('No weekly report data available for this period', 'NO_DATA', { requestId, week })), {
                status: HttpStatus.NOT_FOUND,
                headers,
            });
        }
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate weekly report', 'REPORT_ERROR', {
            requestId,
            week,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
...
async function handlePreMarketReport(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate pre-market report', 'REPORT_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
...
async function handlePreMarketDataGeneration(request, env, headers, requestId) {
    ...
    try {
        ...
        let symbols = ['AAPL', 'MSFT', 'GOOGL', 'TSLA', 'NVDA']; // Default symbols
        try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate pre-market data', 'GENERATION_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
...
async function handleIntradayReport(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate intraday report', 'REPORT_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
...
async function handleEndOfDayReport(request, env, headers, requestId) {
    ...
    try {
        ...
        return new Response(JSON.stringify(ApiResponseFactory.error('Failed to generate end-of-day report', 'REPORT_ERROR', {
            requestId,
            error: error instanceof Error ? error.message : 'Unknown error'
        })), {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
