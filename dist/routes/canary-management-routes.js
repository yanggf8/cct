 ...
 */
export async function handleCanaryStatus(request, env) {
    try {
        ...
        return ApiResponseFactory.error('Failed to retrieve canary status', 'CANARY_STATUS_ERROR', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
}
/**
 * Handle canary configuration update
 */
export async function handleCanaryUpdate(request, env) {
    try {
        ...
        return ApiResponseFactory.error('Failed to update canary configuration', 'CANARY_UPDATE_ERROR', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
}
/**
 * Handle canary enable request
 */
export async function handleCanaryEnable(request, env) {
    try {
        ...
        return ApiResponseFactory.error('Failed to enable canary', 'CANARY_ENABLE_ERROR', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
}
/**
 * Handle canary disable request
 */
export async function handleCanaryDisable(request, env) {
    try {
        const url = new URL(request.url);
        const route = url.searchParams.get('route');
        if (!route) {
        ...
        return ApiResponseFactory.error('Failed to disable canary', 'CANARY_DISABLE_ERROR', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
}
/**
 * Handle canary traffic simulation
 */
export async function handleCanarySimulate(request, env) {
    try {
        const body = await request.json();
        ...
        return ApiResponseFactory.error('Failed to simulate canary traffic', 'CANARY_SIMULATION_ERROR', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
}
/**
 * Canary Management Route Handler
 */
export async function handleCanaryManagementRequest(request, env) {
    try {
        const url = new URL(request.url);
        ...
        return ApiResponseFactory.error('Canary management request failed', 'CANARY_MANAGEMENT_ERROR', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
}
//# sourceMappingURL=canary-management-routes.js.map