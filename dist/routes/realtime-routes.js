 ...
 */
export async function handleRealtimeRoutes(request, env, path, headers) {
    try {
        ...
        if (path === '/api/v1/realtime/refresh') {
            ...
            const body = ApiResponseFactory.success({ message: 'Data refresh triggered for all clients' }, { requestId: headers['X-Request-ID'] });
            return new Response(JSON.stringify(body), {
                status: HttpStatus.OK,
                headers
            });
