...
export class EnhancedRequestHandler {
    ...
    async handleResults(request) {
        ...
        if (!apiKey || !validKeys.includes(apiKey)) {
            const body = ApiResponseFactory.error('API key required for this endpoint', 'UNAUTHORIZED', { requires_api_key: true });
            return new Response(JSON.stringify(body), {
                status: 401,
                headers: {
                    'Content-Type': 'application/json',
        ...
        try {
            ...
            if (!results) {
                const body = ApiResponseFactory.error('No results found for the requested date', 'NOT_FOUND', { requested_date: requestedDate });
                return new Response(JSON.stringify(body), {
                    status: 404,
                    headers: {
                        'Content-Type': 'application/json',
            ...
            const body = ApiResponseFactory.error('Failed to retrieve results', 'INTERNAL_ERROR', { error_message: (error instanceof Error ? error.message : String(error)) });
            return new Response(JSON.stringify(body), {
                status: 500,
                headers: {
                    'Content-Type': 'application/json',
    ...
    async handleEnhancedHealthCheck() {
        const dalStats = this.dalgetPerformanceStats();
        const migrationConfig = this.migrationManagergetConfig();
        ...
        try {
            ...
            cacheData = {
                ...
                detailedMetrics: {
                    ...
                    totalRequests: cacheMetricsStats.overalltotalRequests,
                    l1HitRate: cacheMetricsStats.layersl1.hitRate,
                    l1Hits: cacheMetricsStats.layersl1.hits,
                    l1Misses: cacheMetricsStats.layersl1.misses,
                    l2HitRate: cacheMetricsStats.layersl2.hitRate,
                    l2Hits: cacheMetricsStats.layersl2.hits,
                    l2Misses: cacheMetricsStats.layersl2.misses,
                    issues: cacheMetricsStats.healthissues,
    ...
    async handleDALStatus() {
        const stats = this.dalgetPerformanceStats();
        return new Response(JSON.stringify({
            success: true,
            timestamp: new Date().toISOString(),
            dal: {
            ...
            cache: {
                hit_rate: `${Math.round(stats.cachehitRate * 100)}%`,
                total_operations: stats.cachehits + stats.cachemisses,
                cache_size: stats.performancecacheSize
            }
        }, null, 2), {
            headers: { 'Content-Type': 'application/json' }
        });
    ...
    async handleMigrationStatus() {
        const stats = await this.migrationManagergetMigrationStatistics();
        return new Response(JSON.stringify({
            success: true,
            timestamp: new Date().toISOString(),
            migration: stats,
            config: this.migrationManagergetConfig()
        }, null, 2), {
            headers: { 'Content-Type': 'application/json' }
        });
    }
    ...
    async handlePerformanceTest() {
        ...
        const readResult = await this.dalread(testKey);
        const readTime = Date.now() - readStart;
        // Test cache performance
        const cacheStart = Date.now();
        const cacheResult = await this.dalread(testKey);
        const cacheTime = Date.now() - cacheStart;
        // Cleanup
        await this.daldeleteKey(testKey);
        const dalStats = this.dalgetPerformanceStats();
        return new Response(JSON.stringify({
            success: true,
            timestamp: new Date().toISOString(),
    ...
    async handleCacheClear(request) {
        const url = new URL(request.url);
        const namespace = url.searchParamsget('namespace');
        if (namespace) {
            this.dalclearCache();
    ...
    getDalPerformanceStats() {
        return this.dalgetPerformanceStats();
    }
    /**
     * Expose migration statistics for external diagnostics.
     */
    async getMigrationStatistics() {
        return await this.migrationManagergetMigrationStatistics();
    }
}
/**
...
//# sourceMappingURL=enhanced-request-handler.js.map