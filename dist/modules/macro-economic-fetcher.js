...
import { initializeFredApiClient, MockFredApiClient } from './fred-api-client.js';
...
export class MacroEconomicFetcher {
    constructor(options) {
        ...
        this.useMockData = !this.strictMode && (options.forceMockClient ||
            options.useMockData ||
            !options.fredApiKey);
    ...
    identifyMissingData(macro) {
        const missing = [];
        if (macro.fedFundsRate === 0)
            missing.push('fedFundsRate');
        if (macro.treasury10Y === 0)
            missing.push('treasury10Y');
        if (macro.treasury2Y === 0)
            missing.push('treasury2Y');
        if (macro.cpi === 0)
            missing.push('cpi');
        if (macro.unemploymentRate === 0)
            missing.push('unemploymentRate');
        if (macro.nonFarmPayrolls === 0)
            missing.push('nonFarmPayrolls');
        if (macro.realGDP === 0)
            missing.push('realGDP');
        if (macro.gdpGrowthRate === 0)
            missing.push('gdpGrowthRate');
        return missing;
    }
    /**
     * Health check
 ...
 */
export function initializeMacroEconomicFetcher(options) {
    return new MacroEconomicFetcher(options);
}
export default MacroEconomicFetcher;
//# sourceMappingURL=macro-economic-fetcher.js.map