...
export class AdvancedRiskManagementEngine {
    ...
    async performRiskAssessment(portfolioData, marketData = {}) {
        try {
            ...
            assessment.overallRiskScore = this.calculateOverallRiskScore(assessment.categoryBreakdown);
            // Determine risk level
            assessment.riskLevel = this.determineRiskLevel(assessment.overallRiskScore);
            // Generate recommendations
            assessment.recommendations = this.generateRiskRecommendations(assessment.categoryBreakdown, assessment.riskLevel);
    ...
    async checkRegulatoryCompliance(portfolioData, regulations = []) {
        try {
            ...
            for (const framework of frameworksToCheck) {
                complianceCheck.frameworks[framework] = await this.checkComplianceFramework(portfolioData, framework);
            }
            // Aggregate results
    ...
    calculateMarketRiskScore(metrics, factors) {
        let score = 1;
        // VaR-based scoring
        if ((metrics.var95 ?? 0) > 50000)
            score += 0.5;
        if ((metrics.var99 ?? 0) > 100000)
            score += 0.5;
        // Volatility scoring
        if ((metrics.rollingVolatility ?? 0) > 0.20)
            score += 0.5;
        if (metrics.volatilityRegime === 'HIGH_VOLATILITY')
            score += 0.5;
        // Correlation scoring
        if ((metrics.averageCorrelation ?? 0) > 0.6)
            score += 0.5;
        return Math.min(score, 4);
    }
    calculateCreditRiskScore(metrics, exposures) {
        let score = 1;
        if ((metrics.defaultProbability ?? 0) > 0.05)
            score += 1;
        if ((metrics.creditVaR ?? 0) > 25000)
            score += 0.5;
        return Math.min(score, 4);
    }
    calculateConcentrationRiskScore(metrics, concentrations) {
        let score = 1;
        if ((metrics.maxSingleAssetWeight ?? 0) > 0.15)
            score += 0.5;
        if ((metrics.maxSectorWeight ?? 0) > 0.25)
            score += 0.5;
        if ((metrics.herfindahlIndex ?? 0) > 0.25)
            score += 0.5;
        return Math.min(score, 4);
    }
    calculateLiquidityRiskScore(metrics, factors) {
        let score = 1;
        if ((metrics.liquidityRatio ?? 1) < 0.10)
            score += 0.5;
        if ((metrics.marketImpact ?? 0) > 0.02)
            score += 0.5;
        return Math.min(score, 4);
    }
    calculateModelRiskScore(metrics, models) {
        let score = 1;
        if ((metrics.modelAccuracy ?? 1) < 0.80)
            score += 0.5;
        if ((metrics.modelStability ?? 1) < 0.85)
            score += 0.5;
        return Math.min(score, 4);
    }
    calculateOverallRiskScore(categoryBreakdown) {
        const categories = Object.values(categoryBreakdown).filter((cat) => cat !== undefined);
...
//# sourceMappingURL=advanced-risk-management.js.map