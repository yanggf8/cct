...
export class ModelValidator {
    ...
    async validateModel() {
        ...
        const [crossValidation, outOfSample, significance, overfitting, walkForward, monteCarlo, bootstrap] = await Promise.all([
    ...
    async performRollingWindow(config) {
        ...
        for (let fold = 0; fold < config.folds; fold++) {
            ...
            if (testEnd >= this.equityCurve.length)
                break;
            const trainPeriod = {
    ...
    async performOutOfSampleTesting() {
        ...
        const degradationMetrics = this.calculateDegradationMetrics(trainPerformance, testPerformance, validationPerformance);
        return {
            config,
            trainPerformance,
    ...
    async performBootstrapAnalysis() {
        ...
        const biasCorrectedPerformance = this.calculateBiasCorrectedPerformance(originalPerformance, bootstrapDistribution);
    ...
    async simulatePeriodPerformance(startIndex, endIndex) {
        ...
        const calculator = createPerformanceCalculator(periodEquityCurve, periodTrades, [], // Simplified - positions would need to be filtered too
    ...
    async performBenchmarkComparison(returns, benchmark) {
        ...
        const calculator = createPerformanceCalculator(this.equityCurve, this.trades, this.positions, this.config.execution.initialCapital, 0.02, benchmarkReturns);
    ...
    async evaluateWalkForwardWindow(window) {
        const performance = await this.simulatePeriodPerformance(this.equityCurve.findIndex(point => point.date === window.testStart), this.equityCurve.findIndex(point => point.date === window.testEnd));
        return {
            ...window,
            performance
        };
    ...
    calculateOriginalPerformance() {
        const calculator = createPerformanceCalculator(this.equityCurve, this.trades, this.positions, this.config.execution.initialCapital);
        return calculator.calculateAllMetrics();
    }
    calculateOverallScore(results) {
        ...
        const scores = results.map(result => {
            ...
            if (result.degradationMetrics?.acceptable !== undefined)
                return result.degradationMetrics.acceptable ? 0.8 : 0.3;
            if (result.isSignificant !== undefined)
                return result.isSignificant ? 0.8 : 0.4;
    ...
    generateRecommendation(overallScore, keyResults) {
        const hasMajorIssues = keyResults.some(result => {
            if (result.degradationMetrics?.acceptable === false)
                return true;
            if (result.riskScore > 0.7)
                return true;
            if (result.stabilityScore < 0.3)
                return true;
            return false;
        });
        if (overallScore > 0.7 && !hasMajorIssues)
            return 'accept';
        if (overallScore < 0.4 || hasMajorIssues)
            return 'reject';
        return 'conditional';
    }
 ...
 */
export function createModelValidator(config, env, equityCurve, trades, positions) {
