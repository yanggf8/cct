...
export class D1ColdStorage {
    ...
    async put(key, value, storageClass, options) {
      ...
      `;
            await this.db.prepare(upsertSql).bind(key, value, now, options?.ttl || null, checksum, storageClass, now, now).run();
    ...
    async upsertRollup(day, keyspace, storageClass, metrics) {
      ...
      `;
            await this.db.prepare(upsertSql).bind(day, keyspace, storageClass, metrics.hits, metrics.misses, p50Latency, p99Latency, metrics.errors, metrics.egressBytes, metrics.computeMs, metrics.totalOperations, new Date().toISOString()).run();
    ...
    async getStats() {
      ...
      `).all();
            const entriesByClass = classResult.reduce((acc, row) => {
 ...
 */
export function createD1ColdStorage(db) {
    return new D1ColdStorage(db);
}
/**
 * Default D1 cold storage instance
 */
export const defaultD1ColdStorage = createD1ColdStorage(null);
//# sourceMappingURL=d1-storage.js.map