 ...
 */
export function getCacheConfigForEnvironment(env) {
    const baseConfig = {
        enabled: true,
 ...
 */
export function getCacheNamespaceNames() {
    return Object.keys(CACHE_NAMESPACES);
}
/**
 * Validate cache configuration
 */
export function validateCacheNamespace(namespace) {
    const errors = [];
...
export function createCacheNamespace(config) {
    const { name, prefix, l1TTL, l2TTL, l1MaxSize = 100, version = '1.0' } = config;
    const namespace = {
        name,
        prefix,
        l1Config: {
