...
export async function exampleFacebookMessageWithTracking(env) {
    ...
    const trackingResult = await tracker.createTracking('facebook', // platform
    ...
    try {
        ...
        if (response.ok) {
            ...
            await tracker.updateStatus(trackingId, 'sent', data.message_id // Platform message ID
            );
        ...
        await tracker.updateStatus(trackingId, 'failed', undefined, (error instanceof Error ? error.message : String(error)));
        throw error;
    }
}
/**
...
export async function exampleTelegramMessageWithTracking(env) {
    const tracker = createMessageTracker(env);
    const trackingResult = await tracker.createTracking('telegram', 'alert', env.TELEGRAM_CHAT_ID, {
 ...
 */ 
//# sourceMappingURL=msg-tracking-example.js.map