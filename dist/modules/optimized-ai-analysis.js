    ...
    async analyzeBatch(symbols) {
        ...
        for (let i = 0; i < symbols.length; i++) {
            ...
            try {
                ...
                if (result.metadata.rate_limit_hit)
                    summary.rate_limited++;
                if (result.analysis_type === 'technical_fallback')
 ...
 */
export async function performOptimizedAnalysis(symbols, env) {
 ...
 */
export async function analyzeSingleSymbolOptimized(symbol, env, forceRefresh = false) {
    const analyzer = new OptimizedAIAnalyzer(env);
    return await analyzer.analyzeSymbol(symbol, forceRefresh);
}
//# sourceMappingURL=optimized-ai-analysis.js.map