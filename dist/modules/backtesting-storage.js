...
export class BacktestingStorageManager {
    ...
    async getBacktestHistory(filters = {}, pagination = {}) {
        ...
        if (filters.strategy) {
            history = history.filter((item) => item.config?.strategy?.name === filters.strategy);
        }
        if (filters.symbol) {
            history = history.filter((item) => item.config?.symbols?.includes(filters.symbol));
        }
        if (filters.dateFrom) {
...
//# sourceMappingURL=backtesting-storage.js.map