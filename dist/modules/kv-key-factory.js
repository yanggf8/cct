...
export class KVKeyFactory {
    ...
    static inferKeyType(key) {
        if (key.startsWith('analysis_'))
            return KeyTypes.ANALYSIS;
        if (key.startsWith('dual_ai_analysis_'))
            return KeyTypes.DUAL_AI_ANALYSIS;
        if (key.startsWith('legacy_analysis_'))
            return KeyTypes.LEGACY_ANALYSIS;
        if (key.includes('_status_'))
            return KeyTypes.JOB_STATUS;
        if (key.startsWith('daily_summary_'))
            return KeyTypes.DAILY_SUMMARY;
        if (key.startsWith('facebook_'))
            return KeyTypes.FACEBOOK_STATUS;
        if (key.startsWith('market_cache_'))
            return KeyTypes.MARKET_DATA_CACHE;
        if (key.startsWith('report_cache_'))
            return KeyTypes.REPORT_CACHE;
        if (key.startsWith('test_'))
            return KeyTypes.TEST_DATA;
        if (key.startsWith('debug_'))
            return KeyTypes.DEBUG_DATA;
        // Sector rotation keys
        if (key.startsWith('sector_data_'))
            return KeyTypes.SECTOR_DATA;
        if (key.startsWith('sector_snapshot_'))
            return KeyTypes.SECTOR_SNAPSHOT;
        if (key.startsWith('sector_indicators_'))
            return KeyTypes.SECTOR_INDICATORS;
        if (key.startsWith('sector_performance_'))
            return KeyTypes.SECTOR_PERFORMANCE;
        if (key.startsWith('sector_relative_strength_'))
            return KeyTypes.SECTOR_RELATIVE_STRENGTH;
        // Market Drivers keys
        if (key.startsWith('market_drivers_snapshot_'))
            return KeyTypes.MARKET_DRIVERS_SNAPSHOT;
        if (key.startsWith('market_drivers_macro_'))
            return KeyTypes.MARKET_DRIVERS_MACRO;
        if (key.startsWith('market_drivers_market_structure_'))
            return KeyTypes.MARKET_DRIVERS_MARKET_STRUCTURE;
        if (key.startsWith('market_drivers_geopolitical_'))
            return KeyTypes.MARKET_DRIVERS_GEOPOLITICAL;
        if (key.startsWith('market_drivers_regime_'))
            return KeyTypes.MARKET_DRIVERS_REGIME;
        if (key.startsWith('market_drivers_history_'))
            return KeyTypes.MARKET_DRIVERS_HISTORY;
        if (key.startsWith('market_drivers_fred_data_'))
            return KeyTypes.MARKET_DRIVERS_FRED_DATA;
        if (key.startsWith('market_drivers_risk_assessment_'))
            return KeyTypes.MARKET_DRIVERS_RISK_ASSESSMENT;
...
export const KeyHelpers = {
     ...
     */
    getSectorDataKey: (symbol, timestamp) => KVKeyFactory.generateKey(KeyTypes.SECTOR_DATA, {
     ...
     */
    getSectorSnapshotKey: (date) => KVKeyFactory.generateDateKey(KeyTypes.SECTOR_SNAPSHOT, date),
    /**
     * Get sector indicators key for symbol and date
     */
    getSectorIndicatorsKey: (symbol, date) => KVKeyFactory.generateDateKey(KeyTypes.SECTOR_INDICATORS, date, {
        symbol: KVKeyFactory.sanitizeValue(symbol)
    }),
    /**
     * Get sector performance key for date
     */
    getSectorPerformanceKey: (date) => KVKeyFactory.generateDateKey(KeyTypes.SECTOR_PERFORMANCE, date),
    /**
     * Get sector relative strength key for symbol and date
     */
    getSectorRelativeStrengthKey: (symbol, date) => KVKeyFactory.generateDateKey(KeyTypes.SECTOR_RELATIVE_STRENGTH, date, {
        symbol: KVKeyFactory.sanitizeValue(symbol)
    }),
    /**
     ...
     */
    getMarketDriversSnapshotKey: (date) => KVKeyFactory.generateDateKey(KeyTypes.MARKET_DRIVERS_SNAPSHOT, date),
    /**
     * Get market drivers macro data key for date
     */
    getMarketDriversMacroKey: (date) => KVKeyFactory.generateDateKey(KeyTypes.MARKET_DRIVERS_MACRO, date),
    /**
     * Get market drivers market structure key for date
     */
    getMarketDriversMarketStructureKey: (date) => KVKeyFactory.generateDateKey(KeyTypes.MARKET_DRIVERS_MARKET_STRUCTURE, date),
    /**
     * Get market drivers geopolitical risk key for date
     */
    getMarketDriversGeopoliticalKey: (date) => KVKeyFactory.generateDateKey(KeyTypes.MARKET_DRIVERS_GEOPOLITICAL, date),
    /**
     * Get market drivers regime analysis key for date
     */
    getMarketDriversRegimeKey: (date) => KVKeyFactory.generateDateKey(KeyTypes.MARKET_DRIVERS_REGIME, date),
    /**
     * Get market drivers history key for date and regime type
     */
    getMarketDriversHistoryKey: (date, regimeType) => KVKeyFactory.generateKey(KeyTypes.MARKET_DRIVERS_HISTORY, {
     ...
     */
    getMarketDriversFredDataKey: (series, date) => KVKeyFactory.generateKey(KeyTypes.MARKET_DRIVERS_FRED_DATA, {
     ...
     */
    getMarketDriversRiskAssessmentKey: (date) => KVKeyFactory.generateDateKey(KeyTypes.MARKET_DRIVERS_RISK_ASSESSMENT, date)
};
export default KVKeyFactory;
//# sourceMappingURL=kv-key-factory.js.map