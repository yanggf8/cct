...
import { MemoryStaticDAL } from './memory-static-data.js';
import { PredictivePrefetchDAL } from './predictive-prefetching.js';
...
export class EnhancedOptimizedCacheManager {
    ...
    async read(key, options) {
        ...
        try {
            ...
            let memoryResult = { success: false, data: null };
            if (this.config.enableMemoryStaticData && (options?.useMemoryStatic !== false)) {
                memoryResult = await this.memoryStaticDAL.read(key);
            ...
            let preFetchResult = { success: false, data: null };
            if (this.config.enablePredictivePrefetching && (options?.usePredictive !== false)) {
                preFetchResult = await this.predictivePrefetch.read(key);
            ...
            if (!aliasedResult.success && !memoryResult.success && !preFetchResult.success && this.config.enableBatchOperations) {
                ...
                if (batchData && typeof batchData.entries === 'function') {
                    for (const [batchResultKey, data] of batchData.entries()) {
                        if (batchResultKey === key && data) {
                            aliasedResult = { success: true, data: data };
            ...
            if (!finalResult.success) {
                finalResult = await this.baseDAL.read(key);
            }
            if (finalResult.success) {
     ...
     */
    async processSectors(sectors, operation) {
        const startTime = Date.now();
        const optimizations = ['Vectorized Sector Processing'];
        try {
            let result;
     ...
     */
    async triggerPredictivePreFetch(keys) {
     ...
     */
    updateStats(kvReduction, optimizations) {
     ...
     */
    logOperation(operation, kvOpsSaved, optimizations, duration) {
...
//# sourceMappingURL=enhanced-optimized-cache-manager.js.map