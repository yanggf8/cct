 ...
 */
export function getDefaultHealthData() {
    return {
        status: 'healthy',
 ...
 */
export function getDefaultModelHealthData() {
    return {
        status: 'healthy',
        models: {
 ...
 */
export function getDefaultAnalysisData() {
    return {
        status: 'no_data',
 ...
 */
export function getDefaultMarketData() {
    return {
        indices: [
 ...
 */
export function getDefaultSectorData() {
    return {
        sectors: [
 ...
 */
export async function fetchHealthData(env) {
    try {
        const healthResponse = await createHealthResponse(env);
        return {
            status: 'healthy',
            components: healthResponse.data,
 ...
 */
export async function fetchModelHealthData(env) {
    try {
 ...
 */
export async function fetchLatestAnalysis(env) {
    try {
        const dal = createSimplifiedEnhancedDAL(env);
        // Get today's date in YYYY-MM-DD format
        const today = new Date().toISOString().split('T')[0];
...
export async function fetchMarketData(env) {
    try {
        ...
        try {
            ...
            if (cached.success && cached.data && Date.now() - cached.data.timestamp < 300000) { // 5 minutes
                marketData.indices = cached.data.data || marketData.indices;
            }
        }
        catch (e) {
 ...
 */
export async function fetchSectorData(env) {
    try {
 ...
 */
export function generateDashboardHTML(data, env) {
 ...
 */
export function generateErrorDashboard(errorMessage, requestId) {
    return `
<!DOCTYPE html>
<html lang="en" data-theme="dark">
<head>
...
export async function handleProfessionalDashboard(request, env, ctx) {
    ...
    try {
        ...
        const [healthData, modelHealthData, latestAnalysis, marketData, sectorData] = await Promise.allSettled([
            fetchHealthData(env),
...
//# sourceMappingURL=dashboard-handlers.js.map