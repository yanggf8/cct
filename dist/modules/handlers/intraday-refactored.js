...
import { generateWaitingDisplay, generateErrorDisplay, generateMetricsGrid, generateSignalItem, generateCompletePage } from '../html-generators.js';
...
class IntradayDataRetriever {
     ...
     */
    static async retrieveData(env, date, context = { requestId: 'default' }) {
     ...
     */
    static validateData(data, context) {
        const { requestId } = context;
        if (!data) {
...
class IntradayPerformanceAnalyzer {
     ...
     */
    static async analyzePerformance(data, env, context) {
        const { requestId } = context;
     ...
     */
    static generateMetrics(signals) {
...
class IntradayHTMLGenerator {
     ...
     */
    static generateCompletePage(data, performance, context) {
        const { requestId } = context;
        try {
            ...
            const htmlContent = generateCompletePage('🔍 Intraday Performance Check', 'Real-time signal performance analysis', sectionsContent, 'Operational');
            return htmlContent;
        }
        catch (error) {
     ...
     */
    static generateMetricsSection(metrics) {
     ...
     */
    static generateSignalsSection(signals) {
        if (!signals || signals.length === 0) {
            return `
      ...
      `;
        ...
        const signalsHTML = signals.map(signal => generateSignalItem({
            symbol: signal.symbol,
            direction: signal.direction,
...
class DependencyValidator {
     ...
     */
    static async validateDependencies(date, env, context) {
        const { requestId } = context;
     ...
     */
    static async verifyConsistency(env, context) {
        const { requestId } = context;
 ...
 */
export const handleIntradayCheckRefactored = async (request, env) => {
    ...
    try {
        ...
        if (!dependencyValidation.isValid) {
            ...
            const waitingHTML = generateWaitingDisplay('Intraday Analysis in Progress', dependencyValidation);
        ...
        if (!intradayData) {
            ...
            const errorHTML = generateErrorDisplay('No intraday data available for this period', requestId);
        ...
        if (!isValidData) {
            ...
            const errorHTML = generateErrorDisplay('Invalid intraday data format', requestId);
            return new Response(errorHTML, {
                status: 500,
                headers: { 'Content-Type': 'text/html; charset=utf-8' }
            });
        ...
        // Analyze performance
        const performanceData = await IntradayPerformanceAnalyzer.analyzePerformance(intradayData, env, context);
        // Generate HTML response
        const htmlContent = IntradayHTMLGenerator.generateCompletePage(intradayData, performanceData, context);
        ...
        const errorHTML = generateErrorDisplay(`Intraday check failed: ${error.message}`, requestId);
        return new Response(errorHTML, {
            status: 500,
            headers: { 'Content-Type': 'text/html; charset=utf-8' }
        });
...
//# sourceMappingURL=intraday-refactored.js.map