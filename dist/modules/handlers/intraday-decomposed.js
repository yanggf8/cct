...
import { validateRequestEnvironment } from './common-handlers.js';
...
class IntradayDataModule {
     ...
     */
    static async retrieve(env, date, context = { requestId: 'default' }) {
...
class PerformanceModule {
     ...
     */
    static async analyze(data, env, context) {
        const { requestId } = context;
...
class HTMLModule {
     ...
     */
    static generateComplete(data, performance, context) {
        const { requestId } = context;
        try {
            ...
            const htmlContent = generateCompletePage('🔍 Intraday Performance Check', 'Modular architecture demonstration', content);
            return htmlContent;
        }
        catch (error) {
     ...
     */
    static generateArchitectureSection() {
        return `
...
class ErrorModule {
     ...
     */
    static generateWaitingResponse(message, completionRate, context) {
        const { requestId } = context;
        logger.debug('⏳ Generating waiting response', {
            requestId,
            completionRate
        });
        const waitingHTML = generateWaitingDisplay(`${message} ${completionRate}\n\nAnalysis is in progress. This typically takes 2-3 minutes to complete.`);
 ...
 */
export const handleIntradayCheckDecomposed = async (request, env, ctx) => {
    ...
    try {
        ...
        if (!envValidation.isValid) {
            ...
            return ErrorModule.generateErrorResponse(new Error('Environment validation failed'), context);
        }
        ...
        if (!intradayData) {
            ...
            return ErrorModule.generateWaitingResponse('Intraday Data Preparation', 0.5, // 50% complete when data retrieval starts
            context);
        }
        // Validate data
        const isValidData = IntradayDataModule.validate(intradayData, context);
        if (!isValidData) {
            return ErrorModule.generateErrorResponse(new Error('Invalid intraday data structure'), context);
        }
        // Analyze performance
        const performanceData = await PerformanceModule.analyze(intradayData, env, context);
        // Generate HTML response
        const htmlContent = HTMLModule.generateComplete(intradayData, performanceData, context);
...
//# sourceMappingURL=intraday-decomposed.js.map