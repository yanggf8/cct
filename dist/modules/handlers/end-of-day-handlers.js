 ...
 */
export const handleEndOfDaySummary = createHandler('end-of-day-summary', async (request, env, ctx) => {
 ...
 * Generate comprehensive end-of-day HTML
 */
function generateEndOfDayHTML(endOfDayData, requestId, currentDate) {
    ...
    <script>
        ...
        endOfDayData.performanceDistribution?.pending || 0
            ...
            new Chart(confidenceCtx, {
                type: 'bar',
                data: {
                    labels: ${JSON.stringify(endOfDayData.confidenceData?.map((c) => c.symbol) || [])},
                    datasets: [{
                        label: 'Confidence (%)',
                        data: ${JSON.stringify(endOfDayData.confidenceData?.map((c) => (c.confidence || 0) * 100) || [])},
            ...
            new Chart(symbolCtx, {
                type: 'bar',
                data: {
                    labels: ${JSON.stringify(endOfDayData.symbolPerformance?.map((s) => s.symbol) || [])},
                    datasets: [{
                        label: 'Performance (%)',
                        data: ${JSON.stringify(endOfDayData.symbolPerformance?.map((s) => (s.performance || 0) * 100) || [])},
                        backgroundColor: ${JSON.stringify(endOfDayData.symbolPerformance?.map((s) => s.performance >= 0 ? 'rgba(79, 172, 254, 0.8)' : 'rgba(255, 107, 107, 0.8)') || [])},
                        borderColor: '#4facfe',
                        borderWidth: 1
                    }]
                },
 ...
 */
function generateSignalCards(signals) {
...
//# sourceMappingURL=end-of-day-handlers.js.map