...
import { cacheMetrics } from './DO_CACHE_METRICS.js';
import { createEnhancedHashCache } from './enhanced-hash-cache.js';
import { getCacheConfig, getCacheNamespaces, BUSINESS_HOURS_START, BUSINESS_HOURS_END } from './enhanced-cache-config.js';
import { getPromotionManager } from './enhanced-cache-promotion.js';
import { getMetricsManager } from './enhanced-cache-metrics.js';
...
export class CacheManager {
    ...
    async get(namespace, key, fetchFn) {
        ...
        try {
            ...
            if (cacheNs.l1Config.enabled) {
                const l1Result = await this.getFromL1WithNamespace(fullKey, namespace, cacheNs);
                if (l1Result !== null) {
                    this.stats.l1Hits++;
                    cacheMetrics.recordHit('L1', namespace);
                    logger.debug(`L1 cache hit: ${fullKey}`);
                    return l1Result;
                }
                // Record L1 miss only if we attempted L1 lookup
                cacheMetrics.recordMiss('L1', namespace);
            }
            // Try L2 cache
            if (cacheNs.l2Config.enabled) {
                ...
                if (l2Result !== null) {
                    this.stats.l2Hits++;
                    cacheMetrics.recordHit('L2', namespace);
                ...
                cacheMetrics.recordMiss('L2', namespace);
            }
            ...
            if (fetchFn) {
                ...
                const data = await requestDeduplicator.execute(deduplicationKey, async () => {
     ...
     */
    async set(namespace, key, data, customTTL) {
        if (!this.enabled)
            return;
        const fullKey = this.buildCacheKey(namespace, key);
        const cacheNs = this.namespaces.get(namespace);
    ...
    async delete(namespace, key) {
        ...
        try {
            ...
            // Delete from L2
            const kvKey = this.keyFactory.generateKey(KeyTypes.TEMPORARY, { purpose: fullKey, timestamp: 0 });
     ...
     */
    async getFromL1WithNamespace(key, namespace, cacheNs) {
    ...
    async getFromL2(key, namespace) {
        const kvKey = this.keyFactory.generateKey(KeyTypes.TEMPORARY, { purpose: key, timestamp: 0 });
        ...
        try {
            ...
            if (!simplifiedEntry.data || !simplifiedEntry.cachedAt) {
                ...
                const cacheEntry = simplifiedEntry;
     ...
     */
    async setToL2(key, data, namespace, ttl) {
        ...
        const kvKey = this.keyFactory.generateKey(KeyTypes.TEMPORARY, { purpose: key, timestamp: Date.now() });
     ...
     */
    async intelligentPromotion(fullKey, namespace, data) {
        try {
            ...
            if (decision.shouldPromote) {
                const success = await this.promotionManager.promoteToL1(this.l1Cache, context, decision);
                if (success) {
    ...
    async performHealthAssessment() {
        try {
            ...
            return await this.metricsManager.assessHealth(this.l1Cache, this.stats, promotionStats, cacheConfigs);
        }
        catch (error) {
     ...
     */
    updateMetricsThresholds(newThresholds) {
     ...
     */
    async getWithTimestampInfo(namespace, key) {
        this.stats.totalRequests++;
        ...
        try {
            ...
            // Try L2 cache
            if (cacheNs.l2Config.enabled) {
                const l2Result = await this.getFromL2WithTimestamp(fullKey, namespace);
                if (l2Result !== null) {
                    this.stats.l2Hits++;
                    cacheMetrics.recordHit('L2', namespace);
     ...
     */
    async getFromL2WithTimestamp(key, namespace) {
        const kvKey = this.keyFactory.generateKey(KeyTypes.TEMPORARY, { purpose: key, timestamp: 0 });
        const result = await this.dal.read(kvKey);
        if (!result.success || !result.data)
            return null;
        try {
            ...
            return { data: cacheEntry.data, timestampInfo: { cachedAt: cacheEntry.timestamp, ttlSeconds: cacheEntry.ttl } };
        }
        catch (error) {
            logger.error(`L2 cache parse error for ${key}:`, {
                error: error instanceof Error ? error.message : 'Unknown error',
     ...
     */
    async scheduleBackgroundRefresh(key, namespace) {
        try {
 ...
 */
export function createCacheManager(env, options) {
