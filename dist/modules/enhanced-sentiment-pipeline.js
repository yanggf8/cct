...
// Configuration
const ENHANCED_SENTIMENT_CONFIG = {
    sources: {
...
export class EnhancedSentimentPipeline {
     ...
     */
    async gatherArticlesFromSources(symbol) {
        const allArticles = [];
        ...
        if (ENHANCED_SENTIMENT_CONFIG.sources.dac_pool.enabled) {
            try {
                ...
                if (dacResult.articles.length > 0) {
                    const enhancedArticles = dacResult.articles.map(article => ({
                        ...article,
     ...
     */
    async performSentimentAnalysis(articles, symbol) {
        if (articles.length === 0) {
...
Respond with JSON only:
{
  ...
  "reasoning": "Brief explanation of the sentiment analysis"
     ...
     * Calculate quality metrics
     */
    calculateQualityMetrics(articles) {
        const now = Date.now();
     ...
     */
    buildReasoning(analysis, articles, metrics) {
...
export function createEnhancedSentimentPipeline(env) {
    return new EnhancedSentimentPipeline(env);
}
//# sourceMappingURL=enhanced-sentiment-pipeline.js.map