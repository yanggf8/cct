 ...
 */
async function retryAIcall(operation, maxRetries = 3, baseDelay = 1000) {
        ...
        const response = await retryAIcall(async () => {
            return await circuitBreaker.execute(async () => {
                return await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('AI model timeout')), 30000) // 30s timeout
                    )
                ]);
            });
        });
...
async function performDistilBERTAnalysis(symbol, newsData, env) {
    ...
    try {
        const results = await Promise.all(newsData.slice(0, 10).map(async (article, index) => {
            try {
                ...
                const response = await retryAIcall(async () => {
                    return await circuitBreaker.execute(async () => {
                        return await Promise.race([
                            env.AI.run('@cf/huggingface/distilbert-sst-2-int8', { text: text }),
                            new Promise((_, reject) => setTimeout(() => reject(new Error('DistilBERT model timeout')), 20000) // 20s timeout
                            )
                        ]);
                    });
                });
 ...
 */
export async function enhancedBatchDualAIAnalysis(symbols, env, options = {}) {
    const startTime = Date.now();
    ensureLoggingInitialized(env);
...
//# sourceMappingURL=dual-ai-analysis.js.map