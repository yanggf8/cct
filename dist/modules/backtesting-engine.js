...
export class BacktestingEngine {
    ...
    calculateDataConsistency() {
        if (this.marketData.size < 2)
            return 1.0;
        const dates = Array.from(this.marketData.values()).map(data => data.map(row => row.date).sort());
    ...
    async checkExitSignals(date) {
        for (const [symbol, position] of this.positions.entries()) {
            if (position.exitDate)
                continue; // Already marked for exit
            const currentPrice = this.getPrice(symbol, date);
            if (!currentPrice)
                continue;
            // Check stop loss
    ...
    calculateVolatility(symbol, date) {
        ...
        const returns = recentData.slice(1).map((row, i) => Math.log(row.close / recentData[i].close));
 ...
 */
export async function runBacktest(config, env) {
    const engine = new BacktestingEngine(config, env);
    return await engine.runBacktest();
}
//# sourceMappingURL=backtesting-engine.js.map