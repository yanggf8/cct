...
import { trackCronHealth } from './data.js';
import { initLogging, logSentimentDebug, logAIDebug, logError, logInfo } from './logging.js';
    ...
    try {
        ...
        const sentimentPromises = newsData.slice(0, 8).map(async (newsItem, index) => {
            try {
                ...
                const response = await env.AI.run('@cf/huggingface/distilbert-sst-2-int8', { text: text });
 ...
 */
function analyzeTextSentiment(newsData, symbol) {
    ...
    newsData.forEach(article => {
        ...
        bullishKeywords.forEach(kw => { if (text.includes(kw))
            bullishCount++; });
        bearishKeywords.forEach(kw => { if (text.includes(kw))
            bearishCount++; });
    });
...
//# sourceMappingURL=enhanced_analysis.js.map