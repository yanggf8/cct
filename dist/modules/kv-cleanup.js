...
export async function cleanupKVCache(env, req = {}) {
        ...
        try {
            do {
                ...
                for (const entry of list.keys) {
                    ...
                    examinedThisPrefix++;
                    result.examined++;
                    if (result.samples.examined.length < 20)
                        result.samples.examined.push(entry.name);
                    ...
                    if (ageDays > retentionDays) {
                        toDeleteThisPrefix++;
                        result.toDelete++;
                        if (result.samples.toDelete.length < 20)
                            result.samples.toDelete.push(entry.name);
                        if (!dryRun) {
                            try {
                                await env.MARKET_ANALYSIS_CACHE.delete(entry.name);
                                deletedThisPrefix++;
                                result.deleted++;
                                if (result.samples.deleted.length < 20)
                                    result.samples.deleted.push(entry.name);
                            }
                            catch (err) {
...
//# sourceMappingURL=kv-cleanup.js.map