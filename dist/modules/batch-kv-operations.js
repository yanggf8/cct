...
export class BatchKVOperations {
     ...
     */
    async scheduleRead(key, namespace, priority = 'medium') {
     ...
     */
    async scheduleBatchRead(keys, namespace, priority = 'medium') {
        const batchKey = this.getBatchKey('read', namespace, priority);
     ...
     */
    async scheduleWrite(key, data, namespace, priority = 'medium') {
        return new Promise((resolve, reject) => {
            const operation = {
                type: 'write',
                key,
     ...
     */
    async scheduleBatchWrite(entries, namespace, priority = 'medium') {
        const batchKey = this.getBatchKey('write', namespace, priority);
     ...
     */
    async executeBatchReads(operations) {
        const results = [];
        ...
        for (const [namespace, ops] of namespaceGroups.entries()) {
            ...
            const namespaceResults = await Promise.allSettled(ops.map(async (op) => {
                try {
     ...
     */
    async executeBatchWrites(operations) {
        const results = [];
        // Group by namespace
        const namespaceGroups = this.groupByNamespace(operations);
        for (const [namespace, ops] of namespaceGroups.entries()) {
     ...
     */
    scheduleBatchExecution(batchKey, priority) {
     ...
     * Generate batch key
     */
    getBatchKey(type, namespace, priority) {
     ...
     */
    waitForOperationCompletion(key, resolve, reject) {
     ...
     */
    waitForBatchCompletion(batchKey, resolve, reject) {
        const checkInterval = setInterval(() => {
            if (!this.pendingOperations.has(batchKey)) {
                clearInterval(checkInterval);
                resolve(true);
...
//# sourceMappingURL=batch-kv-operations.js.map