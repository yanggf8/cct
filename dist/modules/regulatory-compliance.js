    ...
    async performComplianceAssessment(portfolioData, clientData = {}, frameworks = []) {
        try {
            ...
            for (const framework of assessment.frameworks) {
                const frameworkResult = await this.assessFramework(portfolioData, clientData, framework);
    ...
    async assessFramework(portfolioData, clientData, framework) {
        try {
            ...
            for (const [requirement, config] of Object.entries(frameworkConfig.requirements)) {
                const requirementResult = await this.assessRequirement(portfolioData, clientData, framework, requirement, config);
                ...
                if (requirementResult.status.value > COMPLIANCE_STATUS.COMPLIANT.value) {
                    frameworkResult.status = this.getWorstStatus(frameworkResult.status, requirementResult.status);
...
//# sourceMappingURL=regulatory-compliance.js.map