...
export class PortfolioRebalancingEngine {
    ...
    async monitorPortfolioDrift(portfolioId, targetWeights, strategy) {
        try {
            ...
            monitoring.driftMetrics = this.calculateDriftMetrics(monitoring.currentWeights, targetWeights);
    ...
    async performTaxLossHarvesting(portfolioId, taxConfig = {}) {
        try {
            ...
            harvesting.opportunities = this.identifyHarvestingOpportunities(harvesting.positions, taxConfig);
    ...
    async createDynamicAllocation(portfolioId, marketConditions, riskTolerance) {
        try {
            ...
            allocation.dynamicWeights = this.calculateDynamicWeights(allocation.allocationSignals, riskTolerance);
            // Apply risk adjustments
            allocation.riskAdjustments = this.applyRiskAdjustments(allocation.dynamicWeights, marketAnalysis);
            // Create execution plan
    ...
    isRebalancingRequired(deviations, strategy) {
        switch (strategy.type) {
            case REBALANCING_STRATEGIES.THRESHOLD_BASED:
                return Object.values(deviations).some(dev => dev.deviationPercent > (strategy.thresholds.deviation || 0.05));
    ...
    async testStrategyUnderStress(portfolioId, strategy, scenarios) {
        ...
        return {
            worstCase: results[Object.keys(results).reduce((worst, key) => results[key].performance < results[worst].performance ? key : worst)],
            bestCase: results[Object.keys(results).reduce((best, key) => results[key].performance > results[best].performance ? key : best)],
    ...
    generateStressTestRecommendations(results) {
        ...
        const bestStrategy = Object.keys(results).reduce((best, key) => results[key].averagePerformance > results[best].averagePerformance ? key : best);
        recommendations.push({
...
//# sourceMappingURL=portfolio-rebalancing.js.map