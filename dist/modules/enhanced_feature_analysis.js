...
export async function runEnhancedFeatureAnalysis(symbols, env) {
    ...
    for (const symbol of symbols) {
        try {
            ...
            const enhancedSignal = await createEnhancedPrediction(neuralAnalysis, // Note: changed from neuralSignal to neuralAnalysis
            technicalFeatures, sentimentData, symbol);
            ...
            try {
                ...
                // @ts-ignore - Type assertion for trading_signals access
                const fallbackSignal = fallbackAnalysis.trading_signals?.[symbol];
...
async function createEnhancedPrediction(neuralSignal, technicalFeatures, sentimentData, symbol) {
    ...
    const combinedPrediction = combineEnhancedPredictions(enhancedSignal.components, neuralSignal?.current_price || 0);
...
function analyzeTechnicalFeatures(features, currentPrice) {
    ...
    if (technicalScore > 0.1)
        direction = 'UP';
    else if (technicalScore < -0.1)
        direction = 'DOWN';
...
function combineEnhancedPredictions(components, currentPrice) {
    ...
    let totalWeight = 0;
    let totalConfidence = 0;
    const directionalVotes = { UP: 0, DOWN: 0, NEUTRAL: 0 };
    // Neural networks component
    if (components.neural_networks) {
        const neuralComponent = components.neural_networks;
        ...
        if (neuralComponent.direction && ['UP', 'DOWN', 'NEUTRAL'].includes(neuralComponent.direction)) {
            directionalVotes[neuralComponent.direction] += neuralComponent.weight;
        }
    }
    // Technical features component
    if (components.technical_features) {
        const techComponent = components.technical_features;
        ...
        if (techComponent.direction && ['UP', 'DOWN', 'NEUTRAL'].includes(techComponent.direction)) {
            directionalVotes[techComponent.direction] += techComponent.weight;
        }
    }
    // Sentiment component
    ...
    const finalDirection = Object.keys(directionalVotes).reduce((a, b) => directionalVotes[a] > directionalVotes[b] ? a : b);
    return {
...
//# sourceMappingURL=enhanced_feature_analysis.js.map