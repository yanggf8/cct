...
export class MarketRegimeClassifier {
    ...
    async classifyMarketRegime(macro, marketStructure, geopolitical) {
        try {
            ...
            const analysis = await this.generateRegimeAnalysis(primaryRegime, confidence, factorScores, regimeProbabilities, macro, marketStructure, geopolitical);
            // Store in history
    ...
    determineRiskLevel(regime, confidence) {
        ...
        if (highRiskRegimes.includes(regime) && confidence > 70)
            return 'extreme';
        if (highRiskRegimes.includes(regime))
            return 'high';
        if (lowRiskRegimes.includes(regime) && confidence > 70)
            return 'low';
        return 'medium';
    }
    ...
    identifyTriggerFactors(regime, factorScores) {
        ...
        if (factorScores.vix.score > 80)
            triggers.push('VIX spike above 30');
        if (factorScores.vix.score < 20)
            triggers.push('VIX compression below 15');
        if (factorScores.yieldCurve.score > 75)
            triggers.push('Significant yield curve inversion');
        if (factorScores.yieldCurve.score < 30)
            triggers.push('Yield curve steepening');
        if (factorScores.geopoliticalRisk.score > 80)
            triggers.push('Major geopolitical escalation');
        if (factorScores.geopoliticalRisk.score < 20)
            triggers.push('Geopolitical risk resolution');
 ...
 */
export function initializeMarketRegimeClassifier(config) {
    return new MarketRegimeClassifier(config);
}
export default MarketRegimeClassifier;
//# sourceMappingURL=market-regime-classifier.js.map