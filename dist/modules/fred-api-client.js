...
import { FRED_SERIES } from './market-drivers.js';
...
export class FredApiClient {
    constructor(options) {
        ...
        this.dal = createSimplifiedEnhancedDAL({ MARKET_ANALYSIS_CACHE: null }); // DAL for caching
    ...
    processSeriesDataToSnapshot(seriesMap) {
        ...
        const extractData = (series, defaultValue = 0) => {
        ...
        const latestUpdate = Math.max(fedFundsRate.lastUpdated ? new Date(fedFundsRate.lastUpdated).getTime() : 0, cpi.lastUpdated ? new Date(cpi.lastUpdated).getTime() : 0, unemploymentRate.lastUpdated ? new Date(unemploymentRate.lastUpdated).getTime() : 0, realGDP.lastUpdated ? new Date(realGDP.lastUpdated).getTime() : 0);
 ...
 */
export function initializeFredApiClient(options) {
    if (!options.apiKey) {
...
//# sourceMappingURL=fred-api-client.js.map