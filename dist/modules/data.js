...
export async function storeFactTableData(env, factTableData) {
    try {
        ...
        await KVUtils.putWithTTL(env.MARKET_ANALYSIS_CACHE, factTableKey, JSON.stringify(factTableData), 'analysis');
...
export async function storeSymbolAnalysis(env, symbol, analysisData) {
    try {
        ...
        await KVUtils.putWithTTL(env.MARKET_ANALYSIS_CACHE, key, dataString, 'granular');
...
export async function batchStoreAnalysisResults(env, analysisResults) {
    try {
        ...
        kvOperations.push(KVUtils.putWithTTL(env.MARKET_ANALYSIS_CACHE, `analysis_${date}`, JSON.stringify(dailyAnalysis), 'analysis'));
        // Add individual symbol analyses to batch
        for (const result of analysisResults) {
            if (result && result.symbol) {
                const compactResult = createCompactAnalysisData(result);
                kvOperations.push(KVUtils.putWithTTL(env.MARKET_ANALYSIS_CACHE, `analysis_${date}_${result.symbol}`, JSON.stringify(compactResult), 'granular'));
            }
        }
...
export async function getSymbolAnalysisByDate(env, dateString, symbols = null) {
    try {
        ...
        const parsedResults = results
            .map((result, index) => (result.success && result.data) ? { ...result.data, symbol: symbols[index] } : null)
            .filter(res => res !== null);
