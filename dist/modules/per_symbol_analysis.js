...
import { initLogging, logInfo, logError, logSentimentDebug, logKVDebug, logWarn } from './logging.js';
...
function convertDualAIToLegacyFormat(dualAIResult, newsData, options = {}) {
    ...
    return {
        ...
        news_data: {
            ...
            time_range: {
                earliest: newsData.length > 0 ? Math.min(...newsData.map(item => new Date(item.publishedAt ?? item.published_at).getTime())) : new Date().getTime(),
                latest: newsData.length > 0 ? Math.max(...newsData.map(item => new Date(item.publishedAt ?? item.published_at).getTime())) : new Date().getTime()
            }
        },
...
function calculateArticleWeight(article) {
    ...
    const ageInHours = (Date.now() - new Date(article.publishedAt ?? article.published_at).getTime()) / (1000 * 60 * 60);
...
//# sourceMappingURL=per_symbol_analysis.js.map