...
export async function handleGetResults(request, env) {
    try {
        ...
        const date = url.searchParamsget('date') || new Date().toISOString().split('T')[0];
...
export async function handleKVGet(request, env) {
    try {
        const url = new URL(request.url);
        const key = url.searchParamsget('key');
        if (!key) {
            return new Response(JSON.stringify({
...
export async function handleTestLlama(request, env) {
    try {
        ...
        const url = new URL(request.url);
        const model = url.searchParamsget('model') || '@cf/meta/llama-3.1-8b-instruct';
        ...
        try {
            const response = await env.AIrun(model, {
                messages: [
                    {
                        role: 'user',
                        content: testPrompt
...
export async function handleModelScopeTest(request, env) {
    try {
        ...
        if (request.method === 'POST') {
            ...
            const url = new URL(request.url);
            apiKey = url.searchParamsget('key') || undefined;
        }
        if (!apiKey) {
            return new Response(JSON.stringify({
                success: false,
...
export async function handleSentimentDebugTest(request, env) {
    try {
        ...
        try {
            const distilTest = await env.AIrun('@cf/huggingface/distilbert-sst-2-int8', {
        ...
        try {
            const gptTest = await env.AIrun('@cf/openchat/openchat-3.5-0106', {
...
export async function handleModelHealth(request, env) {
    try {
        ...
        try {
            const listResponse = await env.ENHANCED_MODELSlist();
            healthResult.bucket_contents = listResponse.objects?.map((obj) => ({
                key: obj.key,
                size: obj.size,
                modified: obj.uploaded
            })) || [];
            console.log(`📋 Found ${healthResult.bucket_contentslength} objects in R2 bucket`);
        }
        catch (listError) {
            healthResult.errorspush(`Failed to list bucket contents: ${listError.message}`);
        }
        ...
        for (const fileName of filesToTest) {
            try {
                ...
                const fileResponse = await env.ENHANCED_MODELSget(fileName);
                if (fileResponse) {
        ...
        for (const [fieldName, file] of formData.entries()) {
            if (file instanceof File) {
                try {
                    ...
                    const uploadResponse = await env.ENHANCED_MODELSput(r2Key, fileData, {
                        httpMetadata: {
                ...
                try {
                    ...
                    const uploadResponse = await env.ENHANCED_MODELSput(r2Key, content, {
                        httpMetadata: {
        ...
        try {
            ...
            try {
                ...
                if (newKVCount > initialKVCount) {
                    ...
                    const testTimestamp = testResults.test_execution_idsplit("_")[3];
                    const newRecords = postKVList.keys?.filter(k => k.includes(testTimestamp) ||
                ...
                testResults.errorspush(`${test.name}: KV storage verification failed - ${kvVerifyError.message}`);
            ...
            testResults.errorspush(`${test.name}: ${error.message}`);
            testResults.overall_success = false;
        }
    }
    // Check KV logs
...
export async function handlePerSymbolAnalysis(request, env) {
    try {
        ...
        console.log('🔍 [TROUBLESHOOT] Headers:', Object.fromEntries(request.headersentries()));
        // Get symbol from URL query parameters
        const url = new URL(request.url);
        const symbol = url.searchParamsget('symbol');
        console.log('🔍 [TROUBLESHOOT] Extracted symbol:', symbol);
        console.log('🔍 [TROUBLESHOOT] All query params:', Object.fromEntries(url.searchParamsentries()));
        if (!symbol) {
        ...
        const options = {
            includeTechnical: url.searchParamsget('include-technical') === 'true',
            timeHorizon: url.searchParamsget('time-horizon') || 'short',
            confidenceThreshold: parseFloat(url.searchParamsget('confidence-threshold')) || 0.6
        };
...
export async function handleDailySummaryAPI(request, env) {
    try {
        ...
        const url = new URL(request.url);
        const dateParam = url.searchParamsget('date');
        ...
        console.log(`✅ [DAILY-SUMMARY-API] Retrieved summary for ${validatedDate}: ${summaryData.summarytotal_predictions} predictions`);
        const response = {
            success: true,
            date: validatedDate,
            data: summaryData,
...
export async function handleBackfillDailySummaries(request, env) {
    try {
        ...
        const url = new URL(request.url);
        const days = parseInt(url.searchParamsget('days')) || 30;
        const skipExisting = url.searchParamsget('skip-existing') !== 'false';
        const tradingDaysOnly = url.searchParamsget('trading-days-only') === 'true';
...
export async function handleVerifyBackfill(request, env) {
    try {
        ...
        const url = new URL(request.url);
        const days = parseInt(url.searchParamsget('days')) || 10;
