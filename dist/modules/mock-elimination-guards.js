 ...
 */
export function detectMockData(value, fieldName, location) {
 ...
 */
export function validateDataSourceConfig(config, location) {
    const detections = [];
 ...
 */
export const REAL_DATA_REQUIREMENTS = {
 ...
 */
export function validateRealDataUsage(data, location) {
    const violations = [];
...
${violations.map(v => `• ${v.detectionReason}`).join('\n')}
    ...
    getComplianceStatus() {
        ...
        const recommendations = Array.from(new Set(this.violations.map(v => v.recommendedAction)));
        return {
 ...
 */
export function requireRealData(location) {
...
//# sourceMappingURL=mock-elimination-guards.js.map