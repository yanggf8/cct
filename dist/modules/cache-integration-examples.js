 ...
 */
import { createEnhancedDAL } from './enhanced-dal.js';
import { createDOCacheAdapter } from './do-cache-adapter.js';
import { createLogger } from './logging.js';
const logger = createLogger('cache-examples');
/**
 * Example 1: Basic Enhanced DAL Usage
 */
export async function basicEnhancedDALExample(env) {
    ...
    try {
        ...
        console.log(`   Cache hit rate: ${(stats.cacheoverallHitRate * 100).toFixed(1)}%`);
        console.log(`   L1 hits: ${stats.cachel1Hits}`);
        console.log(`   L2 hits: ${stats.cachel2Hits}`);
 ...
 */
export async function sectorDataCachingExample(env) {
 ...
 */
export async function marketDriversCachingExample(env) {
    ...
    try {
        for (const driverType of driverTypes) {
            ...
            if (driversResult.success) {
                ...
                const storeResult = await enhancedDAL.storeMarketDriversData(driverType, freshData, 1800 // 30 minutes TTL
                );
 ...
 */
export async function apiResponseCachingExample(env) {
    ...
    try {
        for (const { endpoint, params } of apiEndpoints) {
            ...
            const storeResult = await enhancedDAL.storeApiResponse(endpoint, mockResponse, params, 900 // 15 minutes
            );
            if (storeResult.success) {
 ...
 */
export async function cacheManagementExample(env) {
    ...
    try {
        ...
        const stats = enhancedDAL.getPerformanceStats();
        console.log(`\n📊 Updated Statistics:`);
        console.log(`   Total requests: ${stats.cachetotalRequests}`);
        console.log(`   L1 hits: ${stats.cachel1Hits}`);
        console.log(`   L2 hits: ${stats.cachel2Hits}`);
        console.log(`   Hit rate: ${(stats.cacheoverallHitRate * 100).toFixed(1)}%`);
        ...
        console.log(`   Requests after reset: ${finalStats.cachetotalRequests}`);
 ...
 */
export async function advancedCacheManagerExample(env) {
    ...
    try {
        ...
        const retrievedSession = await cacheManager.get('user_sessions', userId /* @ts-ignore */ /* @ts-ignore */ /* @ts-ignore */ /* @ts-ignore */);
        if (retrievedSession) {
            console.log(`✅ User session retrieved for ${userId}`);
            console.log(`   Login time: ${retrievedSession.loginTime}`);
            console.log(`   Theme: ${retrievedSession.preferencestheme}`);
        }
        ...
        await cacheManager.set('analysis_results', analyticsKey, analyticsData, { l1: 120, l2: 7200 } // Custom TTL: 2min L1, 2hr L2
        );
        ...
        const cachedResult = await cacheManager.get('computation_results', 'expensive_calc_1', expensiveOperation
 ...
 */
export async function cachePerformanceTest(env) {
    ...
    try {
        ...
        const finalStats = enhancedDAL.getPerformanceStats();
        console.log(`\n📈 Final Statistics:`);
        console.log(`   Cache hit rate: ${(finalStats.cacheoverallHitRate * 100).toFixed(1)}%`);
        console.log(`   Total cache hits: ${finalStats.cachel1Hits + finalStats.cachel2Hits}`);
        console.log(`   L1 hits: ${finalStats.cachel1Hits}`);
        console.log(`   L2 hits: ${finalStats.cachel2Hits}`);
    }
    catch (error) {
 ...
 */
export async function runAllCacheExamples(env) {
...
//# sourceMappingURL=cache-integration-examples.js.map