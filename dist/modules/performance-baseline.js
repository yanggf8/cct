...
export class PerformanceBaseline {
    ...
    async getBaselineReport(timeframe = '24h') {
        ...
        for (const [operation, measurements] of this.metrics.entries()) {
            ...
            // Count trends
            if (trend === 'improving')
                report.summary.trends.improving++;
            else if (trend === 'stable')
                report.summary.trends.stable++;
            else if (trend === 'degrading')
                report.summary.trends.degrading++;
        }
        return report;
    }
    /**
 ...
 */
export function createPerformanceMiddleware(operation) {
    return (request, env, ctx, next) => {
