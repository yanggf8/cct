...
export class MarketStructureFetcher {
    ...
    determineYieldCurveTrend(spread) {
        ...
        if (spread > 1.5)
            return 'steepening'; // Significantly steep yield curve
        if (spread < -0.25)
            return 'flattening'; // Inverted or nearly inverted
    ...
    estimateVIXPercentile(vix) {
        ...
        if (vix < 12)
            return 5; // Very low volatility - 5th percentile
        if (vix < 15)
            return 10; // Low volatility - 10th percentile
        if (vix < 18)
            return 25; // Below average - 25th percentile
        if (vix < 22)
            return 45; // Average - 45th percentile
        if (vix < 28)
            return 70; // Elevated - 70th percentile
        if (vix < 35)
            return 85; // High volatility - 85th percentile
        if (vix < 45)
            return 95; // Very high - 95th percentile
        return 98; // Extreme volatility - 98th percentile
    }
    determineVIXVolatilityRegime(vix, percentile) {
        if (vix < 15 && percentile < 25)
            return 'low';
        if (vix < 25 && percentile < 75)
            return 'normal';
        if (vix < 40 && percentile < 90)
            return 'elevated';
        return 'extreme';
    }
    /**
    ...
    async calculateDollarPercentile(currentDollar) {
        ...
        if (currentDollar < 98)
            return 20;
        if (currentDollar < 102)
            return 50;
        if (currentDollar < 106)
            return 80;
        return 90;
    }
    /**
    ...
    calculateRiskAppetite(market) {
        ...
        if (market.vix < 15)
            ...
            score += 10;
        else if (market.vix > 35)
            score -= 30;
        else if (market.vix > 30)
            score -= 10;
        // Yield curve impact (normal curve = higher risk appetite)
        if (market.yieldCurveStatus === 'normal')
            score += 20;
        else if (market.yieldCurveStatus === 'inverted')
            score -= 20;
    ...
    identifyMissingData(market) {
        ...
        if (market.vix === 0)
            missing.push('vix');
        if (market.usDollarIndex === 0)
            missing.push('usDollarIndex');
        if (market.spy === 0)
            missing.push('spy');
        if (market.yield10Y === 0)
            missing.push('yield10Y');
        return missing;
    }
    /**
    ...
    async getEmergencyFallbackMarketStructure() {
        ...
        try {
            ...
            return this.transformRawDataToMarketStructure(basicData);
        }
        catch (error) {
 ...
 */
export function initializeMarketStructureFetcher(options = {}) {
    return new MarketStructureFetcher(options);
}
export default MarketStructureFetcher;
//# sourceMappingURL=market-structure-fetcher.js.map