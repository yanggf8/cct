...
export function generateEmptyStateHTML(options) {
    const { title, reportType, message = 'No data available at this time', diagnosticsLink = '/api/v1/health', requestId = crypto.randomUUID() } = options;
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
export function generateErrorStateHTML(options) {
    const { title, error, reportType, requestId = crypto.randomUUID(), showRetry = true } = options;
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
