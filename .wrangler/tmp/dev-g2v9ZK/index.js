...
// .wrangler/tmp/bundle-zjSWHw/checked-fetch.js
...
function logBusinessMetric(metric, value, metadata = {}) {
  const logger31 = createLogger("business");
  logger31.business(metric, value, metadata);
}
function logHealthCheck(component, status, details = {}) {
  const logger31 = createLogger("health");
  logger31.info(`Health check: ${component}`, {
    type: "health_check",
    component,
    status,
    details
...
function createRequestLogger(service) {
  const logger31 = createLogger(`request-${service}`);
  return {
    logRequest: /* @__PURE__ */ __name((request) => {
      const startTime = Date.now();
      const url = new URL(request.url);
      logger31.info("Request received", {
    ...
    logResponse: /* @__PURE__ */ __name((response, path, startTime, metadata = {}) => {
      ...
      logger31.info("Request completed", {
        path,
...
async function waitForConsistency(key, condition, env, options = {}) {
  ...
  const startTime = Date.now();
  let attempt = 0;
  logger2.info("Starting consistency wait", {
    key,
    strategy,
    timeout,
    description
  ...
  while (Date.now() - startTime < timeout) {
    try {
      ...
      if (result) {
        ...
        logger2.info("Consistency achieved", {
          key,
          attempt,
          elapsed: elapsed2,
          description
      ...
      logger2.debug("Consistency check failed", {
        key,
        attempt,
        error: error.message
      });
    ...
    logger2.debug("Waiting for consistency retry", {
      key,
      attempt,
      delay,
      description
  ...
  logger2.warn("Consistency timeout", {
    key,
    attempt,
    elapsed,
    timeout,
  ...
  await waitForConsistency(
    ...
    async (env2, key) => {
      try {
        ...
        logger2.debug("Dependency consistency check failed", {
          date,
          error: error.message
        });
        return false;
...
async function executeAtomicLikeOperation(operationId, operation, env, options = {}) {
  ...
  const startTime = Date.now();
  logger2.info("Starting atomic-like operation", {
    operationId,
    timeout,
    rollbackOnFailure
  });
  try {
    ...
    if (consistencyAchieved) {
      logger2.info("Atomic-like operation completed with consistency", {
        operationId,
      ...
      logger2.warn("Atomic-like operation completed without consistency", {
        operationId,
        duration: Date.now() - startTime
      });
      if (rollbackOnFailure && options.rollback) {
        try {
          ...
          logger2.info("Rollback completed", { operationId });
        } catch (rollbackError) {
          logger2.error("Rollback failed", {
            operationId,
    ...
    logger2.error("Atomic-like operation failed", {
      operationId,
      error: error.message,
      duration: Date.now() - startTime
    });
    if (rollbackOnFailure && options.rollback) {
      try {
        ...
        logger2.info("Rollback completed after error", { operationId });
      } catch (rollbackError) {
        logger2.error("Rollback failed after error", {
          operationId,
          error: rollbackError.message
        });
      }
...
var logger2, CONSISTENCY_CONFIG, kv_consistency_default;
var init_kv_consistency = __esm({
  "src/modules/kv-consistency.js"() {
    init_checked_fetch();
    init_modules_watch_stub();
    init_logging();
    logger2 = createLogger("kv-consistency");
    CONSISTENCY_CONFIG = {
...
async function getWithRetry(key, env, maxRetries = 3, delay = 1e3) {
  logger3.debug("KV GET operation started", { key, maxRetries, delay });
  for (let i = 0; i < maxRetries; i++) {
    try {
      const result = await env.TRADING_RESULTS.get(key);
      if (result) {
        if (i > 0) {
          logger3.info("KV retry successful", { key, attempt: i + 1 });
        } else {
          logger3.info("KV GET successful", { key, bytes: result.length });
        }
        return result;
      } else {
        logger3.debug("KV GET returned null", { key, attempt: i + 1 });
      }
    } catch (error) {
      logger3.warn("KV operation failed, retrying", { key, attempt: i + 1, error: error.message });
    }
    if (i < maxRetries - 1) {
      const retryDelay = delay * Math.pow(2, i);
      logger3.debug("Waiting before retry", { key, delay: retryDelay });
      await new Promise((resolve) => setTimeout(resolve, retryDelay));
    }
  }
  logger3.error("KV GET failed after all retries", { key, maxRetries });
  throw new Error(`KV key ${key} not found after ${maxRetries} retries`);
}
async function putWithVerification(key, value, env, options = {}) {
  logger3.info("KV PUT operation started", {
    key,
    bytes: value.length,
  ...
  try {
    ...
    if (verifyKey === value) {
      logger3.info("KV PUT successful and verified", {
        key,
        bytes: value.length,
        verification: "passed"
      });
      ...
      logger3.error("KV PUT verification failed - value mismatch", {
        key,
    ...
    logger3.error("KV PUT operation failed", {
      key,
      error: error.message,
      bytes: value.length
    });
...
async function deleteWithVerification(key, env) {
  logger3.info("KV DELETE operation started", { key });
  try {
    const exists = await env.TRADING_RESULTS.get(key);
    if (!exists) {
      logger3.warn("KV DELETE - key does not exist", { key });
      return true;
    }
    await env.TRADING_RESULTS.delete(key);
    const verify = await env.TRADING_RESULTS.get(key);
    if (verify === null) {
      logger3.info("KV DELETE successful and verified", { key });
      return true;
    } else {
      logger3.error("KV DELETE verification failed - key still exists", { key });
      return false;
    }
  } catch (error) {
    logger3.error("KV DELETE operation failed", { key, error: error.message });
    throw error;
  }
}
function logKVOperation(operation, key, success, details = {}) {
  if (success) {
    logger3.info("\u2705 KV OPERATION SUCCESS", {
      operation,
      key,
      ...details,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    ...
    logger3.error("\u274C KV OPERATION FAILED", {
      operation,
      key,
      ...details,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
...
async function getMultipleWithRetry(keys, env) {
  ...
  for (const key of keys) {
    try {
      ...
      logger3.warn("Failed to get KV key in batch", { key, error: error.message });
      results[key] = null;
    }
  }
  return results;
...
async function updateJobStatus(jobType, date, status, env, metadata = {}) {
  ...
  logger3.info("Updating job status", {
    jobType,
    date,
    status,
    key: statusKey,
...
async function getJobStatus(jobType, date, env) {
  ...
  try {
    ...
    logger3.debug("Job status not found", { jobType, date });
    return null;
  }
}
async function validateDependencies(date, requiredJobs, env) {
  ...
  logger3.info("Dependency validation completed", {
    date,
    isValid,
    requiredJobs,
    completed,
...
async function createWeeklyInput(fridayDate, env) {
  ...
  logger3.info("Creating weekly input", { weekKey, fridayDate });
  const eodSummaries = {};
  for (let i = 0; i < weekDates.length; i++) {
    ...
    if (status && status.status === "done") {
      ...
      logger3.debug("EOD summary found for weekly input", { date, dayName: dayNames[i] });
    } else {
      logger3.warn("EOD summary missing for weekly input", { date, dayName: dayNames[i] });
    }
  }
  const weeklyInput = {
  ...
  try {
    ...
    logger3.info("Weekly input created successfully", {
      weekKey,
      eodCount: Object.keys(eodSummaries).length
    });
    return weeklyInput;
  } catch (error) {
    logger3.error("Failed to create weekly input", { weekKey, error: error.message });
    throw error;
  }
}
async function getWeeklyInput(weekKey, env) {
  try {
    ...
    logger3.info("Weekly input not found, building on-demand", { weekKey });
...
async function rateLimitedFetch(url, options = {}, rateLimiter = yahooFinanceRateLimiter) {
  ...
  if (!rateLimiter.isAllowed()) {
    ...
    logger4.warn("Rate limit exceeded", {
      url,
      retryAfter,
      status
    });
  ...
  logger4.debug("Making rate-limited request", {
    url,
  ...
  try {
    ...
    if (error.name === "AbortError") {
      logger4.warn("Request timeout", { url });
      throw new Error("Request timeout - Yahoo Finance API did not respond");
    }
    throw error;
  }
}
var logger4, RateLimiter, yahooFinanceRateLimiter, fallbackApiRateLimiter;
var init_rate_limiter = __esm({
  "src/modules/rate-limiter.js"() {
    init_checked_fetch();
    init_modules_watch_stub();
    init_logging();
    logger4 = createLogger("rate-limiter");
    RateLimiter = class {
      static {
...
async function withCache(symbol, fetchFunction, days = 50) {
  ...
  logger5.debug(`Cache miss for ${symbol}, fetching fresh data`);
  try {
    ...
    logger5.warn(`Failed to fetch fresh data for ${symbol}`, { error: error.message });
    throw error;
  }
}
var logger5, MarketDataCache, globalMarketDataCache;
var init_market_data_cache = __esm({
  "src/modules/market-data-cache.js"() {
    init_checked_fetch();
    init_modules_watch_stub();
    init_logging();
    logger5 = createLogger("market-data-cache");
    MarketDataCache = class {
      ...
      get(symbol, days = 50) {
        ...
        if (this.isValid(entry)) {
          logger5.debug(`Cache hit for ${symbol}`, {
            symbol,
        ...
        if (entry) {
          ...
          logger5.debug(`Cache expired for ${symbol}`, {
            symbol,
            age: Date.now() - entry.timestamp
          });
        }
      ...
      set(symbol, data, days = 50) {
        ...
        logger5.debug(`Cached market data for ${symbol}`, {
          symbol,
      ...
      cleanup() {
        ...
        if (cleanedCount > 0) {
          logger5.debug(`Cleaned up ${cleanedCount} expired cache entries`);
        }
        return cleanedCount;
      }
      /**
      ...
      clear() {
        ...
        this.hitCount = 0;
        this.missCount = 0;
        logger5.info(`Cleared all cache entries`, { entriesCleared: size });
      }
    };
...
var logger6, KV_KEYS, TTL_CONFIG, KVStorageManager, kvStorageManager;
var init_kv_storage_manager = __esm({
  "src/modules/kv-storage-manager.js"() {
    init_checked_fetch();
    init_modules_watch_stub();
    init_logging();
    logger6 = createLogger("kv-storage-manager");
    KV_KEYS = {
    ...
    KVStorageManager = class {
      ...
      async storeHighConfidenceSignals(env, date, signals) {
        ...
        try {
          ...
          logger6.info("Stored high-confidence signals", {
            date: dateStr,
          ...
          logger6.error("Failed to store high-confidence signals", {
            date: dateStr,
            error: error.message
          });
          return false;
      ...
      async getHighConfidenceSignals(env, date) {
        ...
        try {
          ...
          logger6.error("Failed to retrieve high-confidence signals", {
            date: dateStr,
            error: error.message
          });
        }
      ...
      async updateSignalTracking(env, signalId, trackingData, date) {
        ...
        try {
          ...
          logger6.debug("Updated signal tracking", {
            signalId,
            date: dateStr,
          ...
          logger6.error("Failed to update signal tracking", {
            signalId,
            date: dateStr,
            error: error.message
          });
      ...
      async getSignalTracking(env, date) {
        ...
        try {
          ...
          logger6.error("Failed to retrieve signal tracking", {
            date: dateStr,
            error: error.message
          });
        }
      ...
      async storeMarketPrices(env, symbol, priceData) {
        ...
        try {
          ...
          logger6.debug("Stored market prices", {
            symbol,
            currentPrice: priceData.currentPrice,
            changePercent: priceData.changePercent
          });
          ...
          logger6.error("Failed to store market prices", {
            symbol,
            error: error.message
          });
          return false;
      ...
      async getMarketPrices(env, symbol) {
        ...
        try {
          ...
          logger6.error("Failed to retrieve market prices", {
            symbol,
            error: error.message
          });
        }
      ...
      async storeDailyReport(env, reportType, date, reportData) {
        ...
        try {
          ...
          logger6.info("Stored daily report", {
            reportType,
            date: dateStr,
          ...
          logger6.error("Failed to store daily report", {
            reportType,
            date: dateStr,
            error: error.message
          });
      ...
      async getDailyReport(env, reportType, date) {
        ...
        try {
          ...
          logger6.error("Failed to retrieve daily report", {
            reportType,
            date: dateStr,
            error: error.message
          });
      ...
      clearCache() {
        ...
        this.hitCount = 0;
        this.missCount = 0;
        logger6.info("Cleared KV storage cache");
      }
    };
...
var logger7, CronSignalTracker, cronSignalTracker;
var init_cron_signal_tracking = __esm({
  "src/modules/cron-signal-tracking.js"() {
    ...
    init_rate_limiter();
    init_kv_utils();
    logger7 = createLogger("cron-signal-tracking");
    CronSignalTracker = class {
      ...
      async saveMorningPredictions(env, analysisData2, date) {
        ...
        try {
          ...
          if (highConfidenceSignals.length === 0) {
            logger7.info("No high-confidence signals to track", { date: dateStr });
            return false;
          }
          ...
          logger7.info("Saving morning predictions to KV", {
            date: dateStr,
            key: predictionsKey,
          ...
          if (success) {
            ...
            try {
              ...
              logger7.warn("Failed to update morning predictions job status", {
                date: dateStr,
          ...
          logger7.error("Failed to save morning predictions", {
            date: dateStr,
            error: error.message
          });
          return false;
      ...
      async getMorningPredictions(env, date) {
        ...
        try {
          ...
          logger7.error("Failed to retrieve morning predictions", {
            date: dateStr,
            error: error.message
          });
        }
      ...
      async updateSignalPerformance(env, date) {
        ...
        if (!predictionsData || !predictionsData.predictions) {
          logger7.warn("No morning predictions found for performance update", { date: dateStr });
          return null;
        }
        try {
          ...
          logger7.info("Updated signal performance", {
            date: dateStr,
          ...
          logger7.error("Failed to update signal performance", {
            date: dateStr,
            error: error.message
          });
          return null;
      ...
      async getCurrentPrices(symbols) {
        ...
        for (const symbol of symbols) {
          try {
            ...
            logger7.warn("Failed to get current price", { symbol, error: error.message });
          }
        }
        return prices;
      }
      ...
      async generateEndOfDaySummary(env, date) {
        ...
        try {
          ...
          logger7.error("Failed to generate end-of-day summary", {
            date: dateStr,
            error: error.message
          });
          return this.getDefaultSummary();
      ...
      generateTomorrowOutlook(predictions, performance2) {
        ...
        try {
          ...
          logger7.error("Failed to generate tomorrow outlook", { error: error.message });
        }
        return outlook;
      }
      /**
...
async function runBasicAnalysis(env, options = {}) {
  ...
  if (highConfidenceSignals.length > 0) {
    ...
    logger8.info("Generated high-confidence signals for 4-report workflow", {
      signalCount: highConfidenceSignals.length,
...
function generateHighConfidenceSignals(analysisResults, currentTime, env) {
  ...
  for (const [symbol, signal] of Object.entries(analysisResults.trading_signals)) {
    if (signal.confidence >= signalConfidenceThreshold) {
      ...
      logger8.debug("Generated high-confidence signal", {
        symbol,
        confidence: signal.confidence,
        prediction: signal.direction
      });
...
async function saveHighConfidenceSignals(env, signals, currentTime) {
  ...
  try {
    ...
    logger8.info("Saved high-confidence signals to KV storage", {
      date: dateStr,
      signalCount: signals.length,
      trackingKey
    });
  } catch (error) {
    logger8.error("Failed to save high-confidence signals to KV", {
      date: dateStr,
      error: error.message
    });
  }
...
async function getHighConfidenceSignalsForTracking(env, date) {
  ...
  try {
    ...
    logger8.error("Failed to retrieve signals for tracking", {
      date: dateStr,
      error: error.message
    });
  }
  ...
  try {
    ...
    if (trackingData) {
      ...
      if (signal) {
        ...
        logger8.debug("Updated signal performance tracking", {
          signalId,
    ...
    logger8.error("Failed to update signal performance tracking", {
      signalId,
      date: dateStr,
      error: error.message
    });
...
// .wrangler/tmp/bundle-zjSWHw/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-zjSWHw/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
init_modules_watch_stub();
init_logging();
var logger9 = createLogger("weekly-review-analysis");
async function generateWeeklyReviewAnalysis(env, currentTime) {
  logger9.info("Generating comprehensive weekly review analysis");
  try {
    ...
    logger9.error("Error generating weekly review analysis", { error: error.message });
...
async function getWeeklyPerformanceData(env, currentTime) {
  ...
  for (const date of dates) {
    try {
      ...
      logger9.warn(`Failed to get data for ${date.toISOString().split("T")[0]}`, { error: error.message });
    }
  }
...
function createHandler(serviceName, handlerFn, options = {}) {
  const logger31 = createLogger(serviceName);
  const {
  ...
  return async (request, env, ctx) => {
    ...
    try {
      logger31.info(`${serviceName} request started`, {
        requestId,
        method: request.method,
        url: request.url,
        userAgent,
      ...
      if (enableAuth && requiredAuth) {
        ...
        if (!apiKey || apiKey !== env.WORKER_API_KEY) {
          logger31.warn("Unauthorized access attempt", { requestId, userAgent });
      ...
      const duration = Date.now() - startTime;
      logger31.info(`${serviceName} completed successfully`, {
        requestId,
        duration,
        status: "success",
        timestamp: (/* @__PURE__ */ new Date()).toISOString()
      ...
      if (enableMetrics) {
        ...
        if (duration > CONFIG.PERFORMANCE.SLOW_REQUEST_THRESHOLD_MS) {
          logger31.warn(`Slow request detected`, {
            requestId,
            service: serviceName,
            duration,
      ...
      const duration = Date.now() - startTime;
      logger31.error(`${serviceName} failed`, {
        requestId,
        error: error.message,
        stack: error.stack,
        duration,
...
init_modules_watch_stub();
init_logging();
var logger10 = createLogger("tomorrow-outlook-tracker");
var TomorrowOutlookTracker = class {
  ...
  async storeTomorrowOutlook(env, currentDate, outlookData) {
    ...
    try {
      ...
      logger10.info("Stored tomorrow outlook", {
        targetDate: tomorrowString,
        generatedOn: currentDateString,
      ...
      logger10.error("Failed to store tomorrow outlook", {
        targetDate: tomorrowString,
        error: error.message
      });
      return false;
  ...
  async getTodaysOutlook(env, currentDate) {
    ...
    try {
      ...
      if (outlookData) {
        ...
        logger10.debug("Retrieved today's outlook", {
      ...
      logger10.error("Failed to retrieve today's outlook", {
        targetDate: currentDateString,
        error: error.message
      });
    }
  ...
  async evaluateTodaysOutlook(env, currentDate, actualMarketData) {
    ...
    try {
      const outlookData = await env.TRADING_RESULTS.get(outlookKey);
      if (!outlookData) {
        logger10.warn("No outlook found to evaluate", { targetDate: currentDateString });
        return null;
      }
      ...
      logger10.info("Evaluated today's outlook", {
        targetDate: currentDateString,
      ...
      logger10.error("Failed to evaluate today's outlook", {
        targetDate: currentDateString,
        error: error.message
      });
      return null;
  ...
  evaluateOutlookAccuracy(predictedOutlook, actualMarketData) {
    ...
    try {
      ...
      logger10.error("Failed to evaluate outlook accuracy", { error: error.message });
      evaluation.score = 0;
    }
    return evaluation;
  }
  ...
  async getOutlookAccuracyHistory(env, days = 30) {
    ...
    try {
      ...
      logger10.info("Retrieved outlook accuracy history", {
        daysRequested: days,
      ...
      logger10.error("Failed to get outlook accuracy history", { error: error.message });
      return [];
    }
  }
  /**
  ...
  async getOutlookAccuracyStats(env) {
    try {
      ...
      logger10.error("Failed to get outlook accuracy stats", { error: error.message });
      return {
        totalOutlooks: 0,
        averageAccuracy: 0,
        biasAccuracy: 0,
...
// src/modules/report-data-retrieval.js
init_enhanced_analysis();
var logger11 = createLogger("report-data-retrieval");
var ReportDataRetrieval = class {
  ...
  async getPreMarketBriefingData(env, date) {
    ...
    try {
      ...
      if (!analysisData2) {
        logger11.error("\u26A0\uFE0F [PRE-MARKET] CRITICAL: Missing analysis data from KV", {
          date: dateStr,
      ...
      if (!predictionsData) {
        logger11.warn("\u26A0\uFE0F [PRE-MARKET] WARNING: Missing morning predictions data", {
          date: dateStr,
      ...
      logger11.info("Retrieved pre-market briefing data", {
        date: dateStr,
      ...
      logger11.error("Failed to retrieve pre-market briefing data", {
        date: dateStr,
        error: error.message
      });
  ...
  async getIntradayCheckData(env, date) {
    ...
    try {
      ...
      if (!predictionsData) {
        logger11.error("\u26A0\uFE0F [INTRADAY] CRITICAL: Missing morning predictions for performance tracking", {
          date: dateStr,
          key: `morning_predictions_${dateStr}`,
      ...
      logger11.info("Retrieved intraday check data", {
        date: dateStr,
      ...
      logger11.error("Failed to retrieve intraday check data", {
        date: dateStr,
        error: error.message
      });
  ...
  async getEndOfDaySummaryData(env, date) {
    ...
    try {
      ...
      if (predictionsData) {
        ...
        try {
          logger11.info("\u{1F916} [END-OF-DAY] Running AI analysis for tomorrow outlook", { date: dateStr });
          ...
          logger11.info("\u2705 [END-OF-DAY] AI-powered tomorrow outlook generated", {
            date: dateStr,
            marketBias: tomorrowOutlook.marketBias,
            confidence: tomorrowOutlook.confidence
          });
        } catch (error) {
          logger11.warn("\u26A0\uFE0F [END-OF-DAY] AI analysis failed, using fallback", {
            date: dateStr,
            error: error.message
          });
      ...
      if (!predictionsData) {
        logger11.error("\u26A0\uFE0F [END-OF-DAY] CRITICAL: Missing predictions data for summary analysis", {
          date: dateStr,
      ...
      if (!analysisData) {
        logger11.warn("\u26A0\uFE0F [END-OF-DAY] WARNING: Missing analysis data", {
          date: dateStr,
          key: `analysis_${dateStr}`,
      ...
      if (tomorrowOutlook && tomorrowOutlook.basedOnData !== "ai_analysis") {
        logger11.warn("\u26A0\uFE0F [END-OF-DAY] WARNING: Using pattern-based tomorrow outlook instead of AI analysis", {
          date: dateStr,
      ...
      logger11.info("Retrieved end-of-day summary data", {
        date: dateStr,
      ...
      logger11.error("Failed to retrieve end-of-day summary data", {
        date: dateStr,
        error: error.message
      });
  ...
  async getWeeklyReviewData(env, date) {
    ...
    try {
      ...
      if (weeklyData.length === 0) {
        logger11.error("\u26A0\uFE0F [WEEKLY-REVIEW] CRITICAL: No weekly performance data found in KV", {
          date: dateStr,
        ...
        logger11.warn("\u26A0\uFE0F [WEEKLY-REVIEW] WARNING: Insufficient weekly data for comprehensive analysis", {
          date: dateStr,
          expectedTradingDays: 5,
          actualDaysFound: weeklyData.length,
      ...
      logger11.info("Retrieved weekly review data", {
        date: dateStr,
      ...
      logger11.error("\u274C [WEEKLY-REVIEW] CRITICAL: Failed to retrieve weekly review data", {
        date: dateStr,
        error: error.message,
  ...
  async getSingleDayPerformanceData(env, dateStr) {
    try {
      ...
      logger11.warn("Failed to get single day performance data", {
        date: dateStr,
        error: error.message
      });
      return null;
  ...
  async getYesterdaysPredictions(env, currentDate) {
    ...
    try {
      ...
      logger11.warn("Failed to get yesterday's predictions", {
        date: yesterdayStr,
        error: error.message
      });
    }
  ...
  async sendDataErrorNotification(reportType, errorType, dateStr, env) {
    try {
      const errorMessage = `\u{1F6A8} DATA ALERT: ${reportType}
\u26A0\uFE0F Issue: ${errorType}
\u{1F4C5} Date: ${dateStr}
\u{1F527} Impact: Report using fallback data - may not reflect actual analysis
\u{1F6E0}\uFE0F  Action: Required - Check KV storage and cron job execution
\u23F0 Time: ${(/* @__PURE__ */ new Date()).toLocaleString("en-US", { timeZone: "America/New_York" })} EDT`;
      const { sendFacebookMessage: sendFacebookMessage2 } = await Promise.resolve().then(() => (init_facebook(), facebook_exports));
      logger11.info(`\u{1F4F1} [FACEBOOK] Sending data error notification for ${reportType}`, {
        reportType,
        errorType,
        dateStr
      });
      await sendFacebookMessage2(errorMessage, env);
      logger11.info(`\u2705 [FACEBOOK] Data error notification sent for ${reportType}`);
    } catch (fbError) {
      logger11.error("\u274C [FACEBOOK] Failed to send data error notification", {
        reportType,
        errorType,
        dateStr,
...
// src/modules/handlers/weekly-review-handlers.js
var logger12 = createLogger("weekly-review-handlers");
var handleWeeklyReview = createHandler("weekly-review", async (request, env) => {
  const requestId = crypto.randomUUID();
  const startTime = Date.now();
  logger12.info("\u{1F4C8} [WEEKLY-REVIEW] Starting weekly review generation", {
    requestId,
    url: request.url,
    userAgent: request.headers.get("user-agent")?.substring(0, 100) || "unknown"
  });
  const today = /* @__PURE__ */ new Date();
  logger12.debug("\u{1F4CA} [WEEKLY-REVIEW] Retrieving weekly review data", {
    requestId,
  ...
  try {
    ...
    if (weeklyData) {
      logger12.info("\u2705 [WEEKLY-REVIEW] Weekly data retrieved successfully", {
        requestId,
      ...
      logger12.warn("\u26A0\uFE0F [WEEKLY-REVIEW] No weekly data found for this week", {
        requestId
      });
    }
  } catch (error) {
    logger12.error("\u274C [WEEKLY-REVIEW] Failed to retrieve weekly data", {
      requestId,
      error: error.message
    });
  }
  const generationStartTime = Date.now();
  logger12.debug("\u{1F3A8} [WEEKLY-REVIEW] Generating HTML content", {
    requestId,
  ...
  logger12.info("\u2705 [WEEKLY-REVIEW] Weekly review generated successfully", {
    requestId,
...
<body>
    ...
    logger12.error("\u274C [WEEKLY-REVIEW] Error generating weekly review HTML", {
      error: error.message,
      stack: error.stack,
...
init_modules_watch_stub();
init_logging();
var logger13 = createLogger("monitoring");
var SystemMetrics = class {
  ...
  incrementCounter(name, value = 1, tags = {}) {
    ...
    logger13.business(name, current + value, {
      type: "counter",
      tags,
      increment: value
    });
  ...
  recordTimer(name, duration, tags = {}) {
    ...
    logger13.performance(name, duration, {
      type: "timer",
      tags
    });
  }
...
var BusinessKPI = {
  ...
  trackPredictionAccuracy: /* @__PURE__ */ __name((accuracy) => {
    ...
    if (!isOnTarget) {
      logger13.warn("Prediction accuracy below target", {
        accuracy,
        target,
  ...
  trackPerformanceKPI: /* @__PURE__ */ __name((responseTime, operation) => {
    ...
    if (responseTime > target) {
      logger13.warn("Response time exceeds target", {
        responseTime,
        target,
        operation,
  ...
  trackCostEfficiency: /* @__PURE__ */ __name((actualCost = 0) => {
    ...
    if (actualCost > target) {
      logger13.warn("Cost exceeds target", {
        actualCost,
        target,
  ...
  trackUptimeKPI: /* @__PURE__ */ __name((uptimePercentage) => {
    ...
    if (uptimePercentage < target) {
      logger13.error("Uptime below target", {
  ...
  trackCronReliability: /* @__PURE__ */ __name((successCount, totalCount, triggerMode) => {
    ...
    if (reliability < 0.95) {
      logger13.error("Cron reliability below threshold", {
        reliability,
        successCount,
        totalCount,
        triggerMode
  ...
  monitorRequest: /* @__PURE__ */ __name((request, handler) => {
    ...
    return {
      complete: /* @__PURE__ */ __name((response) => {
        ...
        logger13.response(response.status, url.pathname, duration, {
          method: request.method,
          userAgent: request.headers.get("User-Agent"),
          ip: request.headers.get("CF-Connecting-IP")
        });
...
init_modules_watch_stub();
init_logging();
var logger14 = createLogger("html-generators");
...
init_kv_utils();
init_kv_consistency();
var logger15 = createLogger("common-handlers");
function createReportHandler(name, dependencies, reportGenerator, htmlGenerator, options = {}) {
  ...
  return createHandler(name, async (request, env) => {
    ...
    logger15.info(`\u{1F4CA} [${name.toUpperCase()}] Starting report generation`, {
      requestId,
      date: dateStr,
      dependencies
    });
    try {
      logger15.debug(`\u{1F517} [${name.toUpperCase()}] Checking dependencies`, { requestId, dependencies });
      const validation = await validateDependencies(dateStr, dependencies, env);
      if (!validation.isValid) {
        logger15.warn(`\u26A0\uFE0F [${name.toUpperCase()}] Dependencies not satisfied`, {
          requestId,
        ...
        try {
          ...
          logger15.debug(`\u{1F504} [${name.toUpperCase()}] KV consistency check results`, {
            requestId,
            consistentJobs: consistencyResults.consistentJobs,
            inconsistentJobs: consistencyResults.inconsistentJobs
          });
        } catch (consistencyError) {
          logger15.debug(`\u{1F504} [${name.toUpperCase()}] KV consistency check failed`, {
            requestId,
      ...
      logger15.debug(`\u2705 [${name.toUpperCase()}] Dependencies satisfied, generating report`, { requestId });
      ...
      logger15.info(`\u2705 [${name.toUpperCase()}] Report generated successfully`, {
        requestId,
        duration: Date.now() - startTime,
      ...
      logger15.error(`\u274C [${name.toUpperCase()}] Report generation failed`, {
        requestId,
        error: error.message,
        stack: error.stack
      });
...
// src/modules/handlers/analysis-handlers.js
init_kv_utils();
var logger16 = createLogger("analysis-handlers");
...
async function handleEnhancedFeatureAnalysis(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger16.info("Enhanced feature analysis requested", { requestId });
    const analysis = await runEnhancedFeatureAnalysis(env, {
      triggerMode: "enhanced_feature_analysis",
      requestId
    });
    logger16.info("Enhanced feature analysis completed", {
      requestId,
      symbolsAnalyzed: analysis.symbols_analyzed?.length || 0
    });
    ...
    logger16.error("Enhanced feature analysis failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleIndependentTechnicalAnalysis(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger16.info("Independent technical analysis requested", { requestId });
    const analysis = await runIndependentTechnicalAnalysis(env, {
      triggerMode: "independent_technical_analysis",
      requestId
    });
    logger16.info("Independent technical analysis completed", {
      requestId,
      symbolsAnalyzed: analysis.symbols_analyzed?.length || 0
    });
    return new Response(JSON.stringify(analysis, null, 2), {
    ...
    logger16.error("Independent technical analysis failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handlePerSymbolAnalysis(request, env) {
  ...
  try {
    if (!symbol) {
      logger16.warn("Per-symbol analysis requested without symbol parameter", { requestId });
      return new Response(JSON.stringify({
        success: false,
    ...
    logger16.info("Per-symbol analysis requested", { requestId, symbol });
    const analysis = await analyzeSingleSymbol(symbol, env, { requestId });
    logger16.info("Per-symbol analysis completed", {
      requestId,
      symbol,
    ...
    logger16.error("Per-symbol analysis failed", {
      requestId,
      symbol,
      error: error.message,
      stack: error.stack
...
async function handleSentimentTest(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger16.info("Sentiment validation test requested", { requestId });
    const validation = await validateSentimentEnhancement(env, { requestId });
    logger16.info("Sentiment validation completed", {
      requestId,
    ...
    logger16.error("Sentiment validation test failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleGenerateMorningPredictions(request, env) {
  ...
  const dateStr = today.toISOString().split("T")[0];
  try {
    logger16.info("\u{1F305} Morning predictions generation requested", { requestId, date: dateStr });
    const analysisKey = `analysis_${dateStr}`;
    const analysisData2 = await env.TRADING_RESULTS.get(analysisKey);
    if (!analysisData2) {
      logger16.warn("\u26A0\uFE0F No analysis data found for today", { requestId, date: dateStr });
      return new Response(JSON.stringify({
        success: false,
    ...
    if (success) {
      logger16.info("\u2705 Morning predictions generated successfully", { requestId, date: dateStr });
      const predictionsKey = `morning_predictions_${dateStr}`;
      ...
      logger16.error("\u274C Failed to generate morning predictions", { requestId, date: dateStr });
      return new Response(JSON.stringify({
        success: false,
    ...
    logger16.error("\u274C Morning predictions generation failed", {
      requestId,
      date: dateStr,
      error: error.message,
      stack: error.stack
...
async function handleStatusManagement(request, env) {
  ...
  const dateStr = today.toISOString().split("T")[0];
  try {
    logger16.info("\u{1F50D} [STATUS] Status management requested", { requestId, date: dateStr });
    ...
    logger16.error("\u274C [STATUS] Status management failed", { requestId, error: error.message });
    return new Response(JSON.stringify({
      success: false,
      error: error.message,
      request_id: requestId,
...
async function handleKVVerificationTest(request, env) {
  ...
  const dateStr = today.toISOString().split("T")[0];
  try {
    logger16.info("\u{1F9EA} [KV VERIFICATION] Comprehensive KV test requested", { requestId, date: dateStr });
    ...
    try {
      ...
      logger16.info("KV PUT test completed", { success, duration: Date.now() - putStartTime });
    } catch (error) {
      results.test_operations.put_with_verification = {
        success: false,
        error: error.message,
      ...
      logger16.error("KV PUT test failed", { error: error.message });
    }
    ...
    try {
      ...
      logger16.info("KV GET test completed", {
        success: true,
        duration: Date.now() - getStartTime,
      ...
      logger16.error("KV GET test failed", { error: error.message });
    }
    ...
    try {
      ...
      logger16.info("KV job status test completed", {
        success: true,
        duration: Date.now() - statusStartTime,
      ...
      logger16.error("KV job status test failed", { error: error.message });
    }
    ...
    try {
      ...
      logger16.info("KV dependency validation test completed", {
        success: true,
        duration: Date.now() - dependencyStartTime,
      ...
      logger16.error("KV dependency validation test failed", { error: error.message });
    }
    try {
      ...
      logger16.info("KV cleanup test completed", { success: true });
    } catch (error) {
      results.test_operations.cleanup = {
        success: false,
        error: error.message,
      ...
      logger16.error("KV cleanup test failed", { error: error.message });
    }
    ...
    logger16.info("\u{1F9EA} [KV VERIFICATION] Comprehensive KV test completed", {
      requestId,
      successRate,
      totalOperations,
    ...
    logger16.error("\u274C [KV VERIFICATION] Comprehensive KV test failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleGetResults(request, env) {
  ...
  try {
    ...
    if (storedData) {
      ...
      logger17.warn("No analysis results found for today", {
        requestId,
        analysisKey,
    ...
    logger17.error("Failed to retrieve results", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleFactTable(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger17.info("Fact table request received", { requestId });
    const factTableData = await getFactTableData(env);
    logger17.info("Fact table data retrieved", {
      requestId,
    ...
    logger17.error("Failed to generate fact table", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleCronHealth(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger17.info("Cron health check requested", { requestId });
    const healthStatus = await getCronHealthStatus(env);
    logger17.info("Cron health check completed", {
      requestId,
    ...
    logger17.error("Cron health check failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleKVDebug(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger17.info("KV debug operation requested", { requestId });
    ...
    logger17.debug("KV write operation successful", { requestId, testKey });
    const retrievedData = await env.TRADING_RESULTS.get(testKey);
    if (!retrievedData) {
      throw new Error("KV read operation failed - data not found");
    }
    const parsedData = JSON.parse(retrievedData);
    logger17.debug("KV read operation successful", { requestId, testKey });
    await env.TRADING_RESULTS.delete(testKey);
    logger17.debug("KV delete operation successful", { requestId, testKey });
    return new Response(JSON.stringify({
      success: true,
    ...
    logger17.error("KV debug operation failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleKVWriteTest(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger17.info("KV write test requested", { requestId });
    ...
    await env.TRADING_RESULTS.put(testKey, JSON.stringify(testData));
    logger17.info("KV write test successful", { requestId, testKey });
    return new Response(JSON.stringify({
      success: true,
      operation: "write",
      test_key: testKey,
    ...
    logger17.error("KV write test failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleKVReadTest(request, env) {
  ...
  try {
    if (!key) {
      logger17.warn("KV read test requested without key parameter", { requestId });
      return new Response(JSON.stringify({
        success: false,
        operation: "read",
    ...
    logger17.info("KV read test requested", { requestId, key });
    const data = await env.TRADING_RESULTS.get(key);
    if (data) {
      const parsedData = JSON.parse(data);
      logger17.info("KV read test successful", { requestId, key });
      return new Response(JSON.stringify({
        success: true,
        operation: "read",
        key,
      ...
      logger17.warn("KV read test - key not found", { requestId, key });
      return new Response(JSON.stringify({
        success: false,
        operation: "read",
    ...
    logger17.error("KV read test failed", {
      requestId,
      key,
      error: error.message,
      stack: error.stack
...
async function handleKVGet(request, env) {
  ...
  try {
    if (!key) {
      logger17.warn("KV get requested without key parameter", { requestId });
      return new Response(JSON.stringify({
        success: false,
        error: "Key parameter is required",
    ...
    logger17.info("KV get requested", { requestId, key });
    const data = await env.TRADING_RESULTS.get(key);
    if (data) {
      const parsedData = JSON.parse(data);
      logger17.info("KV get successful", {
        requestId,
        key,
      ...
      logger17.warn("KV get - key not found", { requestId, key });
      return new Response(JSON.stringify({
        success: false,
        error: "Key not found in KV storage",
        key,
...
init_facebook();
init_logging();
var logger18 = createLogger("health-handlers");
...
async function handleModelHealth(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger18.info("Model health check requested", { requestId });
    const healthResults = {
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
      request_id: requestId,
      models: {},
    ...
    if (env.AI) {
      try {
        ...
        logger18.debug("GPT-OSS-120B model test successful", { requestId });
      } catch (gptError) {
        healthResults.models.gpt_oss_120b = {
          status: "unhealthy",
        ...
        logger18.warn("GPT-OSS-120B model test failed", {
          requestId,
          error: gptError.message
        });
      }
      try {
        ...
        logger18.debug("DistilBERT model test successful", { requestId });
        ...
        healthResults.overall_status = "degraded";
        logger18.warn("DistilBERT model test failed", {
          requestId,
          error: distilbertError.message
        });
      }
      ...
      logger18.error("Cloudflare AI binding not available", { requestId });
    }
    try {
      if (env.MODEL_BUCKET) {
        ...
        logger18.debug("Neural network models accessible", { requestId });
      } else {
        healthResults.models.neural_networks = {
          status: "unavailable",
          error: "R2 model bucket not configured"
        };
        logger18.warn("R2 model bucket not configured", { requestId });
      }
    } catch (r2Error) {
      healthResults.models.neural_networks = {
        status: "unhealthy",
      ...
      logger18.error("Neural network models health check failed", {
        requestId,
        error: r2Error.message
      });
    }
    try {
      ...
      logger18.debug("KV storage health check successful", { requestId });
    } catch (kvError) {
      healthResults.models.kv_storage = {
        status: "unhealthy",
      ...
      healthResults.overall_status = "degraded";
      logger18.error("KV storage health check failed", {
        requestId,
        error: kvError.message
      });
    }
    ...
    logger18.error("Model health check failed completely", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleDebugEnvironment(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger18.info("Debug environment requested", { requestId });
    const envInfo = {
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
      request_id: requestId,
      environment: {
    ...
    logger18.info("Debug environment completed", {
      requestId,
    ...
    logger18.error("Debug environment failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
init_facebook();
init_logging();
var logger19 = createLogger("facebook-handlers");
async function handleFacebookTest(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger19.info("Facebook test requested", { requestId });
    if (!env.FACEBOOK_PAGE_TOKEN || !env.FACEBOOK_RECIPIENT_ID) {
      logger19.warn("Facebook not configured for testing", { requestId });
      return new Response(JSON.stringify({
        success: false,
    ...
    if (facebookResponse.ok) {
      ...
      logger19.info("Facebook test successful", {
        requestId,
      ...
      logger19.error("Facebook API test failed", {
        requestId,
    ...
    logger19.error("Facebook test failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleTestAllFacebookMessages(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger19.info("Test all Facebook messages requested", { requestId });
    if (!env.FACEBOOK_PAGE_TOKEN || !env.FACEBOOK_RECIPIENT_ID) {
      logger19.warn("Facebook not configured for comprehensive testing", { requestId });
      return new Response(JSON.stringify({
        success: false,
    ...
    logger19.info("Facebook message tests completed", {
      requestId,
    ...
    logger19.error("Test all Facebook messages failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleWeeklyReport(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger19.info("Weekly report requested", { requestId });
    await sendWeeklyAccuracyReportWithTracking(env, requestId);
    logger19.info("Weekly report sent successfully", { requestId });
    ...
    logger19.error("Weekly report failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleFridayMarketCloseReport(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger19.info("Friday market close report requested", { requestId });
    const mockAnalysisResult = {
      symbols_analyzed: ["AAPL", "MSFT", "GOOGL", "TSLA", "NVDA"],
      trading_signals: {},
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    ...
    logger19.info("Friday market close report sent successfully", { requestId });
    ...
    logger19.error("Friday market close report failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleDailySummaryAPI(request, env) {
  ...
  try {
    ...
    if (!dailySummary) {
      logger20.info("Daily summary not found, generating new one", {
        requestId,
        targetDate
      });
      dailySummary = await generateDailySummary(targetDate, env);
    }
    logger20.info("Daily summary API completed", {
      requestId,
      targetDate,
    ...
    logger20.error("Daily summary API failed", {
      requestId,
      dateParam,
      error: error.message,
      stack: error.stack
...
async function handleDailySummaryPageRequest(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger20.info("Daily summary page requested", { requestId });
    const response = await handleDailySummaryPage(request, env);
    logger20.info("Daily summary page served", {
      requestId,
      status: response.status,
    ...
    logger20.error("Daily summary page failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleBackfillDailySummaries(request, env) {
  ...
  try {
    ...
    logger20.info("Backfill daily summaries requested", {
      requestId,
      daysRequested: days,
      skipExisting
    });
    if (days > 365) {
      logger20.warn("Backfill request exceeds maximum days", {
        requestId,
        daysRequested: days,
        maximum: 365
      });
    ...
    logger20.info("Backfill daily summaries completed", {
      requestId,
      daysRequested: days,
    ...
    logger20.error("Backfill daily summaries failed", {
      requestId,
      daysParam,
      error: error.message,
      stack: error.stack
...
async function handleVerifyBackfill(request, env) {
  ...
  try {
    ...
    logger20.info("Verify backfill requested", {
      requestId,
      daysToCheck
    });
    if (daysToCheck > 100) {
      logger20.warn("Verify backfill request exceeds maximum days", {
        requestId,
    ...
    logger20.info("Verify backfill completed", {
      requestId,
    ...
    logger20.error("Verify backfill failed", {
      requestId,
      daysParam,
      error: error.message,
      stack: error.stack
...
init_modules_watch_stub();
init_logging();
var logger21 = createLogger("intraday-analysis");
async function generateIntradayPerformance(analysisData2, morningPredictions, env) {
  logger21.info("Generating real-time intraday performance tracking");
  if (!morningPredictions || !analysisData2) {
    logger21.warn("Missing morning predictions or analysis data");
  ...
  try {
    ...
    logger21.error("Error generating intraday performance", { error: error.message });
    return getDefaultIntradayData();
  }
}
...
// src/modules/handlers/intraday-refactored.js
var logger22 = createLogger("intraday-refactored");
var IntradayDataRetriever = class {
  ...
  static async retrieveData(env, date, context = {}) {
    ...
    logger22.debug("\u{1F4E5} [INTRADAY] Retrieving intraday data", { requestId, date });
    try {
      const data = await getIntradayCheckData(date, env, { requestId });
      if (!data) {
        logger22.warn("\u26A0\uFE0F [INTRADAY] No intraday data found", { requestId, date });
        return null;
      }
      logger22.debug("\u2705 [INTRADAY] Intraday data retrieved", {
        requestId,
        date,
      ...
      logger22.error("\u274C [INTRADAY] Failed to retrieve intraday data", {
        requestId,
        date,
        error: error.message
      });
...
var IntradayPerformanceAnalyzer = class {
  ...
  static async analyze(data, context = {}) {
    const { requestId } = context;
    logger22.debug("\u{1F4CA} [INTRADAY] Analyzing performance", { requestId });
    try {
      const analysis = await generateIntradayPerformance(data, { requestId });
      logger22.debug("\u2705 [INTRADAY] Performance analysis completed", {
        requestId,
      ...
      logger22.error("\u274C [INTRADAY] Performance analysis failed", {
        requestId,
        error: error.message
      });
      throw error;
...
var IntradayHTMLGenerator = class {
  ...
  static async generateHTML(analysis, date, env, context = {}) {
    const { requestId } = context;
    logger22.debug("\u{1F3A8} [INTRADAY] Generating HTML", { requestId, date });
    try {
      ...
      logger22.debug("\u2705 [INTRADAY] HTML generation completed", {
        requestId,
        contentLength: content.length
      });
      return content;
    } catch (error) {
      logger22.error("\u274C [INTRADAY] HTML generation failed", {
        requestId,
        error: error.message
      });
      throw error;
...
var handleIntradayCheckEnhanced = createReportHandler(
  ...
  async (env, date, context) => {
    const { requestId } = context;
    logger22.debug("\u{1F517} [INTRADAY-ENHANCED] Enhanced dependency validation", { requestId, date });
    try {
      ...
      if (!consistencyResults.isValid) {
        logger22.warn("\u26A0\uFE0F [INTRADAY-ENHANCED] KV consistency issues detected", {
          requestId,
          consistentJobs: consistencyResults.consistentJobs,
          inconsistentJobs: consistencyResults.inconsistentJobs
        });
        if (consistencyResults.inconsistentJobs.length > 0) {
          logger22.info("\u{1F504} [INTRADAY-ENHANCED] Proceeding with available data", { requestId });
        }
      }
      const data = await IntradayDataRetriever.retrieveData(env, date, context);
      if (!data) {
      ...
      logger22.error("\u274C [INTRADAY-ENHANCED] Enhanced validation failed", {
        requestId,
        error: error.message
      });
...
init_modules_watch_stub();
init_logging();
var logger23 = createLogger("intraday-decomposed");
var IntradayDataModule = class {
  ...
  static async retrieve(env, date, context = {}) {
    const { requestId } = context;
    logger23.debug("\u{1F4E5} Retrieving intraday data", { requestId, date });
    try {
      const data = await getIntradayCheckData(date, env, { requestId });
      if (!data) {
        logger23.warn("\u26A0\uFE0F No intraday data found", { requestId, date });
        return null;
      }
      logger23.debug("\u2705 Intraday data retrieved", {
        requestId,
        signalCount: data.signals?.length || 0
      });
      return data;
    } catch (error) {
      logger23.error("\u274C Failed to retrieve intraday data", {
        requestId,
        date,
        error: error.message
      });
...
var IntradayAnalysisModule = class {
  ...
  static async analyze(data, context = {}) {
    const { requestId } = context;
    logger23.debug("\u{1F4CA} Analyzing intraday performance", { requestId });
    try {
      const analysis = await generateIntradayPerformance(data, { requestId });
      logger23.debug("\u2705 Intraday analysis completed", {
        requestId,
        accuracy: analysis.overallAccuracy,
      ...
      logger23.error("\u274C Intraday analysis failed", {
        requestId,
        error: error.message
      });
      throw error;
...
var IntradayHTMLModule = class {
  ...
  static async generate(analysis, date, env, context = {}) {
    const { requestId } = context;
    logger23.debug("\u{1F3A8} Generating intraday HTML", { requestId, date });
    try {
      const metrics = createStandardMetrics({
        accuracy: analysis.overallAccuracy / 100,
        totalSignals: analysis.totalSignals,
      ...
      logger23.debug("\u2705 Intraday HTML generated", {
        requestId,
        contentLength: content.length
      });
      return content;
    } catch (error) {
      logger23.error("\u274C Failed to generate intraday HTML", {
        requestId,
        error: error.message
      });
      throw error;
...
var handleIntradayCheckConsistent = createReportHandler(
  ...
  async (env, date, context) => {
    const { requestId } = context;
    logger23.debug("\u{1F517} Enhanced dependency validation", { requestId, date });
    try {
      ...
      if (!consistencyResults.isValid) {
        logger23.warn("\u26A0\uFE0F KV consistency issues detected", {
          requestId,
          consistentJobs: consistencyResults.consistentJobs,
          inconsistentJobs: consistencyResults.inconsistentJobs
        });
        if (consistencyResults.inconsistentJobs.length > 0) {
          logger23.info("\u{1F504} Proceeding with available data", { requestId });
        }
      }
      const data = await IntradayDataModule.retrieve(env, date, context);
      if (!data) {
      ...
      logger23.error("\u274C Enhanced validation failed", {
        requestId,
        error: error.message
      });
      const { validateDependencies: validateDependencies2 } = await Promise.resolve().then(() => (init_kv_utils(), kv_utils_exports));
...
init_modules_watch_stub();
init_logging();
var logger24 = createLogger("pre-market-analysis");

// src/modules/handlers/briefing-handlers.js
init_validation();
init_kv_utils();
var logger25 = createLogger("briefing-handlers");
var handlePreMarketBriefing = createHandler("pre-market-briefing", async (request, env) => {
  const requestId = crypto.randomUUID();
  const startTime = Date.now();
  const today = /* @__PURE__ */ new Date();
  const dateStr = today.toISOString().split("T")[0];
  logger25.info("\u{1F680} [PRE-MARKET] Starting pre-market briefing generation", {
    requestId,
    date: dateStr,
    url: request.url,
    userAgent: request.headers.get("user-agent")?.substring(0, 100) || "unknown"
  ...
  validateEnvironment(env);
  logger25.debug("\u2705 [PRE-MARKET] Input validation passed", { requestId });
  logger25.debug("\u{1F517} [PRE-MARKET] Checking dependencies", { requestId });
  try {
    const validation = await validateDependencies(dateStr, ["analysis", "morning_predictions"], env);
    if (!validation.isValid) {
      logger25.warn("\u26A0\uFE0F [PRE-MARKET] Dependencies not satisfied", {
        requestId,
        missing: validation.missing,
        completionRate: validation.completionRate
      });
    ...
    logger25.info("\u2705 [PRE-MARKET] Dependencies validated", {
      requestId,
      completed: validation.completed,
      completionRate: validation.completionRate
    });
  } catch (error) {
    logger25.error("\u274C [PRE-MARKET] Dependency validation failed", {
      requestId,
      error: error.message
    });
  ...
  logger25.debug("\u{1F4CA} [PRE-MARKET] Retrieving pre-market briefing data", {
    requestId,
    date: dateStr
  });
  ...
  try {
    ...
    if (briefingData) {
      logger25.info("\u2705 [PRE-MARKET] Briefing data retrieved successfully", {
        requestId,
        signalCount: briefingData.signals?.length || 0,
        hasData: true
      });
    } else {
      logger25.warn("\u26A0\uFE0F [PRE-MARKET] No briefing data found for today", {
        requestId
      });
    }
  } catch (error) {
    logger25.error("\u274C [PRE-MARKET] Failed to retrieve briefing data", {
      requestId,
      error: error.message
    });
    await updateJobStatus("pre_market_briefing", dateStr, "failed", env, {
  ...
  const generationStartTime = Date.now();
  logger25.debug("\u{1F3A8} [PRE-MARKET] Generating HTML content", { requestId });
  const html = generatePreMarketBriefingHTML(briefingData, today);
  const totalTime = Date.now() - startTime;
  const generationTime = Date.now() - generationStartTime;
  logger25.info("\u2705 [PRE-MARKET] Pre-market briefing generated successfully", {
    requestId,
    totalTimeMs: totalTime,
    generationTimeMs: generationTime,
...
init_kv_utils();
init_validation();
var logger26 = createLogger("intraday-handlers");
var handleIntradayCheck = createHandler("intraday-check", async (request, env) => {
  const requestId = crypto.randomUUID();
  const startTime = Date.now();
  const today = /* @__PURE__ */ new Date();
  const dateStr = today.toISOString().split("T")[0];
  logger26.info("\u{1F4CA} [INTRADAY] Starting intraday performance check generation", {
    requestId,
    date: dateStr,
    url: request.url,
    userAgent: request.headers.get("user-agent")?.substring(0, 100) || "unknown"
  ...
  validateRequest(request);
  validateEnvironment(env);
  logger26.debug("\u2705 [INTRADAY] Input validation passed", { requestId });
  logger26.debug("\u{1F517} [INTRADAY] Checking dependencies", { requestId });
  try {
    const validation = await validateDependencies(dateStr, ["morning_predictions", "pre_market_briefing"], env);
    if (!validation.isValid) {
      logger26.warn("\u26A0\uFE0F [INTRADAY] Dependencies not satisfied", {
        requestId,
        missing: validation.missing,
        completionRate: validation.completionRate
      });
    ...
    logger26.info("\u2705 [INTRADAY] Dependencies validated", {
      requestId,
      completed: validation.completed,
      completionRate: validation.completionRate
    });
  } catch (error) {
    logger26.error("\u274C [INTRADAY] Dependency validation failed", {
      requestId,
      error: error.message
    });
  ...
  logger26.debug("\u{1F50D} [INTRADAY] Retrieving intraday check data", {
    requestId,
    date: dateStr
  });
  ...
  try {
    ...
    if (intradayData) {
      logger26.info("\u2705 [INTRADAY] Intraday data retrieved successfully", {
        requestId,
        signalCount: intradayData.signals?.length || 0,
        hasData: true
      });
    } else {
      logger26.warn("\u26A0\uFE0F [INTRADAY] No intraday data found for today", {
        requestId
      });
    }
  } catch (error) {
    logger26.error("\u274C [INTRADAY] Failed to retrieve intraday data", {
      requestId,
      error: error.message
    });
    await updateJobStatus("intraday_check", dateStr, "failed", env, {
  ...
  const generationStartTime = Date.now();
  logger26.debug("\u{1F3A8} [INTRADAY] Generating HTML content", {
    requestId,
  ...
  const totalTime = Date.now() - startTime;
  const generationTime = Date.now() - generationStartTime;
  logger26.info("\u2705 [INTRADAY] Intraday performance check generated successfully", {
    requestId,
    totalTimeMs: totalTime,
    generationTimeMs: generationTime,
    signalCount: intradayData?.signals?.length || 0,
...
init_logging();
init_rate_limiter();
var logger27 = createLogger("end-of-day-analysis");

// src/modules/handlers/end-of-day-handlers.js
var logger28 = createLogger("end-of-day-handlers");
var handleEndOfDaySummary = createHandler("end-of-day-summary", async (request, env) => {
  const requestId = crypto.randomUUID();
  const startTime = Date.now();
  logger28.info("\u{1F3C1} [END-OF-DAY] Starting end-of-day summary generation", {
    requestId,
    url: request.url,
    userAgent: request.headers.get("user-agent")?.substring(0, 100) || "unknown"
  });
  const today = /* @__PURE__ */ new Date();
  logger28.debug("\u{1F4CA} [END-OF-DAY] Retrieving end-of-day summary data", {
    requestId,
    date: today.toISOString().split("T")[0]
  });
  ...
  try {
    ...
    if (endOfDayData) {
      logger28.info("\u2705 [END-OF-DAY] End-of-day data retrieved successfully", {
        requestId,
      ...
      logger28.warn("\u26A0\uFE0F [END-OF-DAY] No end-of-day data found for today", {
        requestId
      });
    }
  } catch (error) {
    logger28.error("\u274C [END-OF-DAY] Failed to retrieve end-of-day data", {
      requestId,
      error: error.message
    });
  }
  const generationStartTime = Date.now();
  logger28.debug("\u{1F3A8} [END-OF-DAY] Generating HTML content", {
    requestId,
  ...
  const totalTime = Date.now() - startTime;
  const generationTime = Date.now() - generationStartTime;
  logger28.info("\u2705 [END-OF-DAY] End-of-day summary generated successfully", {
    requestId,
    totalTimeMs: totalTime,
    generationTimeMs: generationTime,
...
init_modules_watch_stub();
init_logging();
var logger29 = createLogger("performance-baseline");
var PerformanceBaseline = class {
  ...
  async recordMeasurement(operation, value, metadata = {}) {
    ...
    logger29.info("Performance measurement recorded", {
      operation,
      value,
  ...
  async checkPerformanceAlerts() {
    ...
    if (alerts.length > 0) {
      logger29.warn("Performance alerts detected", {
...
init_modules_watch_stub();
init_logging();
var logger30 = createLogger("alert-system");
var AlertSeverity = {
  LOW: "low",
  MEDIUM: "medium",
  HIGH: "high",
...
var AlertManager = class {
  ...
  async sendAlert(alert) {
    try {
      if (this.isAlertSuppressed(alert)) {
        logger30.debug("Alert suppressed by rules", { alert: alert.id });
      ...
      logger30.error("Failed to send alert", {
        alertId: alert.id,
        error: error.message,
        stack: error.stack
      });
  ...
  suppressAlert(type, operation, durationMs) {
    ...
    logger30.info("Alert suppression added", {
      type,
      operation,
      durationMs,
