...
async function getNewsAPIData(symbol, env) {
  ...
  return data.articles?.map((article2) => ({
    ...
    sentiment: analyzeTextSentiment2(article2.title + " " + (article2.description || "")),
    confidence: 0.6,
...
async function getYahooNews(symbol, env) {
  try {
    ...
    return news.map((item) => ({
      ...
      url: item.link,
      sentiment: analyzeTextSentiment2(item.title + " " + (item.summary || "")),
      confidence: 0.5,
...
function analyzeTextSentiment2(text) {
...
function logBusinessMetric(metric, value, metadata = {}) {
  const logger33 = createLogger("business");
  logger33.business(metric, value, metadata);
}
function logHealthCheck(component, status, details = {}) {
  const logger33 = createLogger("health");
  logger33.info(`Health check: ${component}`, {
    type: "health_check",
    component,
    status,
    details
...
function createRequestLogger(service) {
  const logger33 = createLogger(`request-${service}`);
  return {
    logRequest: /* @__PURE__ */ __name((request) => {
      const startTime = Date.now();
      const url = new URL(request.url);
      logger33.info("Request received", {
    ...
    logResponse: /* @__PURE__ */ __name((response, path, startTime, metadata = {}) => {
      ...
      logger33.info("Request completed", {
        path,
...
// src/modules/dual-ai-analysis.js
...
async function performDualAIComparison(symbol, newsData, env) {
  ...
  try {
    const [gptResult, distilBERTResult] = await Promise.all([
      performGPTAnalysis2(symbol, newsData, env),
      performDistilBERTAnalysis2(symbol, newsData, env)
    ]);
...
async function performGPTAnalysis2(symbol, newsData, env) {
...
${newsContext}`;
    ...
    const analysisData2 = parseNaturalLanguageResponse2(response.response);
    return {
      model: "gpt-oss-120b",
...
async function performDistilBERTAnalysis2(symbol, newsData, env) {
  if (!newsData || newsData.length === 0) {
    return {
  ...
  try {
    const results = await Promise.all(
      newsData.slice(0, 10).map(async (article2, index) => {
        try {
          ...
          return {
            ...
            title: article2.title.substring(0, 100)
          };
        } catch (error) {
...
var init_dual_ai_analysis = __esm({
  "src/modules/dual-ai-analysis.js"() {
    ...
    __name(performGPTAnalysis2, "performGPTAnalysis");
    __name(performDistilBERTAnalysis2, "performDistilBERTAnalysis");
...
function getTimeout(operationType) {
  return CONFIG2.TIMEOUTS[operationType.toUpperCase()] || CONFIG2.TIMEOUTS.DEFAULT;
}
function getRetryCount(operationType) {
  return CONFIG2.RETRY_COUNTS[operationType.toUpperCase()] || CONFIG2.RETRY_COUNTS.DEFAULT;
}
function isValidSymbol(symbol) {
  return CONFIG2.TRADING.SYMBOLS.includes(symbol.toUpperCase());
}
function getErrorMessage(errorType) {
  return CONFIG2.ERROR_MESSAGES[errorType.toUpperCase()] || "Unknown error";
}
var CONFIG2;
  ...
  "src/modules/config.js"() {
    CONFIG2 = {
      ...
      KV_STORAGE: {
        ...
        // 7 days
        STATUS_TTL: 604800,
        // 7 days for job status
        REPORT_CACHE_TTL: 180,
        // 3 minutes for report caching
        METADATA_TTL: 2592e3,
        // 30 days for metadata
        BATCH_SIZE: 50,
        // For batch operations
        CONSISTENCY_TIMEOUT_MS: 15e3,
        // 15 seconds for KV consistency
...
// src/modules/kv-key-factory.js
var logger2, KeyTypes, KEY_TEMPLATES, KEY_TTL_CONFIG, KVKeyFactory, KeyHelpers;
var init_kv_key_factory = __esm({
  "src/modules/kv-key-factory.js"() {
    init_logging();
    logger2 = createLogger("kv-key-factory");
    KeyTypes = {
      // Analysis Data
      ANALYSIS: "analysis",
    ...
    KVKeyFactory = class {
      ...
      static generateKey(keyType, params = {}) {
        ...
        logger2.debug(`Generated key: ${template} for type: ${keyType}`);
        return template;
      }
      /**
      ...
      static getTTL(keyType) {
        ...
        if (ttl === void 0) {
          logger2.warn(`No TTL configured for key type: ${keyType}, using default 24h`);
          return 86400;
        }
        return ttl;
      }
...
// src/modules/shared-utilities.js
function generateRequestId() {
  return crypto.randomUUID();
}
var logger3, DateUtils, ArrayUtils, NumberUtils, StringUtils, ValidationUtils, AsyncUtils, ErrorUtils, PerformanceUtils, KVUtils, ObjectUtils, shared_utilities_default;
var init_shared_utilities = __esm({
  "src/modules/shared-utilities.js"() {
    init_logging();
    init_config();
    init_kv_key_factory();
    logger3 = createLogger("shared-utilities");
      ...
      async retry(fn, options = {}) {
        ...
        while (attempt <= maxRetries) {
          try {
            ...
            logger3.warn("Retry attempt failed", {
              attempt,
              maxRetries,
              delay,
              error: error.message
    ...
    ErrorUtils = {
      ...
      async withErrorHandling(fn, context = {}) {
        try {
          ...
          logger3.error("Async operation failed", {
            error: error.message,
            stack: error.stack,
            context
          });
      ...
      logError(error, context = {}, logLevel = "error") {
        ...
        logger3[logLevel]("Error occurred", errorData);
      },
      /**
      ...
      async retryWithBackoff(fn, options = {}) {
        ...
        while (attempt <= maxRetries) {
          try {
            ...
            logger3.warn(`Retry attempt ${attempt}/${maxRetries}`, {
              error: error.message,
              delay,
              attempt
            });
    ...
    PerformanceUtils = {
      ...
      async measure(fn, label) {
        ...
        logger3.debug("Performance measurement", {
          label,
      ...
      createTimer(label) {
        ...
        return {
          stop() {
            ...
            logger3.debug("Timer completed", { label, durationMs: duration });
            return duration;
          }
        };
      }
...
async function waitForConsistency(key, condition, env, options = {}) {
  ...
  const startTime = Date.now();
  let attempt = 0;
  logger4.info("Starting consistency wait", {
    key,
    strategy,
    timeout,
    description
  ...
  while (Date.now() - startTime < timeout) {
    try {
      ...
      if (result) {
        ...
        logger4.info("Consistency achieved", {
          key,
          attempt,
          elapsed: elapsed2,
          description
      ...
      logger4.debug("Consistency check failed", {
        key,
        attempt,
        error: error.message
      });
    ...
    logger4.debug("Waiting for consistency retry", {
      key,
      attempt,
      delay,
      description
  ...
  logger4.warn("Consistency timeout", {
    key,
    attempt,
    elapsed,
    timeout,
  ...
  await waitForConsistency(
    ...
    async (env2, key) => {
      try {
        ...
        logger4.debug("Dependency consistency check failed", {
          date,
          error: error.message
        });
        return false;
...
async function executeAtomicLikeOperation(operationId, operation, env, options = {}) {
  ...
  const startTime = Date.now();
  logger4.info("Starting atomic-like operation", {
    operationId,
    timeout,
    rollbackOnFailure
  });
  try {
    ...
    if (consistencyAchieved) {
      logger4.info("Atomic-like operation completed with consistency", {
        operationId,
      ...
      logger4.warn("Atomic-like operation completed without consistency", {
        operationId,
        duration: Date.now() - startTime
      });
      if (rollbackOnFailure && options.rollback) {
        try {
          ...
          logger4.info("Rollback completed", { operationId });
        } catch (rollbackError) {
          logger4.error("Rollback failed", {
            operationId,
    ...
    logger4.error("Atomic-like operation failed", {
      operationId,
      error: error.message,
      duration: Date.now() - startTime
    });
    if (rollbackOnFailure && options.rollback) {
      try {
        ...
        logger4.info("Rollback completed after error", { operationId });
      } catch (rollbackError) {
        logger4.error("Rollback failed after error", {
          operationId,
          error: rollbackError.message
        });
      }
...
var logger4, CONSISTENCY_CONFIG, kv_consistency_default;
var init_kv_consistency = __esm({
  "src/modules/kv-consistency.js"() {
    init_logging();
    logger4 = createLogger("kv-consistency");
    CONSISTENCY_CONFIG = {
...
async function getWithRetry(key, env, maxRetries = 3, delay = 1e3) {
  logger5.debug("KV GET operation started", { key, maxRetries, delay });
  for (let i = 0; i < maxRetries; i++) {
    try {
      const result = await env.TRADING_RESULTS.get(key);
      if (result) {
        if (i > 0) {
          logger5.info("KV retry successful", { key, attempt: i + 1 });
        } else {
          logger5.info("KV GET successful", { key, bytes: result.length });
        }
        return result;
      } else {
        logger5.debug("KV GET returned null", { key, attempt: i + 1 });
      }
    } catch (error) {
      logger5.warn("KV operation failed, retrying", { key, attempt: i + 1, error: error.message });
    }
    if (i < maxRetries - 1) {
      const retryDelay = delay * Math.pow(2, i);
      logger5.debug("Waiting before retry", { key, delay: retryDelay });
      await new Promise((resolve) => setTimeout(resolve, retryDelay));
    }
  }
  logger5.error("KV GET failed after all retries", { key, maxRetries });
  throw new Error(`KV key ${key} not found after ${maxRetries} retries`);
}
async function putWithVerification(key, value, env, options = {}) {
  logger5.info("KV PUT operation started", {
    key,
    bytes: value.length,
  ...
  try {
    ...
    if (verifyKey === value) {
      logger5.info("KV PUT successful and verified", {
        key,
        bytes: value.length,
        verification: "passed"
      });
      ...
      logger5.error("KV PUT verification failed - value mismatch", {
        key,
    ...
    logger5.error("KV PUT operation failed", {
      key,
      error: error.message,
      bytes: value.length
    });
...
async function deleteWithVerification(key, env) {
  logger5.info("KV DELETE operation started", { key });
  try {
    const exists = await env.TRADING_RESULTS.get(key);
    if (!exists) {
      logger5.warn("KV DELETE - key does not exist", { key });
      return true;
    }
    await env.TRADING_RESULTS.delete(key);
    const verify = await env.TRADING_RESULTS.get(key);
    if (verify === null) {
      logger5.info("KV DELETE successful and verified", { key });
      return true;
    } else {
      logger5.error("KV DELETE verification failed - key still exists", { key });
      return false;
    }
  } catch (error) {
    logger5.error("KV DELETE operation failed", { key, error: error.message });
    throw error;
  }
}
function logKVOperation(operation, key, success, details = {}) {
  if (success) {
    logger5.info("\u2705 KV OPERATION SUCCESS", {
      operation,
      key,
      ...details,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    ...
    logger5.error("\u274C KV OPERATION FAILED", {
      operation,
      key,
      ...details,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
...
async function getMultipleWithRetry(keys, env) {
  ...
  for (const key of keys) {
    try {
      ...
      logger5.warn("Failed to get KV key in batch", { key, error: error.message });
      results[key] = null;
    }
  }
  return results;
...
async function updateJobStatus(jobType, date, status, env, metadata = {}) {
  ...
  logger5.info("Updating job status", {
    jobType,
    date,
    status,
    key: statusKey,
...
async function getJobStatus(jobType, date, env) {
  ...
  try {
    ...
    logger5.debug("Job status not found", { jobType, date });
    return null;
  }
}
async function validateDependencies(date, requiredJobs, env) {
  ...
  logger5.info("Dependency validation completed", {
    date,
    isValid,
    requiredJobs,
    completed,
...
async function createWeeklyInput(fridayDate, env) {
  ...
  logger5.info("Creating weekly input", { weekKey, fridayDate });
  const eodSummaries = {};
  for (let i = 0; i < weekDates.length; i++) {
    ...
    if (status && status.status === "done") {
      ...
      logger5.debug("EOD summary found for weekly input", { date, dayName: dayNames[i] });
    } else {
      logger5.warn("EOD summary missing for weekly input", { date, dayName: dayNames[i] });
    }
  }
  const weeklyInput = {
  ...
  try {
    ...
    logger5.info("Weekly input created successfully", {
      weekKey,
      eodCount: Object.keys(eodSummaries).length
    });
    return weeklyInput;
  } catch (error) {
    logger5.error("Failed to create weekly input", { weekKey, error: error.message });
    throw error;
  }
}
async function getWeeklyInput(weekKey, env) {
  try {
    ...
    logger5.info("Weekly input not found, building on-demand", { weekKey });
...
function convertDualAIToLegacyFormat(dualAIResult, newsData, options = {}) {
  return {
    ...
    news_data: {
      ...
      time_range: {
        earliest: newsData.length > 0 ? Math.min(...newsData.map((item) => new Date(item.published_at))) : /* @__PURE__ */ new Date(),
        latest: newsData.length > 0 ? Math.max(...newsData.map((item) => new Date(item.published_at))) : /* @__PURE__ */ new Date()
      }
    },
  ...
  for (const result of pipelineResult.analysis_results) {
    if (result && result.symbol) {
      ...
      tradingSignals[result.symbol] = {
        ...
        // Not available in dual AI analysis
        current_price: null,
...
async function rateLimitedFetch(url, options = {}, rateLimiter = yahooFinanceRateLimiter) {
  ...
  logger6.debug("Making rate-limited request", {
    url,
  ...
  try {
    ...
    if (error.name === "AbortError") {
      logger6.warn("Request timeout", { url });
      throw new Error("Request timeout - Yahoo Finance API did not respond");
    }
    throw error;
  }
}
var logger6, RateLimiter, yahooFinanceRateLimiter, fallbackApiRateLimiter;
var init_rate_limiter = __esm({
  "src/modules/rate-limiter.js"() {
    init_logging();
    logger6 = createLogger("rate-limiter");
    RateLimiter = class {
      static {
...
var logger8, KV_KEYS, TTL_CONFIG, KVStorageManager, kvStorageManager;
var init_kv_storage_manager = __esm({
  "src/modules/kv-storage-manager.js"() {
    init_logging();
    logger8 = createLogger("kv-storage-manager");
    KV_KEYS = {
    ...
    KVStorageManager = class {
      ...
      async storeHighConfidenceSignals(env, date, signals) {
        ...
        try {
          ...
          logger8.info("Stored high-confidence signals", {
            date: dateStr,
          ...
          logger8.error("Failed to store high-confidence signals", {
            date: dateStr,
            error: error.message
          });
          return false;
      ...
      async getHighConfidenceSignals(env, date) {
        ...
        try {
          ...
          logger8.error("Failed to retrieve high-confidence signals", {
            date: dateStr,
            error: error.message
          });
        }
      ...
      async updateSignalTracking(env, signalId, trackingData, date) {
        ...
        try {
          ...
          logger8.debug("Updated signal tracking", {
            signalId,
            date: dateStr,
          ...
          logger8.error("Failed to update signal tracking", {
            signalId,
            date: dateStr,
            error: error.message
          });
      ...
      async getSignalTracking(env, date) {
        ...
        try {
          ...
          logger8.error("Failed to retrieve signal tracking", {
            date: dateStr,
            error: error.message
          });
        }
      ...
      async storeMarketPrices(env, symbol, priceData) {
        ...
        try {
          ...
          logger8.debug("Stored market prices", {
            symbol,
            currentPrice: priceData.currentPrice,
            changePercent: priceData.changePercent
          });
          ...
          logger8.error("Failed to store market prices", {
            symbol,
            error: error.message
          });
          return false;
      ...
      async getMarketPrices(env, symbol) {
        ...
        try {
          ...
          logger8.error("Failed to retrieve market prices", {
            symbol,
            error: error.message
          });
        }
      ...
      async storeDailyReport(env, reportType, date, reportData) {
        ...
        switch (reportType) {
          ...
          case "end-of-day":
            ...
            break;
          default:
            logger8.error("Unknown report type", { reportType });
            return false;
        }
        try {
          ...
          logger8.info("Stored daily report", {
            reportType,
            date: dateStr,
          ...
          logger8.error("Failed to store daily report", {
            reportType,
            date: dateStr,
            error: error.message
          });
      ...
      async getDailyReport(env, reportType, date) {
        ...
        try {
          ...
          logger8.error("Failed to retrieve daily report", {
            reportType,
            date: dateStr,
            error: error.message
          });
      ...
      clearCache() {
        ...
        this.hitCount = 0;
        this.missCount = 0;
        logger8.info("Cleared KV storage cache");
      }
    };
...
var logger9, CronSignalTracker, cronSignalTracker;
var init_cron_signal_tracking = __esm({
  "src/modules/cron-signal-tracking.js"() {
    ...
    init_rate_limiter();
    init_kv_utils();
    logger9 = createLogger("cron-signal-tracking");
    CronSignalTracker = class {
      ...
      async saveMorningPredictions(env, analysisData2, date) {
        ...
        try {
          ...
          if (highConfidenceSignals.length === 0) {
            logger9.info("No high-confidence signals to track", { date: dateStr });
            return false;
          }
          ...
          logger9.info("Saving morning predictions to KV", {
            date: dateStr,
            key: predictionsKey,
          ...
          if (success) {
            ...
            try {
              ...
              logger9.warn("Failed to update morning predictions job status", {
                date: dateStr,
          ...
          logger9.error("Failed to save morning predictions", {
            date: dateStr,
            error: error.message
          });
          return false;
      ...
      async getMorningPredictions(env, date) {
        ...
        try {
          ...
          logger9.error("Failed to retrieve morning predictions", {
            date: dateStr,
            error: error.message
          });
        }
      ...
      async updateSignalPerformance(env, date) {
        ...
        if (!predictionsData || !predictionsData.predictions) {
          logger9.warn("No morning predictions found for performance update", { date: dateStr });
          return null;
        }
        try {
          ...
          logger9.info("Updated signal performance", {
            date: dateStr,
          ...
          logger9.error("Failed to update signal performance", {
            date: dateStr,
            error: error.message
          });
          return null;
      ...
      async getCurrentPrices(symbols) {
        ...
        for (const symbol of symbols) {
          try {
            ...
            logger9.warn("Failed to get current price", { symbol, error: error.message });
          }
        }
        return prices;
      }
      ...
      async generateEndOfDaySummary(env, date) {
        ...
        try {
          ...
          logger9.error("Failed to generate end-of-day summary", {
            date: dateStr,
            error: error.message
          });
          return this.getDefaultSummary();
      ...
      generateTomorrowOutlook(predictions, performance2) {
        ...
        try {
          ...
          logger9.error("Failed to generate tomorrow outlook", { error: error.message });
        }
        return outlook;
      }
      /**
...
`;
  ...
  symbols.forEach((symbol) => {
    ...
    if (signal) {
      symbolCount++;
      const isDualAI = signal.analysis_type === "dual_ai_comparison" || signal.models?.gpt || signal.comparison?.agree !== void 0;
      if (isDualAI) {
...
`;
  reportText += `\u{1F916} **Models:** GPT-OSS-120B + DistilBERT Dual AI
`;
...
async function sendWeeklyAccuracyReportWithTracking(env, cronExecutionId) {
  ...
  let reportText = `\u{1F4C8} **WEEKLY PERFORMANCE SUMMARY**
`;
  reportText += `\u{1F5D3}\uFE0F ${now.toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" })}

`;
  reportText += `\u{1F3AF} **This Week's Highlights:**
`;
...
`;
  reportText += `\u{1F4A1} **Next Week Focus:**
`;
...
`;
  reportText += `\u{1F4CA} **View Full Analysis:**
`;
  reportText += `\u{1F517} https://tft-trading-system.yanggf.workers.dev/weekly-review

`;
  reportText += `\u{1F4C8} Charts \u2022 Accuracy Trends \u2022 Model Insights
`;
  reportText += `\u{1F3AF} Trading Signals \u2022 Risk Analysis
...
async function sendFacebookMessage(messageText, env) {
  ...
  console.log(`\u{1F50D} [FB-DEBUG] ${executionId} Starting Facebook message send`);
  console.log(`\u{1F50D} [FB-DEBUG] ${executionId} Facebook config check:`);
  ...
  console.log(`\u{1F50D} [FB-DEBUG] ${executionId} Message details:`);
  ...
  console.log(`\u{1F50D} [FB-DEBUG] ${executionId} Payload constructed:`, JSON.stringify(facebookPayload, null, 2));
  try {
    console.log(`\u{1F4E4} [FB-DEBUG] ${executionId} Sending to Facebook API...`);
    const response = await fetch(`https://graph.facebook.com/v18.0/me/messages?access_token=${env.FACEBOOK_PAGE_TOKEN}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(facebookPayload)
    });
    console.log(`\u{1F50D} [FB-DEBUG] ${executionId} Response status: ${response.status} ${response.statusText}`);
    if (response.ok) {
      const responseText = await response.text();
      console.log(`\u2705 [FB-DEBUG] ${executionId} Facebook message sent successfully`);
      console.log(`\u{1F50D} [FB-DEBUG] ${executionId} Response: ${responseText}`);
      return { success: true, response: responseText };
    } else {
      const errorText = await response.text();
      console.error(`\u274C [FB-DEBUG] ${executionId} Facebook API error:`, errorText);
      console.error(`\u{1F50D} [FB-DEBUG] ${executionId} Error details:`, {
        status: response.status,
...
async function sendMiddayValidationWithTracking(analysisResult, env, cronExecutionId) {
  ...
  let reportText = `\u{1F504} **MIDDAY SIGNAL UPDATE**
`;
  reportText += `\u{1F4CA} Current Sentiment: ${bullishCount} Bullish | ${bearishCount} Bearish
`;
  if (bullishSymbols.length > 0) {
    reportText += `\u{1F4C8} Bullish: ${bullishSymbols.slice(0, 3).join(", ")}${bullishSymbols.length > 3 ? "..." : ""}
`;
  }
  if (bearishSymbols.length > 0) {
    reportText += `\u{1F4C9} Bearish: ${bearishSymbols.slice(0, 2).join(", ")}${bearishSymbols.length > 2 ? "..." : ""}
`;
  }
  if (highConfidenceSymbols.length > 0) {
    reportText += `\u{1F3AF} High Confidence: ${highConfidenceSymbols.slice(0, 2).join(", ")}
`;
  ...
  reportText += `\u{1F4A1} **Afternoon Strategy**: ${afternoonGuidance}
`;
  reportText += `\u{1F4CA} View Real-Time Performance: Signal Accuracy & Tracking
`;
...
function formatDualAIReport(symbol, signal) {
  ...
  const confidenceEmoji = avgConfidence >= 0.8 ? "\u{1F525}" : avgConfidence >= 0.6 ? "\u2B50" : "\u{1F4CA}";
  if (comparison.agree) {
...
`;
  ...
  if (tradingSignal.action && tradingSignal.action !== "SKIP") {
    const actionEmoji = tradingSignal.action.includes("BUY") ? "\u{1F4C8}" : tradingSignal.action.includes("SELL") ? "\u{1F4C9}" : "\u23F8\uFE0F";
    report += `${actionEmoji} **ACTION**: ${tradingSignal.action}
`;
    if (tradingSignal.strength) {
      const strengthEmoji = tradingSignal.strength === "STRONG" ? "\u{1F525}" : tradingSignal.strength === "MODERATE" ? "\u2B50" : "\u{1F4CA}";
      report += `${strengthEmoji} **Strength**: ${tradingSignal.strength}
`;
    }
  }
  if (gptResult.direction && distilbertResult.direction) {
    const gptEmoji = gptResult.direction === "bullish" ? "\u{1F4C8}" : gptResult.direction === "bearish" ? "\u{1F4C9}" : "\u2796";
    const dbEmoji = distilbertResult.direction === "bullish" ? "\u{1F4C8}" : distilbertResult.direction === "bearish" ? "\u{1F4C9}" : "\u2796";
    report += `\u{1F916} **GPT**: ${gptEmoji} ${Math.round((gptResult.confidence || 0) * 100)}%
`;
    report += `\u{1F9E0} **DistilBERT**: ${dbEmoji} ${Math.round((distilbertResult.confidence || 0) * 100)}%
`;
  }
  if (tradingSignal.reasoning) {
    const shortReason = tradingSignal.reasoning.length > 60 ? tradingSignal.reasoning.substring(0, 60) + "..." : tradingSignal.reasoning;
    report += `\u{1F4A1} **Why**: ${shortReason}
`;
  }
  return report;
}
function formatLegacyReport(symbol, signal) {
  ...
  let report = `\u{1F4C8} **LEGACY ANALYSIS**
`;
  report += `\u{1F3AF} **Signal**: ${sentiment.toUpperCase()} (${confidence}% confidence)
`;
  if (sentimentLayer.reasoning) {
    report += `\u{1F4A1} **Analysis**: ${sentimentLayer.reasoning.substring(0, 100)}${sentimentLayer.reasoning.length > 100 ? "..." : ""}
`;
  }
  return report;
}
var init_facebook = __esm({
  "src/modules/facebook.js"() {
    ...
    __name(formatDualAIReport, "formatDualAIReport");
    __name(formatLegacyReport, "formatLegacyReport");
  }
});
...
init_logging();
var logger7 = createLogger("market-data-cache");
var MarketDataCache = class {
  ...
  get(symbol, days = 50) {
    ...
    if (this.isValid(entry)) {
      logger7.debug(`Cache hit for ${symbol}`, {
        symbol,
    ...
    if (entry) {
      ...
      logger7.debug(`Cache expired for ${symbol}`, {
        symbol,
        age: Date.now() - entry.timestamp
      });
    }
  ...
  set(symbol, data, days = 50) {
    ...
    logger7.debug(`Cached market data for ${symbol}`, {
      symbol,
  ...
  cleanup() {
    ...
    if (cleanedCount > 0) {
      logger7.debug(`Cleaned up ${cleanedCount} expired cache entries`);
    }
    return cleanedCount;
  }
  /**
  ...
  clear() {
    ...
    this.hitCount = 0;
    this.missCount = 0;
    logger7.info(`Cleared all cache entries`, { entriesCleared: size });
  }
};
...
async function withCache(symbol, fetchFunction, days = 50) {
  ...
  logger7.debug(`Cache miss for ${symbol}, fetching fresh data`);
  try {
    ...
    logger7.warn(`Failed to fetch fresh data for ${symbol}`, { error: error.message });
    throw error;
  }
}
...
init_logging();
var logger10 = createLogger("analysis");
async function runBasicAnalysis(env, options = {}) {
  ...
  for (const symbol of symbols) {
    try {
      console.log(`   \u{1F9E0} Analyzing ${symbol} with GPT-OSS-120B + DistilBERT-SST-2 models...`);
      const marketData = await withCache(symbol, () => getMarketData(symbol));
      validateMarketData(marketData);
      console.log(`   \u{1F916} Starting GPT-OSS-120B analysis for ${symbol}...`);
      ...
      console.log(`   \u{1F50D} GPT analysis completed for ${symbol}`);
      console.log(`   \u{1F4C8} Analysis result:`, gptAnalysis.overall_sentiment);
  ...
  if (highConfidenceSignals.length > 0) {
    ...
    logger10.info("Generated high-confidence signals for 4-report workflow", {
      signalCount: highConfidenceSignals.length,
...
function generateHighConfidenceSignals(analysisResults, currentTime, env) {
  ...
  for (const [symbol, signal] of Object.entries(analysisResults.trading_signals)) {
    if (signal.confidence >= signalConfidenceThreshold) {
      ...
      logger10.debug("Generated high-confidence signal", {
        symbol,
        confidence: signal.confidence,
        prediction: signal.direction
      });
...
async function saveHighConfidenceSignals(env, signals, currentTime) {
  ...
  try {
    ...
    logger10.info("Saved high-confidence signals to KV storage", {
      date: dateStr,
      signalCount: signals.length,
      trackingKey
    });
  } catch (error) {
    logger10.error("Failed to save high-confidence signals to KV", {
      date: dateStr,
      error: error.message
    });
  }
...
init_logging();
var logger11 = createLogger("weekly-review-analysis");
async function generateWeeklyReviewAnalysis(env, currentTime) {
  logger11.info("Generating comprehensive weekly review analysis");
  try {
    ...
    logger11.error("Error generating weekly review analysis", { error: error.message });
...
async function getWeeklyPerformanceData(env, currentTime) {
  ...
  for (const date of dates) {
    try {
      ...
      logger11.warn(`Failed to get data for ${date.toISOString().split("T")[0]}`, { error: error.message });
    }
  }
...
function createHandler(serviceName, handlerFn, options = {}) {
  const logger33 = createLogger(serviceName);
  const {
    timeout = CONFIG2.TIMEOUTS.API_REQUEST,
    enableMetrics = true,
    enableAuth = false,
    requiredAuth = false
  } = options;
  return async (request, env, ctx) => {
    ...
    try {
      logger33.info(`${serviceName} request started`, {
        requestId,
        method: request.method,
        url: request.url,
        userAgent,
      ...
      if (enableAuth && requiredAuth) {
        ...
        if (!apiKey || apiKey !== env.WORKER_API_KEY) {
          logger33.warn("Unauthorized access attempt", { requestId, userAgent });
      ...
      const duration = Date.now() - startTime;
      logger33.info(`${serviceName} completed successfully`, {
        requestId,
        duration,
        status: "success",
        timestamp: (/* @__PURE__ */ new Date()).toISOString()
      ...
      if (enableMetrics) {
        ...
        if (duration > CONFIG2.PERFORMANCE.SLOW_REQUEST_THRESHOLD_MS) {
          logger33.warn(`Slow request detected`, {
            requestId,
            service: serviceName,
            duration,
            threshold: CONFIG2.PERFORMANCE.SLOW_REQUEST_THRESHOLD_MS
          });
        }
      }
      return result;
      ...
      const duration = Date.now() - startTime;
      logger33.error(`${serviceName} failed`, {
        requestId,
        error: error.message,
        stack: error.stack,
        duration,
...
function createHealthHandler(serviceName, healthCheckFn) {
  return createHandler(serviceName, async (request, env, ctx) => {
    ...
    enableMetrics: true,
    timeout: CONFIG2.TIMEOUTS.KV_OPERATION
  });
}
...
init_logging();
var logger12 = createLogger("tomorrow-outlook-tracker");
var TomorrowOutlookTracker = class {
  ...
  async storeTomorrowOutlook(env, currentDate, outlookData) {
    ...
    try {
      ...
      logger12.info("Stored tomorrow outlook", {
        targetDate: tomorrowString,
        generatedOn: currentDateString,
      ...
      logger12.error("Failed to store tomorrow outlook", {
        targetDate: tomorrowString,
        error: error.message
      });
      return false;
  ...
  async getTodaysOutlook(env, currentDate) {
    ...
    try {
      ...
      if (outlookData) {
        ...
        logger12.debug("Retrieved today's outlook", {
      ...
      logger12.error("Failed to retrieve today's outlook", {
        targetDate: currentDateString,
        error: error.message
      });
    }
  ...
  async evaluateTodaysOutlook(env, currentDate, actualMarketData) {
    ...
    try {
      const outlookData = await env.TRADING_RESULTS.get(outlookKey);
      if (!outlookData) {
        logger12.warn("No outlook found to evaluate", { targetDate: currentDateString });
        return null;
      }
      ...
      logger12.info("Evaluated today's outlook", {
        targetDate: currentDateString,
      ...
      logger12.error("Failed to evaluate today's outlook", {
        targetDate: currentDateString,
        error: error.message
      });
      return null;
  ...
  evaluateOutlookAccuracy(predictedOutlook, actualMarketData) {
    ...
    try {
      ...
      logger12.error("Failed to evaluate outlook accuracy", { error: error.message });
      evaluation.score = 0;
    }
    return evaluation;
  }
  ...
  async getOutlookAccuracyHistory(env, days = 30) {
    ...
    try {
      ...
      logger12.info("Retrieved outlook accuracy history", {
        daysRequested: days,
      ...
      logger12.error("Failed to get outlook accuracy history", { error: error.message });
      return [];
    }
  }
  /**
  ...
  async getOutlookAccuracyStats(env) {
    try {
      ...
      logger12.error("Failed to get outlook accuracy stats", { error: error.message });
      return {
        totalOutlooks: 0,
        averageAccuracy: 0,
        biasAccuracy: 0,
...
// src/modules/report-data-retrieval.js
init_enhanced_analysis();
var logger13 = createLogger("report-data-retrieval");
var ReportDataRetrieval = class {
  ...
  async getPreMarketBriefingData(env, date) {
    ...
    try {
      ...
      if (!analysisData2) {
        logger13.error("\u26A0\uFE0F [PRE-MARKET] CRITICAL: Missing analysis data from KV", {
          date: dateStr,
      ...
      if (!predictionsData) {
        logger13.warn("\u26A0\uFE0F [PRE-MARKET] WARNING: Missing morning predictions data", {
          date: dateStr,
      ...
      logger13.info("Retrieved pre-market briefing data", {
        date: dateStr,
        hasAnalysis: !!result.analysis,
        hasPredictions: !!result.morningPredictions,
        outlookEvaluated: !!outlookEvaluation,
        usingFallback: !analysisData2 || !predictionsData
      });
      return result;
    } catch (error) {
      logger13.error("Failed to retrieve pre-market briefing data", {
        date: dateStr,
        error: error.message
      });
  ...
  async getIntradayCheckData(env, date) {
    ...
    try {
      ...
      if (!predictionsData) {
        logger13.error("\u26A0\uFE0F [INTRADAY] CRITICAL: Missing morning predictions for performance tracking", {
          date: dateStr,
          key: `morning_predictions_${dateStr}`,
      ...
      logger13.info("Retrieved intraday check data", {
        date: dateStr,
      ...
      logger13.error("Failed to retrieve intraday check data", {
        date: dateStr,
        error: error.message
      });
  ...
  async getEndOfDaySummaryData(env, date) {
    ...
    try {
      ...
      if (predictionsData) {
        ...
        try {
          logger13.info("\u{1F916} [END-OF-DAY] Running AI analysis for tomorrow outlook", { date: dateStr });
          ...
          logger13.info("\u2705 [END-OF-DAY] AI-powered tomorrow outlook generated", {
            date: dateStr,
            marketBias: tomorrowOutlook.marketBias,
            confidence: tomorrowOutlook.confidence
          });
        } catch (error) {
          logger13.warn("\u26A0\uFE0F [END-OF-DAY] AI analysis failed, using fallback", {
            date: dateStr,
            error: error.message
          });
      ...
      if (!predictionsData) {
        logger13.error("\u26A0\uFE0F [END-OF-DAY] CRITICAL: Missing predictions data for summary analysis", {
          date: dateStr,
      ...
      if (!analysisData) {
        logger13.warn("\u26A0\uFE0F [END-OF-DAY] WARNING: Missing analysis data", {
          date: dateStr,
          key: `analysis_${dateStr}`,
      ...
      if (tomorrowOutlook && tomorrowOutlook.basedOnData !== "ai_analysis") {
        logger13.warn("\u26A0\uFE0F [END-OF-DAY] WARNING: Using pattern-based tomorrow outlook instead of AI analysis", {
          date: dateStr,
      ...
      logger13.info("Retrieved end-of-day summary data", {
        date: dateStr,
      ...
      logger13.error("Failed to retrieve end-of-day summary data", {
        date: dateStr,
        error: error.message
      });
  ...
  async getWeeklyReviewData(env, date) {
    ...
    try {
      ...
      if (weeklyData.length === 0) {
        logger13.error("\u26A0\uFE0F [WEEKLY-REVIEW] CRITICAL: No weekly performance data found in KV", {
          date: dateStr,
        ...
        logger13.warn("\u26A0\uFE0F [WEEKLY-REVIEW] WARNING: Insufficient weekly data for comprehensive analysis", {
          date: dateStr,
          expectedTradingDays: 5,
          actualDaysFound: weeklyData.length,
      ...
      logger13.info("Retrieved weekly review data", {
        date: dateStr,
      ...
      logger13.error("\u274C [WEEKLY-REVIEW] CRITICAL: Failed to retrieve weekly review data", {
        date: dateStr,
        error: error.message,
  ...
  async getSingleDayPerformanceData(env, dateStr) {
    try {
      ...
      logger13.warn("Failed to get single day performance data", {
        date: dateStr,
        error: error.message
      });
      return null;
  ...
  async getYesterdaysPredictions(env, currentDate) {
    ...
    try {
      ...
      logger13.warn("Failed to get yesterday's predictions", {
        date: yesterdayStr,
        error: error.message
      });
    }
...
// src/modules/handlers/weekly-review-handlers.js
var logger14 = createLogger("weekly-review-handlers");
var handleWeeklyReview = createHandler("weekly-review", async (request, env) => {
  const requestId = crypto.randomUUID();
  const startTime = Date.now();
  logger14.info("\u{1F4C8} [WEEKLY-REVIEW] Starting weekly review generation", {
    requestId,
    url: request.url,
    userAgent: request.headers.get("user-agent")?.substring(0, 100) || "unknown"
  });
  const today = /* @__PURE__ */ new Date();
  logger14.debug("\u{1F4CA} [WEEKLY-REVIEW] Retrieving weekly review data", {
    requestId,
  ...
  try {
    ...
    if (weeklyData) {
      logger14.info("\u2705 [WEEKLY-REVIEW] Weekly data retrieved successfully", {
        requestId,
      ...
      logger14.warn("\u26A0\uFE0F [WEEKLY-REVIEW] No weekly data found for this week", {
        requestId
      });
    }
  } catch (error) {
    logger14.error("\u274C [WEEKLY-REVIEW] Failed to retrieve weekly data", {
      requestId,
      error: error.message
    });
  }
  const generationStartTime = Date.now();
  logger14.debug("\u{1F3A8} [WEEKLY-REVIEW] Generating HTML content", {
    requestId,
  ...
  logger14.info("\u2705 [WEEKLY-REVIEW] Weekly review generated successfully", {
    requestId,
...
<body>
    ...
    logger14.error("\u274C [WEEKLY-REVIEW] Error generating weekly review HTML", {
      error: error.message,
      stack: error.stack,
...
init_logging();
init_config();
var logger15 = createLogger("monitoring");
var SystemMetrics = class {
  ...
  incrementCounter(name, value = 1, tags = {}) {
    ...
    logger15.business(name, current + value, {
      type: "counter",
      tags,
      increment: value
    });
  ...
  recordTimer(name, duration, tags = {}) {
    ...
    logger15.performance(name, duration, {
      type: "timer",
      tags
    });
  }
...
var BusinessKPI = {
  ...
  trackPredictionAccuracy: /* @__PURE__ */ __name((accuracy) => {
    const target = CONFIG2.BUSINESS_KPI.PREDICTION_ACCURACY_TARGET;
    ...
    if (!isOnTarget) {
      logger15.warn("Prediction accuracy below target", {
        accuracy,
        target,
  ...
  trackPerformanceKPI: /* @__PURE__ */ __name((responseTime, operation) => {
    const target = CONFIG2.BUSINESS_KPI.RESPONSE_TIME_TARGET_MS;
    ...
    if (responseTime > target) {
      logger15.warn("Response time exceeds target", {
        responseTime,
        target,
        operation,
  ...
  trackCostEfficiency: /* @__PURE__ */ __name((actualCost = 0) => {
    const target = CONFIG2.BUSINESS_KPI.COST_PER_ANALYSIS_TARGET;
    ...
    if (actualCost > target) {
      logger15.warn("Cost exceeds target", {
        actualCost,
        target,
  ...
  trackUptimeKPI: /* @__PURE__ */ __name((uptimePercentage) => {
    const target = CONFIG2.BUSINESS_KPI.UPTIME_TARGET;
    ...
    if (uptimePercentage < target) {
      logger15.error("Uptime below target", {
  ...
  trackCronReliability: /* @__PURE__ */ __name((successCount, totalCount, triggerMode) => {
    ...
    if (reliability < 0.95) {
      logger15.error("Cron reliability below threshold", {
        reliability,
        successCount,
        totalCount,
        triggerMode
  ...
  generateKPIDashboard: /* @__PURE__ */ __name(() => {
    ...
    return {
      prediction_accuracy: {
        ...
        target: CONFIG2.BUSINESS_KPI.PREDICTION_ACCURACY_TARGET * 100,
      ...
      uptime: {
        ...
        target: CONFIG2.BUSINESS_KPI.UPTIME_TARGET * 100,
  ...
  monitorRequest: /* @__PURE__ */ __name((request, handler) => {
    ...
    return {
      complete: /* @__PURE__ */ __name((response) => {
        ...
        logger15.response(response.status, url.pathname, duration, {
          method: request.method,
          userAgent: request.headers.get("User-Agent"),
          ip: request.headers.get("CF-Connecting-IP")
        });
...
init_logging();
var logger16 = createLogger("html-generators");
...
init_kv_utils();
init_kv_consistency();
var logger17 = createLogger("common-handlers");
function createReportHandler(name, dependencies, reportGenerator, htmlGenerator, options = {}) {
  ...
  return createHandler(name, async (request, env) => {
    ...
    logger17.info(`\u{1F4CA} [${name.toUpperCase()}] Starting report generation`, {
      requestId,
      date: dateStr,
      dependencies
    });
    try {
      logger17.debug(`\u{1F517} [${name.toUpperCase()}] Checking dependencies`, { requestId, dependencies });
      const validation = await validateDependencies(dateStr, dependencies, env);
      if (!validation.isValid) {
        logger17.warn(`\u26A0\uFE0F [${name.toUpperCase()}] Dependencies not satisfied`, {
          requestId,
        ...
        try {
          ...
          logger17.debug(`\u{1F504} [${name.toUpperCase()}] KV consistency check results`, {
            requestId,
            consistentJobs: consistencyResults.consistentJobs,
            inconsistentJobs: consistencyResults.inconsistentJobs
          });
        } catch (consistencyError) {
          logger17.debug(`\u{1F504} [${name.toUpperCase()}] KV consistency check failed`, {
            requestId,
      ...
      logger17.debug(`\u2705 [${name.toUpperCase()}] Dependencies satisfied, generating report`, { requestId });
      ...
      logger17.info(`\u2705 [${name.toUpperCase()}] Report generated successfully`, {
        requestId,
        duration: Date.now() - startTime,
      ...
      logger17.error(`\u274C [${name.toUpperCase()}] Report generation failed`, {
        requestId,
        error: error.message,
        stack: error.stack
      });
...
function createStandardMetrics(data) {
  const metrics = [];
  ...
  if (data.highConfidenceSignals !== void 0) {
    metrics.push({
...
async function getStockSentiment(symbol, env) {
  try {
    ...
    if (env.MODELSCOPE_API_KEY && newsData.length > 0) {
      ...
      return analyzeTextSentiment2(newsData);
    }
  } catch (error) {
...
// src/modules/handlers/analysis-handlers.js
init_kv_utils();
var logger18 = createLogger("analysis-handlers");
...
async function handleEnhancedFeatureAnalysis(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger18.info("Enhanced feature analysis requested", { requestId });
    const analysis = await runEnhancedFeatureAnalysis(env, {
      triggerMode: "enhanced_feature_analysis",
      requestId
    });
    logger18.info("Enhanced feature analysis completed", {
      requestId,
      symbolsAnalyzed: analysis.symbols_analyzed?.length || 0
    });
    ...
    logger18.error("Enhanced feature analysis failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleIndependentTechnicalAnalysis(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger18.info("Independent technical analysis requested", { requestId });
    const analysis = await runIndependentTechnicalAnalysis(env, {
      triggerMode: "independent_technical_analysis",
      requestId
    });
    logger18.info("Independent technical analysis completed", {
      requestId,
      symbolsAnalyzed: analysis.symbols_analyzed?.length || 0
    });
    return new Response(JSON.stringify(analysis, null, 2), {
    ...
    logger18.error("Independent technical analysis failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handlePerSymbolAnalysis(request, env) {
  ...
  try {
    if (!symbol) {
      logger18.warn("Per-symbol analysis requested without symbol parameter", { requestId });
      return new Response(JSON.stringify({
        success: false,
    ...
    logger18.info("Per-symbol analysis requested", { requestId, symbol });
    const analysis = await analyzeSingleSymbol(symbol, env, { requestId });
    logger18.info("Per-symbol analysis completed", {
      requestId,
      symbol,
    ...
    logger18.error("Per-symbol analysis failed", {
      requestId,
      symbol,
      error: error.message,
      stack: error.stack
...
async function handleSentimentTest(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger18.info("Sentiment validation test requested", { requestId });
    const validation = await validateSentimentEnhancement(env, { requestId });
    logger18.info("Sentiment validation completed", {
      requestId,
    ...
    logger18.error("Sentiment validation test failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleGenerateMorningPredictions(request, env) {
  ...
  const dateStr = today.toISOString().split("T")[0];
  try {
    logger18.info("\u{1F305} Morning predictions generation requested", { requestId, date: dateStr });
    const analysisKey = `analysis_${dateStr}`;
    const analysisData2 = await env.TRADING_RESULTS.get(analysisKey);
    if (!analysisData2) {
      logger18.warn("\u26A0\uFE0F No analysis data found for today", { requestId, date: dateStr });
      return new Response(JSON.stringify({
        success: false,
    ...
    const analysis = JSON.parse(analysisData2);
    ...
    if (success) {
      logger18.info("\u2705 Morning predictions generated successfully", { requestId, date: dateStr });
      const predictionsKey = `morning_predictions_${dateStr}`;
      ...
      logger18.error("\u274C Failed to generate morning predictions", { requestId, date: dateStr });
      return new Response(JSON.stringify({
        success: false,
    ...
    logger18.error("\u274C Morning predictions generation failed", {
      requestId,
      date: dateStr,
      error: error.message,
      stack: error.stack
...
async function handleStatusManagement(request, env) {
  ...
  const dateStr = today.toISOString().split("T")[0];
  try {
    logger18.info("\u{1F50D} [STATUS] Status management requested", { requestId, date: dateStr });
    ...
    logger18.error("\u274C [STATUS] Status management failed", { requestId, error: error.message });
    return new Response(JSON.stringify({
      success: false,
      error: error.message,
      request_id: requestId,
...
async function handleKVVerificationTest(request, env) {
  ...
  const dateStr = today.toISOString().split("T")[0];
  try {
    logger18.info("\u{1F9EA} [KV VERIFICATION] Comprehensive KV test requested", { requestId, date: dateStr });
    ...
    try {
      ...
      logger18.info("KV PUT test completed", { success, duration: Date.now() - putStartTime });
    } catch (error) {
      results.test_operations.put_with_verification = {
        success: false,
        error: error.message,
      ...
      logger18.error("KV PUT test failed", { error: error.message });
    }
    ...
    try {
      ...
      logger18.info("KV GET test completed", {
        success: true,
        duration: Date.now() - getStartTime,
      ...
      logger18.error("KV GET test failed", { error: error.message });
    }
    ...
    try {
      ...
      logger18.info("KV job status test completed", {
        success: true,
        duration: Date.now() - statusStartTime,
      ...
      logger18.error("KV job status test failed", { error: error.message });
    }
    ...
    try {
      ...
      logger18.info("KV dependency validation test completed", {
        success: true,
        duration: Date.now() - dependencyStartTime,
      ...
      logger18.error("KV dependency validation test failed", { error: error.message });
    }
    try {
      ...
      logger18.info("KV cleanup test completed", { success: true });
    } catch (error) {
      results.test_operations.cleanup = {
        success: false,
        error: error.message,
      ...
      logger18.error("KV cleanup test failed", { error: error.message });
    }
    ...
    logger18.info("\u{1F9EA} [KV VERIFICATION] Comprehensive KV test completed", {
      requestId,
      successRate,
      totalOperations,
    ...
    logger18.error("\u274C [KV VERIFICATION] Comprehensive KV test failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleGetResults(request, env) {
  ...
  try {
    ...
    if (storedData) {
      ...
      logger19.warn("No analysis results found for today", {
        requestId,
        analysisKey,
    ...
    logger19.error("Failed to retrieve results", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleFactTable(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger19.info("Fact table request received", { requestId });
    const factTableData = await getFactTableData(env);
    logger19.info("Fact table data retrieved", {
      requestId,
    ...
    logger19.error("Failed to generate fact table", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleCronHealth(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger19.info("Cron health check requested", { requestId });
    const healthStatus = await getCronHealthStatus(env);
    logger19.info("Cron health check completed", {
      requestId,
    ...
    logger19.error("Cron health check failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleKVDebug(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger19.info("KV debug operation requested", { requestId });
    ...
    logger19.debug("KV write operation successful", { requestId, testKey });
    const retrievedData = await env.TRADING_RESULTS.get(testKey);
    if (!retrievedData) {
      throw new Error("KV read operation failed - data not found");
    }
    const parsedData = JSON.parse(retrievedData);
    logger19.debug("KV read operation successful", { requestId, testKey });
    await env.TRADING_RESULTS.delete(testKey);
    logger19.debug("KV delete operation successful", { requestId, testKey });
    return new Response(JSON.stringify({
      success: true,
    ...
    logger19.error("KV debug operation failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleKVWriteTest(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger19.info("KV write test requested", { requestId });
    ...
    await env.TRADING_RESULTS.put(testKey, JSON.stringify(testData));
    logger19.info("KV write test successful", { requestId, testKey });
    return new Response(JSON.stringify({
      success: true,
      operation: "write",
      test_key: testKey,
    ...
    logger19.error("KV write test failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleKVReadTest(request, env) {
  ...
  try {
    if (!key) {
      logger19.warn("KV read test requested without key parameter", { requestId });
      return new Response(JSON.stringify({
        success: false,
        operation: "read",
    ...
    logger19.info("KV read test requested", { requestId, key });
    const data = await env.TRADING_RESULTS.get(key);
    if (data) {
      const parsedData = JSON.parse(data);
      logger19.info("KV read test successful", { requestId, key });
      return new Response(JSON.stringify({
        success: true,
        operation: "read",
        key,
      ...
      logger19.warn("KV read test - key not found", { requestId, key });
      return new Response(JSON.stringify({
        success: false,
        operation: "read",
    ...
    logger19.error("KV read test failed", {
      requestId,
      key,
      error: error.message,
      stack: error.stack
...
async function handleKVGet(request, env) {
  ...
  try {
    if (!key) {
      logger19.warn("KV get requested without key parameter", { requestId });
      return new Response(JSON.stringify({
        success: false,
        error: "Key parameter is required",
    ...
    logger19.info("KV get requested", { requestId, key });
    const data = await env.TRADING_RESULTS.get(key);
    if (data) {
      const parsedData = JSON.parse(data);
      logger19.info("KV get successful", {
        requestId,
        key,
      ...
      logger19.warn("KV get - key not found", { requestId, key });
      return new Response(JSON.stringify({
        success: false,
        error: "Key not found in KV storage",
        key,
...
init_facebook();
init_logging();
var logger20 = createLogger("health-handlers");
...
async function handleModelHealth(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger20.info("Model health check requested", { requestId });
    const healthResults = {
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
      request_id: requestId,
      models: {},
    ...
    if (env.AI) {
      try {
        ...
        logger20.debug("GPT-OSS-120B model test successful", { requestId });
      } catch (gptError) {
        healthResults.models.gpt_oss_120b = {
          status: "unhealthy",
        ...
        logger20.warn("GPT-OSS-120B model test failed", {
          requestId,
          error: gptError.message
        });
      }
      try {
        ...
        logger20.debug("DistilBERT model test successful", { requestId });
        ...
        healthResults.overall_status = "degraded";
        logger20.warn("DistilBERT model test failed", {
          requestId,
          error: distilbertError.message
        });
      }
      ...
      logger20.error("Cloudflare AI binding not available", { requestId });
    }
    try {
      if (env.MODEL_BUCKET) {
        ...
        logger20.debug("Neural network models accessible", { requestId });
      } else {
        healthResults.models.neural_networks = {
          status: "unavailable",
          error: "R2 model bucket not configured"
        };
        logger20.warn("R2 model bucket not configured", { requestId });
      }
    } catch (r2Error) {
      healthResults.models.neural_networks = {
        status: "unhealthy",
      ...
      logger20.error("Neural network models health check failed", {
        requestId,
        error: r2Error.message
      });
    }
    try {
      ...
      logger20.debug("KV storage health check successful", { requestId });
    } catch (kvError) {
      healthResults.models.kv_storage = {
        status: "unhealthy",
      ...
      healthResults.overall_status = "degraded";
      logger20.error("KV storage health check failed", {
        requestId,
        error: kvError.message
      });
    }
    ...
    logger20.error("Model health check failed completely", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleDebugEnvironment(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger20.info("Debug environment requested", { requestId });
    const envInfo = {
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
      request_id: requestId,
      environment: {
    ...
    logger20.info("Debug environment completed", {
      requestId,
    ...
    logger20.error("Debug environment failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
init_enhanced_analysis();
init_logging();
var logger21 = createLogger("facebook-handlers");
async function handleFacebookTest(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger21.info("Facebook test requested", { requestId });
    if (!env.FACEBOOK_PAGE_TOKEN || !env.FACEBOOK_RECIPIENT_ID) {
      logger21.warn("Facebook not configured for testing", { requestId });
      return new Response(JSON.stringify({
        success: false,
    ...
    const testMessage = `\u{1F9EA} **TEST MESSAGE**\\n\\n\u{1F916} DUAL AI SYSTEM TEST\\n\u{1F552} ${(/* @__PURE__ */ new Date()).toLocaleString()}\\n\\n\u{1F4CA} **NEW**: Dual AI Trading Analysis with GPT-OSS-120B + DistilBERT!\\n\\n\u{1F517} View Pre-Market Briefing: https://tft-trading-system.yanggf.workers.dev/pre-market-briefing`;
    ...
    if (facebookResponse.ok) {
      ...
      logger21.info("Facebook test successful", {
        requestId,
      ...
      logger21.error("Facebook API test failed", {
        requestId,
    ...
    logger21.error("Facebook test failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleTestAllFacebookMessages(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger21.info("Test all Facebook messages requested", { requestId });
    if (!env.FACEBOOK_PAGE_TOKEN || !env.FACEBOOK_RECIPIENT_ID) {
      logger21.warn("Facebook not configured for comprehensive testing", { requestId });
      return new Response(JSON.stringify({
        success: false,
    ...
    logger21.info("Facebook message tests completed", {
      requestId,
    ...
    logger21.error("Test all Facebook messages failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleWeeklyReport(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger21.info("Weekly report requested", { requestId });
    await sendWeeklyAccuracyReportWithTracking(env, requestId);
    logger21.info("Weekly report sent successfully", { requestId });
    ...
    logger21.error("Weekly report failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleFridayMarketCloseReport(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger21.info("Friday market close report requested", { requestId });
    const mockAnalysisResult = {
      symbols_analyzed: ["AAPL", "MSFT", "GOOGL", "TSLA", "NVDA"],
      trading_signals: {},
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    ...
    logger21.info("Friday market close report sent successfully", { requestId });
    ...
    logger21.error("Friday market close report failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleDailySummaryAPI(request, env) {
  ...
  try {
    ...
    if (!dailySummary) {
      logger22.info("Daily summary not found, generating new one", {
        requestId,
        targetDate
      });
      dailySummary = await generateDailySummary(targetDate, env);
    }
    logger22.info("Daily summary API completed", {
      requestId,
      targetDate,
    ...
    logger22.error("Daily summary API failed", {
      requestId,
      dateParam,
      error: error.message,
      stack: error.stack
...
async function handleDailySummaryPageRequest(request, env) {
  const requestId = crypto.randomUUID();
  try {
    logger22.info("Daily summary page requested", { requestId });
    const response = await handleDailySummaryPage(request, env);
    logger22.info("Daily summary page served", {
      requestId,
      status: response.status,
    ...
    logger22.error("Daily summary page failed", {
      requestId,
      error: error.message,
      stack: error.stack
    });
...
async function handleBackfillDailySummaries(request, env) {
  ...
  try {
    ...
    logger22.info("Backfill daily summaries requested", {
      requestId,
      daysRequested: days,
      skipExisting
    });
    if (days > 365) {
      logger22.warn("Backfill request exceeds maximum days", {
        requestId,
        daysRequested: days,
        maximum: 365
      });
    ...
    logger22.info("Backfill daily summaries completed", {
      requestId,
      daysRequested: days,
    ...
    logger22.error("Backfill daily summaries failed", {
      requestId,
      daysParam,
      error: error.message,
      stack: error.stack
...
async function handleVerifyBackfill(request, env) {
  ...
  try {
    ...
    logger22.info("Verify backfill requested", {
      requestId,
      daysToCheck
    });
    if (daysToCheck > 100) {
      logger22.warn("Verify backfill request exceeds maximum days", {
        requestId,
    ...
    logger22.info("Verify backfill completed", {
      requestId,
    ...
    logger22.error("Verify backfill failed", {
      requestId,
      daysParam,
      error: error.message,
      stack: error.stack
...
init_logging();
var logger23 = createLogger("intraday-analysis");
async function generateIntradayPerformance(analysisData2, morningPredictions, env) {
  logger23.info("Generating real-time intraday performance tracking");
  if (!morningPredictions || !analysisData2) {
    logger23.warn("Missing morning predictions or analysis data");
  ...
  try {
    ...
    logger23.error("Error generating intraday performance", { error: error.message });
    return getDefaultIntradayData();
  }
}
...
// src/modules/handlers/intraday-refactored.js
var logger24 = createLogger("intraday-refactored");
var IntradayDataRetriever = class {
  static {
  ...
  static async retrieveData(env, date, context = {}) {
    ...
    logger24.debug("\u{1F4E5} [INTRADAY] Retrieving intraday data", { requestId, date });
    try {
      const data = await getIntradayCheckData(date, env, { requestId });
      if (!data) {
        logger24.warn("\u26A0\uFE0F [INTRADAY] No intraday data found", { requestId, date });
        return null;
      }
      logger24.debug("\u2705 [INTRADAY] Intraday data retrieved", {
        requestId,
        date,
      ...
      logger24.error("\u274C [INTRADAY] Failed to retrieve intraday data", {
        requestId,
        date,
        error: error.message
      });
...
var IntradayPerformanceAnalyzer = class {
  static {
  ...
  static async analyze(data, context = {}) {
    const { requestId } = context;
    logger24.debug("\u{1F4CA} [INTRADAY] Analyzing performance", { requestId });
    try {
      const analysis = await generateIntradayPerformance(data, { requestId });
      logger24.debug("\u2705 [INTRADAY] Performance analysis completed", {
        requestId,
      ...
      logger24.error("\u274C [INTRADAY] Performance analysis failed", {
        requestId,
        error: error.message
      });
      throw error;
...
var IntradayHTMLGenerator = class {
  static {
  ...
  static async generateHTML(analysis, date, env, context = {}) {
    const { requestId } = context;
    logger24.debug("\u{1F3A8} [INTRADAY] Generating HTML", { requestId, date });
    try {
      ...
      logger24.debug("\u2705 [INTRADAY] HTML generation completed", {
        requestId,
        contentLength: content.length
      });
      return content;
    } catch (error) {
      logger24.error("\u274C [INTRADAY] HTML generation failed", {
        requestId,
        error: error.message
      });
      throw error;
...
var handleIntradayCheckRefactored = createReportHandler(
  "intraday-check",
...
var handleIntradayCheckEnhanced = createReportHandler(
  "intraday-check-enhanced",
  ["morning_predictions", "pre_market_briefing"],
  async (env, date, context) => {
    const { requestId } = context;
    logger24.debug("\u{1F517} [INTRADAY-ENHANCED] Enhanced dependency validation", { requestId, date });
    try {
      ...
      if (!consistencyResults.isValid) {
        logger24.warn("\u26A0\uFE0F [INTRADAY-ENHANCED] KV consistency issues detected", {
          requestId,
          consistentJobs: consistencyResults.consistentJobs,
          inconsistentJobs: consistencyResults.inconsistentJobs
        });
        if (consistencyResults.inconsistentJobs.length > 0) {
          logger24.info("\u{1F504} [INTRADAY-ENHANCED] Proceeding with available data", { requestId });
        }
      }
      const data = await IntradayDataRetriever.retrieveData(env, date, context);
      if (!data) {
      ...
      logger24.error("\u274C [INTRADAY-ENHANCED] Enhanced validation failed", {
        requestId,
        error: error.message
      });
...
init_logging();
var logger25 = createLogger("intraday-decomposed");
var IntradayDataModule = class {
  static {
    __name(this, "IntradayDataModule");
  }
  static async retrieve(env, date, context = {}) {
    const { requestId } = context;
    logger25.debug("\u{1F4E5} Retrieving intraday data", { requestId, date });
    try {
      const data = await getIntradayCheckData(date, env, { requestId });
      if (!data) {
        logger25.warn("\u26A0\uFE0F No intraday data found", { requestId, date });
        return null;
      }
      logger25.debug("\u2705 Intraday data retrieved", {
        requestId,
        signalCount: data.signals?.length || 0
      });
      return data;
    } catch (error) {
      logger25.error("\u274C Failed to retrieve intraday data", {
        requestId,
        date,
        error: error.message
      });
...
var IntradayAnalysisModule = class {
  static {
    __name(this, "IntradayAnalysisModule");
  }
  static async analyze(data, context = {}) {
    const { requestId } = context;
    logger25.debug("\u{1F4CA} Analyzing intraday performance", { requestId });
    try {
      const analysis = await generateIntradayPerformance(data, { requestId });
      logger25.debug("\u2705 Intraday analysis completed", {
        requestId,
        accuracy: analysis.overallAccuracy,
      ...
      logger25.error("\u274C Intraday analysis failed", {
        requestId,
        error: error.message
      });
      throw error;
...
var IntradayHTMLModule = class {
  static {
    __name(this, "IntradayHTMLModule");
  }
  static async generate(analysis, date, env, context = {}) {
    const { requestId } = context;
    logger25.debug("\u{1F3A8} Generating intraday HTML", { requestId, date });
    try {
      const metrics = createStandardMetrics({
        accuracy: analysis.overallAccuracy / 100,
        totalSignals: analysis.totalSignals,
      ...
      logger25.debug("\u2705 Intraday HTML generated", {
        requestId,
        contentLength: content.length
      });
      return content;
    } catch (error) {
      logger25.error("\u274C Failed to generate intraday HTML", {
        requestId,
        error: error.message
      });
      throw error;
...
var handleIntradayCheckDecomposed = createReportHandler(
  "intraday-check",
  ["morning_predictions", "pre_market_briefing"],
  async (env, date, context) => {
...
var handleIntradayCheckConsistent = createReportHandler(
  "intraday-check-consistent",
  ["morning_predictions", "pre_market_briefing"],
  async (env, date, context) => {
    const { requestId } = context;
    logger25.debug("\u{1F517} Enhanced dependency validation", { requestId, date });
    try {
      ...
      if (!consistencyResults.isValid) {
        logger25.warn("\u26A0\uFE0F KV consistency issues detected", {
          requestId,
          consistentJobs: consistencyResults.consistentJobs,
          inconsistentJobs: consistencyResults.inconsistentJobs
        });
        if (consistencyResults.inconsistentJobs.length > 0) {
          logger25.info("\u{1F504} Proceeding with available data", { requestId });
        }
      }
      const data = await IntradayDataModule.retrieve(env, date, context);
      if (!data) {
      ...
      logger25.error("\u274C Enhanced validation failed", {
        requestId,
        error: error.message
      });
      const { validateDependencies: validateDependencies2 } = await Promise.resolve().then(() => (init_kv_utils(), kv_utils_exports));
...
init_logging();
var logger26 = createLogger("pre-market-analysis");

// src/modules/handlers/briefing-handlers.js
init_validation();
init_kv_utils();
var logger27 = createLogger("briefing-handlers");
var handlePreMarketBriefing = createHandler("pre-market-briefing", async (request, env) => {
  const requestId = crypto.randomUUID();
  const startTime = Date.now();
  const today = /* @__PURE__ */ new Date();
  const dateStr = today.toISOString().split("T")[0];
  logger27.info("\u{1F680} [PRE-MARKET] Starting pre-market briefing generation", {
    requestId,
    date: dateStr,
    url: request.url,
    userAgent: request.headers.get("user-agent")?.substring(0, 100) || "unknown"
  ...
  validateEnvironment(env);
  logger27.debug("\u2705 [PRE-MARKET] Input validation passed", { requestId });
  logger27.debug("\u{1F517} [PRE-MARKET] Checking dependencies", { requestId });
  try {
    const validation = await validateDependencies(dateStr, ["analysis", "morning_predictions"], env);
    if (!validation.isValid) {
      logger27.warn("\u26A0\uFE0F [PRE-MARKET] Dependencies not satisfied", {
        requestId,
        missing: validation.missing,
        completionRate: validation.completionRate
      });
    ...
    logger27.info("\u2705 [PRE-MARKET] Dependencies validated", {
      requestId,
      completed: validation.completed,
      completionRate: validation.completionRate
    });
  } catch (error) {
    logger27.error("\u274C [PRE-MARKET] Dependency validation failed", {
      requestId,
      error: error.message
    });
  ...
  logger27.debug("\u{1F4CA} [PRE-MARKET] Retrieving pre-market briefing data", {
    requestId,
    date: dateStr
  });
  ...
  try {
    ...
    if (briefingData) {
      logger27.info("\u2705 [PRE-MARKET] Briefing data retrieved successfully", {
        requestId,
        signalCount: briefingData.signals?.length || 0,
        hasData: true
      });
    } else {
      logger27.warn("\u26A0\uFE0F [PRE-MARKET] No briefing data found for today", {
        requestId
      });
    }
  } catch (error) {
    logger27.error("\u274C [PRE-MARKET] Failed to retrieve briefing data", {
      requestId,
      error: error.message
    });
    await updateJobStatus("pre_market_briefing", dateStr, "failed", env, {
  ...
  const generationStartTime = Date.now();
  logger27.debug("\u{1F3A8} [PRE-MARKET] Generating HTML content", { requestId });
  const html = generatePreMarketBriefingHTML(briefingData, today);
  const totalTime = Date.now() - startTime;
  const generationTime = Date.now() - generationStartTime;
  logger27.info("\u2705 [PRE-MARKET] Pre-market briefing generated successfully", {
    requestId,
    totalTimeMs: totalTime,
    generationTimeMs: generationTime,
...
init_kv_utils();
init_validation();
var logger28 = createLogger("intraday-handlers");
var handleIntradayCheck = createHandler("intraday-check", async (request, env) => {
  const requestId = crypto.randomUUID();
  const startTime = Date.now();
  const today = /* @__PURE__ */ new Date();
  const dateStr = today.toISOString().split("T")[0];
  logger28.info("\u{1F4CA} [INTRADAY] Starting intraday performance check generation", {
    requestId,
    date: dateStr,
    url: request.url,
    userAgent: request.headers.get("user-agent")?.substring(0, 100) || "unknown"
  ...
  validateRequest(request);
  validateEnvironment(env);
  logger28.debug("\u2705 [INTRADAY] Input validation passed", { requestId });
  logger28.debug("\u{1F517} [INTRADAY] Checking dependencies", { requestId });
  try {
    const validation = await validateDependencies(dateStr, ["morning_predictions", "pre_market_briefing"], env);
    if (!validation.isValid) {
      logger28.warn("\u26A0\uFE0F [INTRADAY] Dependencies not satisfied", {
        requestId,
        missing: validation.missing,
        completionRate: validation.completionRate
      });
    ...
    logger28.info("\u2705 [INTRADAY] Dependencies validated", {
      requestId,
      completed: validation.completed,
      completionRate: validation.completionRate
    });
  } catch (error) {
    logger28.error("\u274C [INTRADAY] Dependency validation failed", {
      requestId,
      error: error.message
    });
  ...
  logger28.debug("\u{1F50D} [INTRADAY] Retrieving intraday check data", {
    requestId,
    date: dateStr
  });
  ...
  try {
    ...
    if (intradayData) {
      logger28.info("\u2705 [INTRADAY] Intraday data retrieved successfully", {
        requestId,
        signalCount: intradayData.signals?.length || 0,
        hasData: true
      });
    } else {
      logger28.warn("\u26A0\uFE0F [INTRADAY] No intraday data found for today", {
        requestId
      });
    }
  } catch (error) {
    logger28.error("\u274C [INTRADAY] Failed to retrieve intraday data", {
      requestId,
      error: error.message
    });
    await updateJobStatus("intraday_check", dateStr, "failed", env, {
  ...
  const generationStartTime = Date.now();
  logger28.debug("\u{1F3A8} [INTRADAY] Generating HTML content", {
    requestId,
  ...
  const totalTime = Date.now() - startTime;
  const generationTime = Date.now() - generationStartTime;
  logger28.info("\u2705 [INTRADAY] Intraday performance check generated successfully", {
    requestId,
    totalTimeMs: totalTime,
    generationTimeMs: generationTime,
    signalCount: intradayData?.signals?.length || 0,
...
init_logging();
init_rate_limiter();
var logger29 = createLogger("end-of-day-analysis");

// src/modules/handlers/end-of-day-handlers.js
var logger30 = createLogger("end-of-day-handlers");
var handleEndOfDaySummary = createHandler("end-of-day-summary", async (request, env) => {
  const requestId = crypto.randomUUID();
  const startTime = Date.now();
  logger30.info("\u{1F3C1} [END-OF-DAY] Starting end-of-day summary generation", {
    requestId,
    url: request.url,
    userAgent: request.headers.get("user-agent")?.substring(0, 100) || "unknown"
  });
  const today = /* @__PURE__ */ new Date();
  logger30.debug("\u{1F4CA} [END-OF-DAY] Retrieving end-of-day summary data", {
    requestId,
    date: today.toISOString().split("T")[0]
  });
  ...
  try {
    ...
    if (endOfDayData) {
      logger30.info("\u2705 [END-OF-DAY] End-of-day data retrieved successfully", {
        requestId,
      ...
      logger30.warn("\u26A0\uFE0F [END-OF-DAY] No end-of-day data found for today", {
        requestId
      });
    }
  } catch (error) {
    logger30.error("\u274C [END-OF-DAY] Failed to retrieve end-of-day data", {
      requestId,
      error: error.message
    });
  }
  const generationStartTime = Date.now();
  logger30.debug("\u{1F3A8} [END-OF-DAY] Generating HTML content", {
    requestId,
  ...
  const totalTime = Date.now() - startTime;
  const generationTime = Date.now() - generationStartTime;
  logger30.info("\u2705 [END-OF-DAY] End-of-day summary generated successfully", {
    requestId,
    totalTimeMs: totalTime,
    generationTimeMs: generationTime,
...
init_logging();
init_config();
var logger31 = createLogger("performance-baseline");
var PerformanceBaseline = class {
  ...
  async recordMeasurement(operation, value, metadata = {}) {
    ...
    await this.env.TRADING_RESULTS.put(key, JSON.stringify(measurement), {
      expirationTtl: CONFIG2.KV_STORAGE.GRANULAR_TTL
      // 90 days
    });
    ...
    logger31.info("Performance measurement recorded", {
      operation,
      value,
  ...
  getOperationTarget(operation) {
    const targetMap = {
      "api_response_time": CONFIG2.BUSINESS_KPI.RESPONSE_TIME_TARGET_MS,
      "analysis_duration": 3e4,
      // 30 seconds
      "kv_operation_time": 1e3,
      // 1 second
      "prediction_accuracy": CONFIG2.BUSINESS_KPI.PREDICTION_ACCURACY_TARGET * 100
    };
  ...
  async checkPerformanceAlerts() {
    ...
    if (alerts.length > 0) {
      logger31.warn("Performance alerts detected", {
...
init_logging();
init_config();
var logger32 = createLogger("alert-system");
var AlertSeverity = {
  LOW: "low",
  MEDIUM: "medium",
  HIGH: "high",
...
var AlertManager = class {
  ...
  async sendAlert(alert) {
    try {
      if (this.isAlertSuppressed(alert)) {
        logger32.debug("Alert suppressed by rules", { alert: alert.id });
      ...
      logger32.error("Failed to send alert", {
        alertId: alert.id,
        error: error.message,
        stack: error.stack
      });
  ...
  suppressAlert(type, operation, durationMs) {
    ...
    logger32.info("Alert suppression added", {
      type,
      operation,
      durationMs,
...
var handleOptimizationTest = createHandler("optimization-test", async (request, env, ctx) => {
  ...
  if (testType === "all" || testType === "config") {
    results.tests.configuration = {
      ...
      tradingSymbols: CONFIG2.TRADING.SYMBOLS,
      businessKPIs: CONFIG2.BUSINESS_KPI,
      symbolValidation: {
  ...
  switch (errorType) {
    case "validation":
      return createErrorResponse("Invalid symbol provided", {
        status: 400,
        details: { validSymbols: CONFIG2.TRADING.SYMBOLS },
        requestId: ctx.requestId,
        service: "error-test"
      });
    case "timeout":
