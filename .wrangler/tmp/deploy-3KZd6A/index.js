...
function calculateGLM45Cost(inputLength, outputLength) {
  ...
  return {
    ...
    input_cost: inputTokens / 1e6 * 0.59,
    // GLM-4.5 input cost
    output_cost: outputTokens / 1e6 * 2.19,
    // GLM-4.5 output cost
    total_cost: inputTokens / 1e6 * 0.59 + outputTokens / 1e6 * 2.19
  };
}
...
var init_cloudflare_ai_sentiment_pipeline = __esm({
  "src/modules/cloudflare_ai_sentiment_pipeline.js"() {
    ...
    __name(calculateGLM45Cost, "calculateGLM45Cost");
...
`;
  ...
  console.log(`\u{1F4F1} [FB-MORNING] ${cronExecutionId} Morning predictions sent via Facebook`);
...
async function getBasicSentiment(symbol, newsData, env) {
  console.log(`\u{1F50D} SENTIMENT DEBUG: Starting getBasicSentiment for ${symbol}`);
  ...
  try {
    if (env.MODELSCOPE_API_KEY) {
      console.log(`\u{1F50D} SENTIMENT DEBUG: ModelScope API key found, calling getModelScopeAISentiment...`);
      console.log(`   \u{1F916} Using ModelScope GLM-4.5 sentiment analysis for ${symbol}...`);
      const result = await getModelScopeAISentiment(symbol, newsData, env);
      console.log(`\u{1F50D} SENTIMENT DEBUG: ModelScope result:`, {
        sentiment: result?.sentiment,
        confidence: result?.confidence,
    ...
    console.log(`\u{1F50D} SENTIMENT DEBUG: No ModelScope API key, using rule-based sentiment`);
    console.log(`   \u{1F4DD} Using rule-based sentiment analysis for ${symbol}...`);
    const ruleResult = getRuleBasedSentiment(newsData);
    console.log(`\u{1F50D} SENTIMENT DEBUG: Rule-based result:`, ruleResult);
    return ruleResult;
  } catch (error) {
    console.error(`\u{1F50D} SENTIMENT DEBUG: ModelScope failed, error:`, {
      error_message: error.message,
...
function checkDirectionAgreement(sentimentDirection, technicalDirection) {
  ...
  if (normalizeSentiment === "UP" && normalizeTechnical === "BULLISH" || normalizeSentiment === "DOWN" && normalizeTechnical === "BEARISH" || normalizeSentiment === "NEUTRAL" && (normalizeTechnical === "FLAT" || normalizeTechnical === "NEUTRAL")) {
    return true;
  }
  return false;
}
...
async function validateSentimentEnhancement(env) {
  ...
  try {
    ...
    const modelScopeSuccess = sentimentResult && sentimentResult.source === "modelscope_glm45" && !sentimentResult.error_details && sentimentResult.confidence > 0 && sentimentResult.method !== "rule_based";
