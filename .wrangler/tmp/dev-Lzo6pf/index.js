...
// .wrangler/tmp/bundle-RzFtCn/checked-fetch.js
...
async function waitForConsistency(key, condition, env, options = {}) {
  ...
  const startTime = Date.now();
  let attempt = 0;
  logger5.info("Starting consistency wait", {
    key,
    strategy,
    timeout,
    description
  ...
  while (Date.now() - startTime < timeout) {
    try {
      ...
      if (result) {
        ...
        logger5.info("Consistency achieved", {
          key,
          attempt,
          elapsed: elapsed2,
          description
      ...
      logger5.debug("Consistency check failed", {
        key,
        attempt,
        error: error.message
      });
    ...
    logger5.debug("Waiting for consistency retry", {
      key,
      attempt,
      delay,
      description
  ...
  logger5.warn("Consistency timeout", {
    key,
    attempt,
    elapsed,
    timeout,
  ...
  await waitForConsistency(
    ...
    async (env2, key) => {
      try {
        ...
        logger5.debug("Dependency consistency check failed", {
          date,
          error: error.message
        });
        return false;
...
async function executeAtomicLikeOperation(operationId, operation, env, options = {}) {
  ...
  const startTime = Date.now();
  logger5.info("Starting atomic-like operation", {
    operationId,
    timeout,
    rollbackOnFailure
  });
  try {
    ...
    if (consistencyAchieved) {
      logger5.info("Atomic-like operation completed with consistency", {
        operationId,
      ...
      logger5.warn("Atomic-like operation completed without consistency", {
        operationId,
        duration: Date.now() - startTime
      });
      if (rollbackOnFailure && options.rollback) {
        try {
          ...
          logger5.info("Rollback completed", { operationId });
        } catch (rollbackError) {
          logger5.error("Rollback failed", {
            operationId,
    ...
    logger5.error("Atomic-like operation failed", {
      operationId,
      error: error.message,
      duration: Date.now() - startTime
    });
    if (rollbackOnFailure && options.rollback) {
      try {
        ...
        logger5.info("Rollback completed after error", { operationId });
      } catch (rollbackError) {
        logger5.error("Rollback failed after error", {
          operationId,
          error: rollbackError.message
        });
      }
...
var logger5, CONSISTENCY_CONFIG, kv_consistency_default;
var init_kv_consistency = __esm({
  "src/modules/kv-consistency.js"() {
    init_checked_fetch();
    init_modules_watch_stub();
    init_logging();
    logger5 = createLogger("kv-consistency");
    CONSISTENCY_CONFIG = {
...
async function getWithRetry(key, env, maxRetries = 3, delay = 1e3) {
  logger6.debug("KV GET operation started", { key, maxRetries, delay });
  for (let i = 0; i < maxRetries; i++) {
    try {
      const result = await env.TRADING_RESULTS.get(key);
      if (result) {
        if (i > 0) {
          logger6.info("KV retry successful", { key, attempt: i + 1 });
        } else {
          logger6.info("KV GET successful", { key, bytes: result.length });
        }
        return result;
      } else {
        logger6.debug("KV GET returned null", { key, attempt: i + 1 });
      }
    } catch (error) {
      logger6.warn("KV operation failed, retrying", { key, attempt: i + 1, error: error.message });
    }
    if (i < maxRetries - 1) {
      const retryDelay = delay * Math.pow(2, i);
      logger6.debug("Waiting before retry", { key, delay: retryDelay });
      await new Promise((resolve) => setTimeout(resolve, retryDelay));
    }
  }
  logger6.error("KV GET failed after all retries", { key, maxRetries });
  throw new Error(`KV key ${key} not found after ${maxRetries} retries`);
}
async function putWithVerification(key, value, env, options = {}) {
  logger6.info("KV PUT operation started", {
    key,
    bytes: value.length,
  ...
  try {
    ...
    if (verifyKey === value) {
      logger6.info("KV PUT successful and verified", {
        key,
        bytes: value.length,
        verification: "passed"
      });
      ...
      logger6.error("KV PUT verification failed - value mismatch", {
        key,
    ...
    logger6.error("KV PUT operation failed", {
      key,
      error: error.message,
      bytes: value.length
    });
...
async function deleteWithVerification(key, env) {
  logger6.info("KV DELETE operation started", { key });
  try {
    const exists = await env.TRADING_RESULTS.get(key);
    if (!exists) {
      logger6.warn("KV DELETE - key does not exist", { key });
      return true;
    }
    await env.TRADING_RESULTS.delete(key);
    const verify = await env.TRADING_RESULTS.get(key);
    if (verify === null) {
      logger6.info("KV DELETE successful and verified", { key });
      return true;
    } else {
      logger6.error("KV DELETE verification failed - key still exists", { key });
      return false;
    }
  } catch (error) {
    logger6.error("KV DELETE operation failed", { key, error: error.message });
    throw error;
  }
}
function logKVOperation(operation, key, success, details = {}) {
  if (success) {
    logger6.info("\u2705 KV OPERATION SUCCESS", {
      operation,
      key,
      ...details,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    ...
    logger6.error("\u274C KV OPERATION FAILED", {
      operation,
      key,
      ...details,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
...
async function getMultipleWithRetry(keys, env) {
  ...
  for (const key of keys) {
    try {
      ...
      logger6.warn("Failed to get KV key in batch", { key, error: error.message });
      results[key] = null;
    }
  }
  return results;
...
async function updateJobStatus(jobType, date, status, env, metadata = {}) {
  ...
  logger6.info("Updating job status", {
    jobType,
    date,
    status,
    key: statusKey,
...
async function getJobStatus(jobType, date, env) {
  ...
  try {
    ...
    logger6.debug("Job status not found", { jobType, date });
    return null;
  }
}
async function validateDependencies(date, requiredJobs, env) {
  ...
  logger6.info("Dependency validation completed", {
    date,
    isValid,
    requiredJobs,
    completed,
...
async function createWeeklyInput(fridayDate, env) {
  ...
  logger6.info("Creating weekly input", { weekKey, fridayDate });
  const eodSummaries = {};
  for (let i = 0; i < weekDates.length; i++) {
    ...
    if (status && status.status === "done") {
      ...
      logger6.debug("EOD summary found for weekly input", { date, dayName: dayNames[i] });
    } else {
      logger6.warn("EOD summary missing for weekly input", { date, dayName: dayNames[i] });
    }
  }
  const weeklyInput = {
  ...
  try {
    ...
    logger6.info("Weekly input created successfully", {
      weekKey,
      eodCount: Object.keys(eodSummaries).length
    });
    return weeklyInput;
  } catch (error) {
    logger6.error("Failed to create weekly input", { weekKey, error: error.message });
    throw error;
  }
}
async function getWeeklyInput(weekKey, env) {
  try {
    ...
    logger6.info("Weekly input not found, building on-demand", { weekKey });
...
// .wrangler/tmp/bundle-RzFtCn/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-RzFtCn/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
