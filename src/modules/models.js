...
/**
 * Run TFT (Temporal Fusion Transformer) model inference
 */
export async function runTFTInference(symbol, ohlcv, options = {}) {
  console.log(`   🧠 Running TFT neural network inference for ${symbol}...`);
  ...
  console.log(`   ✅ TFT: ${prediction.direction} $${currentPrice.toFixed(2)} → $${prediction.predicted_price.toFixed(2)} (${(prediction.confidence * 100).toFixed(1)}%)`);
  ...
  return {
    ...
    inference_time: 1 // Simulated inference time
  };
}

/**
 * Run N-HITS (Neural Hierarchical Interpolation) model inference
 */
export async function runNHITSInference(symbol, ohlcv, options = {}) {
  console.log(`   🔄 Running N-HITS neural network inference for ${symbol}...`);
  ...
  console.log(`   ✅ N-HITS: ${prediction.direction} $${currentPrice.toFixed(2)} → $${prediction.predicted_price.toFixed(2)} (${(prediction.confidence * 100).toFixed(1)}%)`);
