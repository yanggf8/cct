...
import { toAppError, AppError, isNetworkError, isDatabaseError, isValidationError, isTimeoutError } from '../types/errors.js';
 ...
 */
export async function wrapAsyncErrors<T>(
  fn: () => Promise<T>,
  context?: Record<string, any>
): Promise<T> {
  try {
 ...
 */
export function wrapSyncErrors<T>(
  fn: () => T,
  context?: Record<string, any>
): T {
  try {
 ...
 */
export function handleKVError(error: unknown, operation: string, key?: string): never {
 ...
 */
export function handleAIError(error: unknown, model: string, input?: any): never {
 ...
 */
export function migrateCatchBlocks(code: string, operationName: string): string {
  return code
    // Replace basic catch blocks
    .replace(/catch\s*\(\s*error\s*:\s*any\s*\)\s*\{\s*console\.error\(error\);\s*\}/g,
      `catch (error) { handleError(error, { operation: '${operationName}' }); }`)

    // Replace return null patterns
    .replace(/catch\s*\(\s*error\s*:\s*any\s*\)\s*\{\s*return\s+null;\s*\}/g,
      `catch (error) { return handleErrorWithFallback(error, null, { operation: '${operationName}' }); }`)

    // Replace throw error patterns
    .replace(/catch\s*\(\s*error\s*:\s*any\s*\)\s*\{\s*throw\s+error;\s*\}/g,
      `catch (error) { handleError(error, { operation: '${operationName}' }); }`)
